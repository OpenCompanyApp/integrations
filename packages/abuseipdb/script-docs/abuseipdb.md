...
```ruby
check = app.integrations.abuseipdb.check(ip_address: "198.51.100.10", max_age_in_days: 30, verbose: true)
block = app.integrations.abuseipdb.check_block(network: "198.51.100.0/24", max_age_in_days: 15)
feed = app.integrations.abuseipdb.blacklist(confidence_minimum: 90, ip_version: 4)
```
