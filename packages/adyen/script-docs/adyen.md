...
```ruby
app.integrations.adyen.start_transaction(body: {amount: {value: 1000, currency: "EUR"}, paymentMethod: {type: "scheme"}, reference: "ORDER-123", returnUrl: "https://example.test/return"})
```
...
```ruby
app.integrations.adyen.get_list_available_payment_methods(body: {amount: {value: 1000, currency: "EUR"}, countryCode: "NL", channel: "Web"})
```
Management paths accept explicit IDs or use configured defaults for common account identifiers.

```ruby
app.integrations.adyen.adyen_management_get_merchants_merchant_id_stores(merchant_id: "MerchantECOM", page_size: 20)
app.integrations.adyen.adyen_management_get_companies_company_id_users(company_id: "ExampleCompany")
```
Additional documented query parameters can be passed exactly as named through `query`.

```ruby
app.integrations.adyen.adyen_management_get_stores(query: {merchantId: "MerchantECOM", pageSize: 50})
```
## Return Shape
