# Affinity JavaScript API Reference
...
```js
var result = app.integrations.affinity.list_contacts({
  limit: 100,
})
```
...
```js
var result = app.integrations.affinity.list_organization_field_values({
...
```js
var result = app.integrations.affinity.update_list_entry_field({
  list_id: "list_test",
...
```js
var result = app.integrations.affinity.semantic_search({
  payload: {
    ["entity-type"]: "person",
...
```js
var result = app.integrations.affinity.api_get({
  path: "/persons",
  params: {
    limit: 50,
  }
