# Agora Cloud Recording — JavaScript API Supplement
...
```js
var acquired = app.integrations.agora.acquire_recording_resource({
  cname: "team-standup",
  uid: "527841",
  scene: 0,
...
var resource_id = acquired.resourceId
```
...
```js
var started = app.integrations.agora.start_recording({
...
```js
var status = app.integrations.agora.query_recording({
  resource_id: resource_id,
  sid: sid,
  mode: "mix",
})
...
```js
var stopped = app.integrations.agora.stop_recording({
  resource_id: resource_id,
  sid: sid,
  mode: "mix",
  cname: "team-standup",
...
console.log(stopped.serverResponse.uploadingStatus)
```
...
```js
var ips = app.integrations.agora.get_notification_ips({})

for (const host of (ips.data.service.hosts)) {
  console.log(host.primaryIP)
}
```
...
```js
app.integrations.agora.acquire_recording_resource({ /* parameters */ })
app.integrations.agora.default.acquire_recording_resource({ /* parameters */ })
app.integrations.agora.production.acquire_recording_resource({ /* parameters */ })
```