...
```ruby
metrics = app.integrations.ahrefs.get_metrics(params: {target: "example.test", date: "2026-05-06", mode: "domain", country: "us"})
dr = app.integrations.ahrefs.get_domain_rating(params: {target: "example.test", date: "2026-05-06"})
backlinks = app.integrations.ahrefs.list_backlinks(target: "example.test", mode: "domain", limit: 25)
broken = app.integrations.ahrefs.list_broken_backlinks(params: {target: "example.test", mode: "domain", limit: 25})
competitors = app.integrations.ahrefs.list_organic_competitors(params: {target: "example.test", mode: "domain", date: "2026-05-06", country: "us"})
```
...
```ruby
usage = app.integrations.ahrefs.get_limits_and_usage()
```
...
```ruby
result = app.integrations.ahrefs.api_get(path: "/v3/site-explorer/metrics-by-country", params: {target: "example.test", date: "2026-05-06", mode: "domain"})
```