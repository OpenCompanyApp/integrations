# Aircall JavaScript API Reference
...
```js
var result = app.integrations.aircall.list_calls({
  from: 1704067200,
  to: 1706745600,
  per_page: 50,
  fetch_contact: true,
...
```js
var result = app.integrations.aircall.tag_call({
  call_id: "123",
  tags: [ 456, 789 ],
})
```
...
```js
var result = app.integrations.aircall.transfer_call({
  call_id: "123",
  user_id: "456",
})
```
...
```js
var result = app.integrations.aircall.create_webhook({
...
```js
var result = app.integrations.aircall.api_get({
  path: "/calls/search",
  params: {
