...
```ruby
result = app.integrations.aircall.list(from: 1704067200, to: 1706745600, per_page: 50, fetch_contact: true)
```
Tag a call:

```ruby
result = app.integrations.aircall.tag(call_id: "123", tags: [456, 789])
```
Transfer a call:

```ruby
result = app.integrations.aircall.transfer(call_id: "123", user_id: "456")
```
Create a webhook:

```ruby
result = app.integrations.aircall.create_webhook(url: "https://example.test/aircall/webhook", events: ["call.created", "call.ended"])
```
## Coverage
...
```ruby
result = app.integrations.aircall.api_get(path: "/calls/search", params: {phone_number: "+15551234567"})
```
