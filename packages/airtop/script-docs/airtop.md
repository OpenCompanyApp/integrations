...
```ruby
session = app.integrations.airtop.create_session(body: {configuration: {timeoutMinutes: 10}})
```
```ruby
window = app.integrations.airtop.create_window(session_id: session.id, body: {url: "https://example.test"})
```
...
```ruby
app.integrations.airtop.load_url(session_id: "sess_123", window_id: "win_123", body: {url: "https://example.test"})
```
```ruby
app.integrations.airtop.scrape(session_id: "sess_123", window_id: "win_123", body: {includeLinks: true})
```
Async operations return a request identifier. Poll the official status endpoint:

```ruby
app.integrations.airtop.get_status_asynchronous_request(request_id: "req_123")
```
## Return Shape
