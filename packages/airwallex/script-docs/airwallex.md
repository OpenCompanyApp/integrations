...
```ruby
token = app.integrations.airwallex.obtain_access_token()
account = app.integrations.airwallex.retrieve_account_details()
customers = app.integrations.airwallex.get_list_blling_customers(page_num: "0", page_size: "20")
```
Use fake account IDs, customer IDs, card IDs, payment IDs, transfer IDs, API keys, client IDs, and access tokens in tests and examples. Never store real Airwallex credentials, customer details, payment details, card data, bank account details, or production account IDs in fixtures or Ruby examples.
