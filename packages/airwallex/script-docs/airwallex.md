...
```js
var token = app.integrations.airwallex.authentication_obtain_access_token({})

var account = app.integrations.airwallex.scale_retrieve_account_details({})

var customers = app.integrations.airwallex.billing_get_list_of_blling_customers({
  page_num: "0",
  page_size: "20",
})
```
Use fake account IDs, customer IDs, card IDs, payment IDs, transfer IDs, API keys, client IDs, and access tokens in tests and examples. Never store real Airwallex credentials, customer details, payment details, card data, bank account details, or production account IDs in fixtures or JavaScript examples.
