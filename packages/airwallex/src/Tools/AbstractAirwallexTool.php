...
/**
 * Shared executor for Airwallex endpoint-specific tools.
 *
 * Handles configured-state checks, path/query/header mapping, body shaping,
 * host selection, authentication mode selection, dispatch, and error conversion.
 */
abstract class AbstractAirwallexTool implements Tool
{
    ...
    /**
     * @param  AirwallexService  $service  Airwallex API client.
     */
    public function __construct(protected AirwallexService $service) {}
    ...
    /**
     * Execute the mapped Airwallex API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Airwallex integration is not configured.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the Airwallex API request schema.');
        }
