# Algolia JavaScript API Reference
...
```js
var result = app.integrations.algolia.search({
  indexName: "products",
...
var multi = app.integrations.algolia.search_multiple({
  requests: [
...
var facets = app.integrations.algolia.search_facet_values({
  indexName: "products",
  facetName: "brand",
...
```js
var object = app.integrations.algolia.get_object({
  indexName: "products",
  objectID: "prod-123",
})
...
```js
var indices = app.integrations.algolia.list_indices({})
var settings = app.integrations.algolia.get_settings({ indexName: "products" })
...
var found = app.integrations.algolia.search_synonyms({
  indexName: "products",
  params: { query: "phone" },
})
```
...
var rules = app.integrations.algolia.search_rules({
  indexName: "products",
...
```js
var keys = app.integrations.algolia.list_api_keys({})
var logs = app.integrations.algolia.list_logs({
  query: { length: 10, type: "all" },
})

var task = app.integrations.algolia.get_task({
  indexName: "products",
  taskID: "123456",
})
```
...
```js
var response = app.integrations.algolia.api_get({
