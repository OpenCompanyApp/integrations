...
```ruby
app.integrations.apify.run_actor(actor_id: "apify/web-scraper", query: {waitForFinish: 60}, body: {startUrls: [{url: "https://example.test"}]})
```
```ruby
app.integrations.apify.apify_actor_run_get(run_id: "run_123")
```
```ruby
app.integrations.apify.get_dataset_items(dataset_id: "dataset_123", clean: true, format: "json", limit: 100)
```
```ruby
app.integrations.apify.get_record(store_id: "store_123", record_key: "OUTPUT")
```
Additional documented query parameters can be passed exactly as named through `query`.

```ruby
app.integrations.apify.get_list_actors(query: {offset: 0, limit: 20, my: true})
```
## Return Shape
