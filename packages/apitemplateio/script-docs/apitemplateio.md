...
```ruby
result = app.integrations.apitemplateio.create_pdf(template_id: "tpl_invoice", data: {invoice_number: "INV-001", company_name: "Example Corp", amount: "$500.00"}, filename: "invoice-INV-001.pdf", expiration: 1440)
```
...
```ruby
result = app.integrations.apitemplateio.create_image(template_id: "tpl_social", output_image_type: "pngOnly", overrides: [{name: "title", text: "Launch Week"}, {name: "hero", src: "https://example.test/hero.png"}])
```
...
```ruby
result = app.integrations.apitemplateio.create_pdf_from_html(body: "<h1>Hello {{name}}</h1>", css: "<style>h1 { color: #2563eb; }</style>", data: {name: "World"}, settings: {paper_size: "A4", orientation: "1", margin_top: "40", margin_right: "10", margin_bottom: "40", margin_left: "10"})
```
## create_pdf_from_url
...
```ruby
result = app.integrations.apitemplateio.create_pdf_from_url(url: "https://example.test/report", settings: {paper_size: "A4", print_background: "1"})
```
...
```ruby
result = app.integrations.apitemplateio.create_pdf_from_markdown(body: "# {{title}}\n\nGenerated report body.", data: {title: "Monthly Report"}, css: "<style>body { font-family: sans-serif; }</style>")
```
## merge_pdfs
...
```ruby
result = app.integrations.apitemplateio.merge_pdfs(urls: ["https://example.test/a.pdf", "https://example.test/b.pdf"], export_type: "json", expiration: 1440)
```
## list_objects
...
```ruby
result = app.integrations.apitemplateio.list_generated_objects(limit: 50, transaction_type: "PDF", template_id: "tpl_invoice")
```
...
```ruby
result = app.integrations.apitemplateio.delete_generated_object(transaction_ref: "txn_123")
```
## get_current_user
...
```ruby
account = app.integrations.apitemplateio.get_account_information()
```
## list_templates
...
```ruby
templates = app.integrations.apitemplateio.list(limit: 100, format: "PDF", group_name: "invoices")
```
...
```ruby
template = app.integrations.apitemplateio.get(template_id: "tpl_invoice")
```
## update_template
...
```ruby
result = app.integrations.apitemplateio.update(template_id: "tpl_invoice", body: "<h1>{{title}}</h1>", css: "<style>body { background: white; }</style>", settings: {custom_footer: "<div>Page <span class='pageNumber'></span></div>"})
```
## Multi-Account Usage
