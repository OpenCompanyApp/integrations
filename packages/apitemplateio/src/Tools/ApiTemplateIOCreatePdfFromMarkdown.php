 ...
 *
 * Converts Markdown plus optional Jinja2 data and rendering settings into a PDF.
 */
class ApiTemplateIOCreatePdfFromMarkdown implements Tool
{
    /**
    ...
    {
        return 'apitemplateio_create_pdf_from_markdown';
    }

    public function description(): string
    {
        return 'Generate a PDF from Markdown, optional CSS, dynamic data, and rendering settings.';
    }

    public function parameters(): array
    {
        return [
            'body' => ['type' => 'string', 'required' => true, 'description' => 'Markdown content. Jinja2 variables can reference keys from data.'],
            'css' => ['type' => 'string', 'description' => 'Optional CSS, usually including a style tag.'],
            'data' => ['type' => 'object', 'description' => 'Values for dynamic variables in the Markdown body.'],
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            return ToolResult::success($this->service->createPdfFromMarkdown(
                $body,
                (string) ($args['css'] ?? ''),
                $data,
                $settings,
