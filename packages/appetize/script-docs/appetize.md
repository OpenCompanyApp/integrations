...
```ruby
app.integrations.appetize.create(payload: {platform: "android", url: "https://example.test/app.apk", note: "QA smoke build"})
```
`appetize_get_usage_summary` returns usage rows grouped by month and public key. A paginated example:

```ruby
app.integrations.appetize.get_usage_summary(query: {startMonth: "2026-05", nextKey: "cursor"})
```
