...
```ruby
users = app.integrations.appwrite.list_users(queries: ["limit(25)", "orderDesc(\"$createdAt\")"], search: "ada")
```
## Databases

```ruby
database = app.integrations.appwrite.create_database(database_id: "crm", name: "CRM")
collection = app.integrations.appwrite.create_collection(database_id: "crm", collection_id: "contacts", name: "Contacts", permissions: ["read(\"any\")"], document_security: true)
document = app.integrations.appwrite.create_document(database_id: "crm", collection_id: "contacts", document_id: "unique()", data: {name: "Ada Example", email: "ada@example.test"})
```
...
```ruby
user = app.integrations.appwrite.create_user(user_id: "unique()", email: "ada@example.test", password: "replace-with-generated-secret", name: "Ada Example")
app.integrations.appwrite.update_user_status(user_id: user["$id"], status: false)
team = app.integrations.appwrite.create_team(team_id: "ops", name: "Operations", roles: ["owner"])
```
## Storage

```ruby
bucket = app.integrations.appwrite.create_bucket(bucket_id: "imports", name: "Imports", file_security: true, maximum_file_size: 10485760, allowed_file_extensions: ["csv", "json"])
files = app.integrations.appwrite.list_files(bucket_id: "imports", queries: ["limit(10)"])
```
...
```ruby
execution = app.integrations.appwrite.create_execution(function_id: "sync_contacts", body: "{\"dry_run\":true}", async: false, method: "POST")
```
...
```ruby
topic = app.integrations.appwrite.create_topic(topic_id: "product_updates", name: "Product updates", subscribe: ["user:example"])
email = app.integrations.appwrite.create_email(message_id: "unique()", subject: "Status update", content: "Deployment completed.", users: ["user_123"], draft: true)
```
