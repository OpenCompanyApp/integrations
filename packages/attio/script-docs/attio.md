# Attio CRM JavaScript API Reference
...
```js
var result = app.integrations.attio.api_get({
  path: "/v2/lists",
})
```
## Records And Objects
...
```js
var companies = app.integrations.attio.list_records({
  object_id: "companies",
  limit: 25,
  filters: { name: "Acme" },
})

var created = app.integrations.attio.create_record({
  object_id: "companies",
  data: {
    name: "Example Corp",
...
var updated = app.integrations.attio.update_record({
  object_id: "companies",
...
```js
var attrs = app.integrations.attio.list_attributes({
  target: "objects",
...
```js
var entries = app.integrations.attio.list_entries({
  list_id: "sales-prospects",
  limit: 50,
  filter: { name: "Ada Lovelace" },
})

var entry = app.integrations.attio.create_entry({
  list_id: "sales-prospects",
...
```js
var notes = app.integrations.attio.list_notes({
  parent_object: "people",
  parent_record_id: "891dcbfc-9141-415d-9b2a-2238a6cc012d",
})

var task = app.integrations.attio.create_task({
...
```js
var result = app.integrations.attio.accounts.sales.list_records({
  object_id: "companies",
})
```
## Safety Notes
