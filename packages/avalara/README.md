...
Official AvaTax REST API integration for OpenCompany and KosmoKrator agents.

This package is generated from Avalara's official Swagger / OpenAPI document at `https://rest.avatax.com/swagger/v2/swagger.json` for AvaTax API v2. It exposes AvaTax API operations across accounts, companies, transactions, certificates, customers, items, locations, nexus, reports, subscriptions, users, definitions, and utilities.

## Authentication

Avalara supports two authentication modes in this package:

- OAuth bearer token: set `access_token`.
- Basic authentication: set `account_id` and `license_key`.

Use `https://sandbox-rest.avatax.com` as `base_url` for sandbox credentials. A default `company_id` can be configured and is used automatically for path parameters named `companyId` or `companyid`.

## Tool Shape

Generated tools use snake_case parameter names, while requests are mapped back to Avalara's original names. For example `$top` becomes `top`, `$filter` becomes `filter`, and `companyId` becomes `company_id`. For complex write operations, pass the documented JSON request model as `body`.
...
Additional operations are available through the provider catalog; this table intentionally shows the first 100 sorted operations only.
