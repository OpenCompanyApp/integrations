# Baserow JavaScript API Reference
...
```js
var databases = app.integrations.baserow.list_databases({ page: 1, size: 100 })
var tables = app.integrations.baserow.list_database_tables({ database_id: 123 })
var fields = app.integrations.baserow.list_fields({ table_id: 456 })
```
...
```js
var rows = app.integrations.baserow.list_rows({
  table_id: 456,
  search: "Acme",
  order_by: "-id",
  filters: [
...
var row = app.integrations.baserow.get_row({
  table_id: 456,
  row_id: 1,
})
```
...
```js
var created = app.integrations.baserow.create_row({
  table_id: 456,
  data: {
    Name: "Acme",
    Status: "Active",
...
var updated = app.integrations.baserow.update_row({
  table_id: 456,
  row_id: 1,
  data: {
    Status: "Inactive",
...
Batch payloads use Baserow's `items` wrapper internally. You can pass a JSON array or JavaScript array of row objects.

```js
var result = app.integrations.baserow.batch_create({
  table_id: 456,
  records: [
    { Name: "Acme" },
    { Name: "Globex" }
...
```js
var field = app.integrations.baserow.create_field({
  table_id: 456,
  payload: {
    name: "Status",
    type: "single_select",
...
```js
var response = app.integrations.baserow.api_get({
