...
```ruby
products = app.integrations.bigcommerce.list_products(limit: 25, include: "variants,images", query: {"is_visible" => true})
created = app.integrations.bigcommerce.create_product(payload: {name: "Example Widget", type: "physical", price: 29.99, weight: 1, categories: [1]})
orders = app.integrations.bigcommerce.list_orders(status_id: 2, limit: 10)
customer = app.integrations.bigcommerce.get_customer(customer_id: 123)
```
## Coverage Notes
