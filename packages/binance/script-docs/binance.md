...
```ruby
time = app.integrations.binance.check_server_time()
ticker = app.integrations.binance.symbol_price_ticker(symbol: "BNBUSDT")
account = app.integrations.binance.account_information_user_data(recv_window: 5000)
```
Use fake API keys and public symbols in tests and examples. Never store real Binance API keys, API secrets, account IDs, order IDs, balances, wallet addresses, or signed query strings in fixtures or Ruby examples.
