...
```ruby
apps = app.integrations.bitrise.list_apps(query: {limit: 20})
build = app.integrations.bitrise.trigger_build(app_slug: "app-slug", payload: {hook_info: {type: "bitrise"}, build_params: {branch: "main", workflow_id: "primary"}})
log = app.integrations.bitrise.get_build_log(app_slug: "app-slug", build_slug: "build-slug")
secret = app.integrations.bitrise.put_secret(app_slug: "app-slug", secret_name: "DEPLOY_TOKEN", payload: {value: "dummy-token", is_protected: true, expand_in_step_inputs: true})
```
## Return Shapes
