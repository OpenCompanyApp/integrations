...
```ruby
app.integrations.box.list_items_folder(folder_id: "0", limit: 100, fields: "id,type,name,size,modified_at")
```
```ruby
app.integrations.box.get_file_information(file_id: "12345", fields: "id,type,name,size,shared_link")
```
```ruby
app.integrations.box.search_content(query: "quarterly report", type: "file", limit: 25)
```
...
```ruby
app.integrations.box.upload_file(body: {attributes: "{\"name\":\"notes.txt\",\"parent\":{\"id\":\"0\"}}", file: "/tmp/notes.txt"})
```
Additional documented query parameters can be passed exactly as named through `query`.

```ruby
app.integrations.box.list_user_and_enterprise_events(query: {stream_type: "admin_logs", limit: 100})
```
## Return Shape
...
```ruby
example = {body: "...", content_type: "application/octet-stream"}
```
