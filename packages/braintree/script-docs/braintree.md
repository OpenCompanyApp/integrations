...
```ruby
merchant = app.integrations.braintree.viewer(selection: "merchant { id status companyName } user { id email name }")
transactions = app.integrations.braintree.search_transactions(input: {}, first: 10, selection: "edges { node { id legacyId status amount { value currencyCode } } } pageInfo { hasNextPage endCursor }")
refund = app.integrations.braintree.refund_transaction(input: {transactionId: "gid://braintree/Transaction/example", amount: "10.00"}, selection: "refund { id legacyId status amount { value currencyCode } }")
```
## Common Tools
