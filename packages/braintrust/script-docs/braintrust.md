...
```js
var projects = braintrust_list_projects({
  query: { limit: 10 },
})
```
...
```js
var experiments = braintrust_list_experiments({
  query: {
...
```js
var result = braintrust_insert_dataset({
...
```js
var rows = braintrust_query_btql({
  body: {
...
```js
var output = braintrust_invoke_function({
...
```js
var completion = braintrust_proxy_chat_completions({
  body: {
