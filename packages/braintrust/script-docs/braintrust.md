...
```ruby
projects = app.integrations.braintrust.list_projects(query: {limit: 10})
```
Filter experiments by nested metadata:

```ruby
experiments = app.integrations.braintrust.list_experiments(query: {project_id: "00000000-0000-0000-0000-000000000000", metadata: {env: "test", model: {name: "gpt-5-mini"}}})
```
Insert dataset rows:

```ruby
result = app.integrations.braintrust.insert_dataset_rows(dataset_id: "00000000-0000-0000-0000-000000000000", body: {events: [{id: "row-1", input: {question: "What is 2+2?"}, expected: "4"}]})
```
Query BTQL:

```ruby
rows = app.integrations.braintrust.query_btql(body: {query: "SELECT id, input, output FROM project_logs('00000000-0000-0000-0000-000000000000') LIMIT 10", fmt: "json"})
```
Invoke a function:

```ruby
output = app.integrations.braintrust.invoke_function(function_id: "00000000-0000-0000-0000-000000000000", body: {input: {text: "Summarize this safely."}, environment: "production"})
```
Use the proxy for chat completions:

```ruby
completion = app.integrations.braintrust.proxy_chat_completions(body: {model: "openai/gpt-4o-mini", messages: [{role: "user", content: "Write a concise release note."}]})
```
## Coverage notes
