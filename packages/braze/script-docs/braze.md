...
```js
var result = app.integrations.braze.track_users({
  payload: {
    attributes: [
      {
...
```js
var result = app.integrations.braze.trigger_campaign_send({
  payload: {
...
```js
var result = app.integrations.braze.get_campaign_analytics({
  campaign_id: "campaign-id",
  length: 7,
...
```js
var result = app.integrations.braze.create_catalog_item({
