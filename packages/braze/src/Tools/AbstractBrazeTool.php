...
/**
 * Base class for focused Braze API endpoint tools.
 *
 * Handles configuration checks, required arguments, path interpolation,
 * query parameters, and JSON payload shaping.
 */
abstract class AbstractBrazeTool implements Tool
{
    ...
    /**
     * @param  BrazeService  $service  The Braze REST API client.
     */
    public function __construct(
        protected BrazeService $service,
    ) {}
    ...
    /**
     * Execute one Braze API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Braze integration is not configured. API key is required.');
            }
            ...
            $result = match ($this->method) {
                ...
                'DELETE' => $this->service->apiDelete($path, $query, $body),
                default => throw new RuntimeException("Unsupported Braze tool method: {$this->method}"),
            };
