...
```ruby
app.integrations.brevo.create_contact(email: "person@example.test", attributes: {FIRSTNAME: "Ada"}, list_ids: [12], update_enabled: true)
```
Send a transactional email:

```ruby
app.integrations.brevo.send_email(payload: {sender: {email: "noreply@example.test", name: "Example"}, to: [{email: "person@example.test", name: "Ada"}], subject: "Welcome", htmlContent: "<p>Hello Ada</p>"})
```
Create an email campaign:

```ruby
campaign = app.integrations.brevo.create_email_campaign(payload: {name: "Launch update", subject: "Launch update", sender: {id: 3}, recipients: {listIds: [12]}, htmlContent: "<p>News</p>"})
```
## Coverage Notes
