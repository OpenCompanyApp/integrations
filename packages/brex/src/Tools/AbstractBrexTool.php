...
/**
 * Shared executor for Brex endpoint-specific tools.
 *
 ...
 */
abstract class AbstractBrexTool implements Tool
{
    ...
    /** @param  BrexService  $service  Brex API client. */
    public function __construct(protected BrexService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Brex integration is not configured.');
            return ToolResult::success($this->service->request(static::METHOD, static::PATH, $this->mapped($args, static::PATH_PARAMS, true), $this->mapped($args, static::QUERY_PARAMS), $this->mapped($args, static::HEADER_PARAMS), $this->body($args)));
    ...
    {
        $body = $args['body'] ?? [];
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) throw new InvalidArgumentException('body must be a non-empty object matching the Brex API request schema.');
