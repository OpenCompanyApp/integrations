...
```ruby
projects = app.integrations.browserbase.projects_list()
session = app.integrations.browserbase.sessions_create(body: {projectId: "prj_example"})
search = app.integrations.browserbase.search_web(body: {query: "OpenCompany integrations", numResults: 5})
```
## Tools
...
All examples use fake IDs and safe placeholder values. Configure API keys through the host credential resolver, not in Ruby source.
