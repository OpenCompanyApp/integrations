...
/**
 * Shared executor for Browserbase endpoint-specific tools.
 *
 ...
 */
abstract class AbstractBrowserbaseTool implements Tool
{
    ...
    /**
     * @param  BrowserbaseService  $service  Browserbase API client.
     */
    public function __construct(protected BrowserbaseService $service) {}
    ...
    /**
     * Execute the mapped Browserbase API operation.
     * @param  array<string, mixed>  $args  Tool arguments for the mapped endpoint.
     */
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Browserbase integration is not configured.'); }
            return ToolResult::success($this->service->request(static::METHOD, static::PATH, $this->mapped($args, static::PATH_PARAMS, true), $this->mapped($args, static::QUERY_PARAMS), [], $this->body($args), static::BODY_CONTENT_TYPE));
    ...
    {
        if (static::BODY_CONTENT_TYPE === 'multipart') { $this->mapped($args, static::FORM_REQUIRED_PARAMS, true); return $this->mapped($args, static::FORM_PARAMS); }
        $body = $args['body'] ?? [];
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Browserbase API request schema.'); }
