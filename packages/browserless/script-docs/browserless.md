...
JSON request bodies are passed through `body`. Ruby function/download endpoints use a `code` string and send it as `application/javascript`. Wildcard routes expose `path_suffix` for the dynamic portion of the URL.
...
```ruby
version = app.integrations.browserless.get_json_version()
screenshot = app.integrations.browserless.post_screenshot(body: {url: "https://example.test"})
fn = app.integrations.browserless.post_chrome_function(code: "module.exports = async ({ page }) => await page.title();")
```
## Common Tools
...
All examples use fake URLs and safe placeholder values. Configure API tokens through the host credential resolver, not in Ruby source.
