...
```ruby
plan = app.integrations.browserstack.get_plan()
builds = app.integrations.browserstack.list_builds(query: {limit: 20, status: "running"})
session_logs = app.integrations.browserstack.get_session_logs(session_id: "session-id")
uploaded = app.integrations.browserstack.upload_app(payload: {url: "https://example.test/app-debug.apk", custom_id: "SampleApp"})
```
## Return Shapes
