...
```ruby
orgs = app.integrations.bugsnag.list_organizations()
projects = app.integrations.bugsnag.list_organization_projects(organization_id: "org_id")
```
...
```ruby
errors = app.integrations.bugsnag.list_errors(project_id: "project_id", query: {"filters[error.status][][value]" => "open", "filters[error.status][][type]" => "eq"})
```
...
```ruby
trend = app.integrations.bugsnag.get_project_trend(project_id: "project_id", resolution: "30m", query: {"filters[event.since][][value]" => "1d", "filters[event.since][][type]" => "eq"})
users = app.integrations.bugsnag.list_pivot_values(project_id: "project_id", error_id: "error_id", pivot: "user.id")
```
### Create Privacy Event Data Requests

```ruby
request = app.integrations.bugsnag.create_organization_event_data_request(organization_id: "org_id", query: {report_type: "gdpr", "filters[user.id][][value]" => "user_123", "filters[user.id][][type]" => "eq"})
```
...
```ruby
build = app.integrations.bugsnag.notify_build(payload: {apiKey: "project_api_key", appVersion: "1.2.3", releaseStage: "production"})
session = app.integrations.bugsnag.report_session(payload: {apiKey: "project_api_key", notifier: {name: "custom-agent", version: "1.0.0", url: "https://example.test"}, sessions: {}})
```
