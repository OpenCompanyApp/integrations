...
```ruby
result = app.integrations.calendly.list_event_types(active: true)
result.collection.each do |et|
  puts(et.name, et.duration, et.scheduling_url)
end
```
---
...
```ruby
et = app.integrations.calendly.get_event_type(uuid: "abc-123")
puts(et.resource.name, et.resource.duration)
```
---
...
```ruby
result = app.integrations.calendly.create_booking(host: "https://api.calendly.com/users/abc-123", start_time: "2024-06-15T10:00:00Z", end_time: "2024-06-15T11:00:00Z", name: "Quick Sync", location: {type: "zoom"})
puts(result.resource.scheduling_url)
```
---
...
```ruby
result = app.integrations.calendly.list_bookings(status: "active", min_start_time: "2024-01-01T00:00:00Z", max_start_time: "2024-12-31T23:59:59Z")
result.collection.each do |booking|
  puts(booking.name, booking.start_time, booking.end_time)
end
```
---
...
```ruby
result = app.integrations.calendly.list_organizations()
result.collection.each do |org|
  puts(org.name, org.slug)
end
```
---
...
```ruby
result = app.integrations.calendly.list_users(organization: "https://api.calendly.com/organizations/org-123")
result.collection.each do |member|
  puts(member.user.name, member.user.email, member.role)
end
```
---
...
```ruby
user = app.integrations.calendly.get_current_user()
puts(user.resource.name, user.resource.email)
```
---
