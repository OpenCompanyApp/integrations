...
```ruby
designs = app.integrations.canva.list_designs(limit: 10, query: "quarterly report")
exportResult = app.integrations.canva.create_design_export_job(payload: {design_id: "DAFexample", format: {type: "pdf"}})
```