...
/**
 * Shared executor for Cerebras endpoint-specific tools.
 *
 * Keeps configured-state checks, path validation, query/body shaping,
 * multipart uploads, and error conversion consistent across Cerebras tools.
 */
abstract class AbstractCerebrasTool implements Tool
{
    ...
    /**
     * @param  CerebrasService  $service  Cerebras API client.
     */
    public function __construct(
        protected CerebrasService $service,
    ) {}
    ...
    /**
     * Execute the mapped Cerebras API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Cerebras integration is not configured.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the Cerebras API request schema.');
        }
