...
```ruby
customers = app.integrations.chargeover.list_customers(limit: 25, offset: 0, where: "superuser_email:EQUALS:person@example.test", order: "customer_id:DESC")
```
```ruby
packages = app.integrations.chargeover.list_packages(customer_id: 123, expand: "line_items")
```
```ruby
transactions = app.integrations.chargeover.list_transactions(where: "applied_to.invoice_id:EQUALS:456", expand: "applied_to")
```