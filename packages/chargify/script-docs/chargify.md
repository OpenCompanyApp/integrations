...
```ruby
subscriptions = app.integrations.chargify.list_subscriptions(page: 1, per_page: 50, state: "active")
```
```ruby
invoice = app.integrations.chargify.get_invoice(invoice_id: "inv_123")
```
```ruby
site = app.integrations.chargify.get_site()
```
