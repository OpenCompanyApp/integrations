...
/**
 * Fetch detailed availability metrics and aggregated or non-aggregated TCP Check metrics across custom time ranges. For example, you can get the p99 and p95 of all the check phases of your TCP check together with the availability percentage for any time range. Rate-limiting is applied to this endpoint, you can send 30 requests / 60 seconds at most..
 *
 * Maps to the official Checkly endpoint GET /v1/analytics/tcp-checks/{id}.
 */
class ChecklyGetV1AnalyticsTcpchecksId extends AbstractChecklyTool
{
    protected const NAME = 'checkly_get_v1_analytics_tcpchecks_id';
    protected const DESCRIPTION = 'Fetch detailed availability metrics and aggregated or non-aggregated TCP Check metrics across custom time ranges. For example, you can get the p99 and p95 of all the check phases of your TCP check together with the availability percentage for any time range. Rate-limiting is applied to this endpoint, you can send 30 requests / 60 seconds at most.

Official Checkly endpoint: GET /v1/analytics/tcp-checks/{id}.';
    ...
    protected const PATH = '/v1/analytics/tcp-checks/{id}';
