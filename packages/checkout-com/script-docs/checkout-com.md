...
```ruby
methods = app.call("integrations.checkout-com.get_payment_methods")
payment = app.call("integrations.checkout-com.request_payment_or_payout", cko_idempotency_key: "example-payment-key-001", body: {source: {type: "token", token: "tok_example"}, amount: 1299, currency: "USD", reference: "example-order-001"})
token = app.call("integrations.checkout-com.request_access_token", body: {grant_type: "client_credentials", client_id: "ack_example", client_secret: "example-secret", scope: "gateway"})
```
Use fake IDs, tokens, customers, disputes, cardholders, and metadata in tests and prompts. Never place real card data, customer data, payment IDs, account prefixes, API keys, access tokens, or files in fixtures or Ruby examples.
