...
```ruby
pipeline = app.integrations.circleci.trigger_pipeline(project_slug: "gh/example-org/example-repo", branch: "main", parameters: {deploy: false})
```
Inspect the pipeline and workflow:

```ruby
workflows = app.integrations.circleci.list_pipeline_workflows(pipeline_id: pipeline.id)
jobs = app.integrations.circleci.list_workflow_jobs(workflow_id: workflows.items[0].id)
```
Manage a context secret:

```ruby
app.integrations.circleci.upsert_context_env_var(context_id: "context-uuid", env_var_name: "DEPLOY_TOKEN", value: "secret-value")
```
## Coverage Notes
