...
```ruby
person = app.integrations.clearbit.enrich_person(email: "person@example.test")
combined = app.integrations.clearbit.enrich_person_and_company(email: "person@example.test")
company = app.integrations.clearbit.enrich_company(domain: "example.test")
```
...
```ruby
reveal = app.integrations.clearbit.reveal_company(ip: "203.0.113.10")
```
...
```ruby
prospects = app.integrations.clearbit.prospect_people(domain: "example.test", roles: "sales,engineering", seniority: "executive", page: 1)
```
...
```ruby
suggestions = app.integrations.clearbit.company_autocomplete(name: "Example")
domain = app.integrations.clearbit.name_domain(name: "Example")
```
...
```ruby
companies = app.integrations.clearbit.discovery_search(params: {query: "name:example", limit: 10})
risk = app.integrations.clearbit.calculate_risk(params: {email: "person@example.test", ip: "203.0.113.10", name: "Example Person"})
```
...
```ruby
result = app.integrations.clearbit.api_get(api: "company", path: "/companies/find", params: {domain: "example.test"})
```
