# Client for the ClickSend REST API — JavaScript API Reference
...
```js
var result = app.integrations.clicksend.clicksend_get_account_balance({
})
```
...
```js
var result = app.integrations.clicksend.clicksend_get_email_history({
  limit: 0,
  page: 0,
})
```
...
```js
var result = app.integrations.clicksend.clicksend_get_sms_history({
  date_from: "",
  date_to: "",
  limit: 0,
})
...
```js
var result = app.integrations.clicksend.clicksend_get_sms_price({
  messages: {},
})
```
...
```js
var result = app.integrations.clicksend.clicksend_get_voice_history({
  limit: 0,
  page: 0,
})
```
...
```js
var result = app.integrations.clicksend.clicksend_list_contact_lists({
  limit: 0,
  page: 0,
})
```
...
```js
var result = app.integrations.clicksend.clicksend_send_email({
  to: "",
  subject: "",
  body: "",
})
...
```js
var result = app.integrations.clicksend.clicksend_send_post_letter({
  file_url: "",
  template_id: 0,
  recipients: {},
})
...
```js
var result = app.integrations.clicksend.clicksend_send_sms({
  messages: {},
})
```
...
```js
var result = app.integrations.clicksend.clicksend_send_voice({
  messages: {},
})
```
---
...
app.integrations.clicksend.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.clicksend.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.clicksend.work.function_name({ /* parameters */ })
app.integrations.clicksend.personal.function_name({ /* parameters */ })
```
