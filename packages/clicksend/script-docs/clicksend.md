# Client for the ClickSend REST API — Ruby API Reference
...
```ruby
result = app.integrations.clicksend.get_account_balance()
```
...
```ruby
result = app.integrations.clicksend.get_email_history(limit: 0, page: 0)
```
...
```ruby
result = app.integrations.clicksend.get_sms_history(date_from: "", date_to: "", limit: 0)
```
...
```ruby
result = app.integrations.clicksend.get_sms_price(messages: {})
```
...
```ruby
result = app.integrations.clicksend.get_voice_history(limit: 0, page: 0)
```
...
```ruby
result = app.integrations.clicksend.list_contact_lists(limit: 0, page: 0)
```
...
```ruby
result = app.integrations.clicksend.email(to: "", subject: "", body: "")
```
...
```ruby
result = app.integrations.clicksend.post_letter(file_url: "", template_id: 0, recipients: {})
```
## clicksend_send_sms
...
```ruby
result = app.integrations.clicksend.sms(messages: {})
```
...
```ruby
result = app.integrations.clicksend.voice(messages: {})
```
---
