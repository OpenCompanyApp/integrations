...
class ClickUpLogTime implements Tool
{
    ...
    public function parameters(): array
    {
        return [
            ...
            'start' => ['type' => 'string', 'required' => true, 'description' => 'Start time in ISO 8601 format.'],
            'duration' => ['type' => 'string', 'required' => true, 'description' => 'Duration in milliseconds.'],
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            if (empty($start)) {
                return ToolResult::error('start time is required (ISO 8601).');
            }
            if (empty($duration)) {
                return ToolResult::error('duration is required in milliseconds.');
            }
