...
```ruby
token = app.integrations.cloudflare.verify_token()
accounts = app.integrations.cloudflare.list_accounts(per_page: 20)
zones = app.integrations.cloudflare.list_zones(name: "example.com", per_page: 20)
```
...
```ruby
records = app.integrations.cloudflare.list_dns_records(zone_id: "zone_123", type: "A", name: "www.example.com")
created = app.integrations.cloudflare.create_dns_record(zone_id: "zone_123", type: "A", name: "www.example.com", content: "192.0.2.10", ttl: 1, proxied: true)
```
...
```ruby
ssl = app.integrations.cloudflare.get_zone_setting(zone_id: "zone_123", setting_id: "ssl")
purge = app.integrations.cloudflare.purge_cache(zone_id: "zone_123", files: ["https://www.example.com/app.css"])
```
