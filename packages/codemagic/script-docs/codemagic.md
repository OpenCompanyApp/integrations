...
```ruby
apps = app.integrations.codemagic.list_apps()
build = app.integrations.codemagic.start_build(payload: {appId: "app-id", workflowId: "release", branch: "main", labels: ["nightly"]})
public_artifact = app.integrations.codemagic.create_artifact_public_url(secure_filename: "secure/build/path/app.ipa", payload: {expiresAt: 1767225600})
```
## Return Shapes
