...
```ruby
quotes = app.integrations.coinmarketcap.cryptocurrency_quotes_latest(symbol: "BTC,ETH", convert: "USD")
listings = app.integrations.coinmarketcap.cryptocurrency_listings(start: 1, limit: 10, convert: "USD")
key = app.integrations.coinmarketcap.key_info()
```
Use fake API keys and public asset symbols in tests and examples. Never store real CoinMarketCap API keys, customer account metadata, or paid-plan response snapshots in fixtures or Ruby examples.
