...
```js
var quotes = app.integrations.coinmarketcap.get_v3_cryptocurrency_quotes_latest({
  symbol: "BTC,ETH",
  convert: "USD",
})

var listings = app.integrations.coinmarketcap.get_v3_cryptocurrency_listings_latest({
  start: 1,
  limit: 10,
  convert: "USD",
})

var key = app.integrations.coinmarketcap.get_v1_key_info({})
```
Use fake API keys and public asset symbols in tests and examples. Never store real CoinMarketCap API keys, customer account metadata, or paid-plan response snapshots in fixtures or JavaScript examples.
