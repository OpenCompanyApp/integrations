# Confluent Cloud - Ruby API Reference
...
```ruby
environments = app.integrations.confluent.list_environments(page_size: 10)
clusters = app.integrations.confluent.confluent_list_clusters(environment: "env-abc123")
topics = app.integrations.confluent.list_topics(cluster_id: "lkc-abc123")
topic = app.integrations.confluent.get_topic(cluster_id: "lkc-abc123", topic_name: "orders")
app.integrations.confluent.create_topic(cluster_id: "lkc-abc123", topic_name: "orders", partitions_count: 6)
```
## Generated Tool Shape

Tool names follow the upstream operation name, normalized to snake_case with a `confluent_` prefix in metadata and exposed without the prefix in Ruby. Existing common names such as `list_topics`, `get_topic`, `create_topic`, `list_clusters`, `get_cluster`, and `list_environments` are preserved where they map to official endpoints.
