...
/**
 * Confluent Cloud Kafka API service for communicating with the Confluent Cloud REST API v1.
 *
 * Handles authentication via Bearer token and provides methods for
 * topics, clusters, environments, and user information.
 */
class ConfluentService
{
    /**
     * Create a new ConfluentService instance.
     *
    ...
    {
        return $this->request('GET', '/orgs/{org_id}/environments');
    }
    ...
    private function rawRequest(string $method, string $path, array $data = []): \Illuminate\Http\Client\Response
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Confluent API error: {$method} {$path}", [
                    'status' => $response->status(),
                    'error' => $error,
                ]);

                throw new \RuntimeException("Confluent API error ({$response->status()}): {$error}");
            }

            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Confluent API connection error: {$method} {$path}", [
                'error' => $e->getMessage(),
            ]);
            throw new \RuntimeException("Failed to connect to Confluent API: {$e->getMessage()}");
        }
    }
}
