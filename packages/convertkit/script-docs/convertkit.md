# Kit (ConvertKit) Ruby API Reference
...
```ruby
result = app.integrations.convertkit.list_subscribers(email_address: "reader@example.test", per_page: 25)
```
Create or upsert a subscriber:

```ruby
result = app.integrations.convertkit.create_subscriber(email_address: "reader@example.test", first_name: "Ada", fields: {plan: "pro"})
```
Tag a subscriber by email address:

```ruby
result = app.integrations.convertkit.tag_subscriber_by_email(tag_id: 42, email_address: "reader@example.test")
```
Create a broadcast from a full V4 payload:

```ruby
result = app.integrations.convertkit.create_broadcast(payload: {subject: "Weekly update", preview_text: "What changed this week", description: "Weekly update", content: "<p>Hello readers</p>", public: false, published_at: "2026-01-01T00:00:00Z", subscriber_filter: {}})
```
## Resource Coverage
...
```ruby
result = app.integrations.convertkit.api_get(path: "/segments", params: {per_page: 50})
```
