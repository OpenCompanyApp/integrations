# Client for the Convex REST API — JavaScript API Reference
...
```js
var result = app.integrations.convex.convex_list_tables({
})
```
## convex_get_table
...
```js
var result = app.integrations.convex.convex_get_table({
  table: "users",
})
```
...
```js
var result = app.integrations.convex.convex_query_documents({
  table: "users",
...
```js
var result = app.integrations.convex.convex_create_document({
  table: "users",
...
```js
var result = app.integrations.convex.convex_update_document({
  table: "users",
...
```js
var result = app.integrations.convex.convex_delete_document({
  table: "users",
  document_id: "doc_abc123",
})
```
...
```js
var result = app.integrations.convex.convex_get_current_user({
})
```
---
...
app.integrations.convex.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.convex.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.convex.staging.function_name({ /* parameters */ })
app.integrations.convex.production.function_name({ /* parameters */ })
```
