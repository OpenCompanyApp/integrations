...
```ruby
sent = app.integrations.courier.send_message(payload: {message: {to: {user_id: "user_123"}, template: "template_id", data: {name: "Ada"}}})
profile = app.integrations.courier.create_profile(user_id: "user_123", payload: {profile: {email: "ada@example.test"}})
```