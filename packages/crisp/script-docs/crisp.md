# Crisp JavaScript API Reference
...
```js
var conversations = app.integrations.crisp.list_conversations({
  page_number: 1,
  query: { per_page: 50 },
})
```
...
```js
var result = app.integrations.crisp.send_message_in_conversation({
...
```js
var profile = app.integrations.crisp.get_people_profile({
...
```js
var articles = app.integrations.crisp.list_helpdesk_locale_articles({
  locale: "en",
  page_number: 1,
})
```
