...
```ruby
conversations = app.integrations.crisp.list_conversations(page_number: 1, query: {per_page: 50})
```
...
```ruby
result = app.integrations.crisp.send_message_conversation(session_id: "session_700c65e1-85e2-465a-b9ac-ecb5ec2c9881", payload: {type: "text", from: "operator", origin: "chat", content: "Hey there! Need help?"})
```
### Get a people profile

```ruby
profile = app.integrations.crisp.get_people_profile(people_id: "people_123")
```
### List helpdesk articles

```ruby
articles = app.integrations.crisp.list_helpdesk_locale_articles(locale: "en", page_number: 1)
```
## Notes
