...
```ruby
works = app.integrations.crossref.list_works(query: "large language models", rows: 10, filter: {type: "journal-article", "from-pub-date" => "2024-01-01"}, mailto: "agent@example.test")
work = app.integrations.crossref.get_work(doi: "10.1128/mbio.01735-25")
```
...
```ruby
journal = app.integrations.crossref.get_journal(issn: "0306-4530")
member = app.integrations.crossref.get_member(id: "98")
prefix = app.integrations.crossref.get_prefix(prefix: "10.5555")
article_type = app.integrations.crossref.get_type(id: "journal-article")
```
## Return Shape
...
Filters can be sent as a raw Crossref filter string or as a Ruby object; tables are converted to `filter:value` comma syntax.
