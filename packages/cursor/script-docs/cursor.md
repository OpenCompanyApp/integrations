...
```ruby
result = app.integrations.cursor.list_team_members()
(result.teamMembers || []).each do |member|
  puts((member.name).to_s + " (" + (member.email).to_s + ") - " + (member.role).to_s)
end
```
...
```ruby
usage = app.integrations.cursor.get_daily_usage_data(start_date: 1748411762359, end_date: 1751003762359)
```
## get_spend
...
```ruby
spend = app.integrations.cursor.get_spend(search_term: "developer@example.test", page: 1, page_size: 25)
```
## get_usage_events
...
```ruby
blocklists = app.integrations.cursor.list_repo_blocklists()
```
...
```ruby
result = app.integrations.cursor.upsert_repo_blocklists(repos: [{url: "https://github.com/example/sensitive-repo", patterns: ["*.env", "config/*", "secrets/**"]}])
```
