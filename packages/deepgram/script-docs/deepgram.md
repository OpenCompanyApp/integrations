...
```ruby
result = app.integrations.deepgram.transcribe_url(body: {url: "https://example.test/audio.wav"}, model: "nova-3", smart_format: true, diarize: true, utterances: true)
```
Use `deepgram_transcribe_audio` when the host has already read audio bytes:

```ruby
result = app.integrations.deepgram.transcribe_audio(content: audio_bytes, content_type: "audio/wav", model: "nova-3", smart_format: true)
```
...
```ruby
analysis = app.integrations.deepgram.analyze_text(body: {text: "Customer transcript goes here."}, summarize: true, sentiment: true, topics: true, intents: true)
```
## Text To Speech
...
```ruby
audio = app.integrations.deepgram.speak(body: {text: "Hello from Deepgram."}, model: "aura-2-thalia-en")
```
