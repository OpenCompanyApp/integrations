...
```ruby
translated = app.integrations.deepl.request_translation(body: {text: ["Hello world"], target_lang: "DE"})
usage = app.integrations.deepl.check_usage_and_limits()
languages = app.integrations.deepl.retrieve_supported_languages(type: "target")
```
