...
```ruby
app.integrations.delighted.send_person_survey(email: "customer@example.test", payload: {name: "Example Customer", properties: {plan: "pro"}})
```
```ruby
app.integrations.delighted.list_survey_responses(payload: {since: 1777833600, per_page: 100})
```
```ruby
app.integrations.delighted.delete_person(person_identifier: "email:customer@example.test")
```
