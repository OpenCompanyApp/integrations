...
```ruby
company = app.integrations.dialpad.company_get()
users = app.integrations.dialpad.user_list(state: "active", email: "user@example.test")
message = app.integrations.dialpad.sms_send(body: {text: "Hello from an agent", to_numbers: ["+14155550100"], user_id: "123456789"})
```
## Common Tools
