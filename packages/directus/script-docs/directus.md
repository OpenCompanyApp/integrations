...
```ruby
items = app.integrations.directus.list_items(collection: "articles", limit: 10, fields: ["id", "title", "status"])
article = app.integrations.directus.retrieve_item(collection: "articles", id: "example-id")
created = app.integrations.directus.create_item(collection: "articles", body: {title: "Example", status: "draft"})
```
...
Use fake collection names, ids, and instance URLs in examples and tests. Do not store real Directus tokens in Ruby programs or committed fixtures.