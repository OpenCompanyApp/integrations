# Docker Hub JavaScript API Reference
...
```js
var repos = docker.docker_list_repositories({
...
```
```js
var repo = docker.docker_get_repository({
  namespace: "example-org",
  repository: "api",
})
```
```js
var tags = docker.docker_list_tags({
  namespace: "example-org",
  repository: "api",
  page_size: 50,
})
```
```js
var created = docker.docker_create_repository({
  namespace: "example-org",
  body: {
    name: "agent-demo",
