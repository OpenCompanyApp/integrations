# HTTP client for the Dropbox API v2 — Ruby API Reference
...
```ruby
result = app.integrations.dropbox.copy(from_path: "", to_path: "", autorename: true)
```
...
```ruby
result = app.integrations.dropbox.create_folder(path: "", autorename: true)
```
...
```ruby
result = app.integrations.dropbox.create_shared_link(path: "", settings: "")
```
## dropbox_delete
...
```ruby
result = app.integrations.dropbox.delete(path: "")
```
...
```ruby
result = app.integrations.dropbox.download_file(path: "")
```
...
```ruby
result = app.integrations.dropbox.get_current_account()
```
...
```ruby
result = app.integrations.dropbox.get_metadata(path: "", include_media_info: true, include_deleted: true)
```
...
```ruby
result = app.integrations.dropbox.get_temporary_link(path: "")
```
## dropbox_list_folder
...
```ruby
result = app.integrations.dropbox.list_folder(path: "", recursive: true, limit: 0)
```
...
```ruby
result = app.integrations.dropbox.list_folder_continue(cursor: "")
```
...
```ruby
result = app.integrations.dropbox.list_revisions(path: "", limit: 0)
```
...
```ruby
result = app.integrations.dropbox.list_shared_links(path: "", cursor: "")
```
## dropbox_move
...
```ruby
result = app.integrations.dropbox.move(from_path: "", to_path: "", autorename: true)
```
## dropbox_restore
...
```ruby
result = app.integrations.dropbox.restore(path: "", rev: "")
```
...
```ruby
result = app.integrations.dropbox.search_continue(cursor: "")
```
...
```ruby
result = app.integrations.dropbox.search_files(query: "", path: "", file_categories: {})
```
## dropbox_upload_file
...
```ruby
result = app.integrations.dropbox.upload_file(path: "", content: "", mode: "")
```
---
