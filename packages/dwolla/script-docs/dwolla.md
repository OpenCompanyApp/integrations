...
```ruby
root = app.integrations.dwolla.get_root()
customers = app.integrations.dwolla.list_and_search_customers(limit: 25, search: "example")
token = app.integrations.dwolla.create_application_access_token(body: {grant_type: "client_credentials"})
```
Use fake customer IDs, funding source IDs, transfer IDs, document paths, webhook URLs, client IDs, and access tokens in tests and examples. Never store real bank, customer, routing, account, SSN, OAuth, or webhook-secret data in fixtures or Ruby examples.
