...
```ruby
shop = app.integrations.etsy.get_shop()
puts(shop.shop_id)
```
### list_shop_sections
...
```ruby
sections = app.integrations.etsy.list_shop_sections()
```
...
```ruby
profiles = app.integrations.etsy.list_shipping_profiles()
```
## Listings
...
```ruby
listings = app.integrations.etsy.list_listings(state: "active", limit: 25, offset: 0)
```
### get_listing
...
```ruby
listing = app.integrations.etsy.get_listing(listing_id: 1234567890)
```
### create_listing
...
```ruby
listing = app.integrations.etsy.create_listing(title: "Handmade Ceramic Mug", description: "A wheel-thrown mug with a blue glaze.", price: 28, quantity: 10, shipping_profile_id: 567890, taxonomy_id: 1234, who_made: "i_did", when_made: "made_to_order", is_supply: false, tags: ["ceramic", "mug"])
```
### update_listing
...
```ruby
updated = app.integrations.etsy.update_listing(listing_id: 1234567890, data: {title: "Updated Ceramic Mug", state: "active", section_id: 222222})
```
### delete_listing
...
```ruby
result = app.integrations.etsy.delete_listing(listing_id: 1234567890)
```
...
```ruby
images = app.integrations.etsy.list_listing_images(listing_id: 1234567890)
```
...
```ruby
image = app.integrations.etsy.upload_listing_image(listing_id: 1234567890, image_path: "/tmp/example-listing.jpg", fields: {rank: 1, alt_text: "Blue ceramic mug on a white table"})
```
...
```ruby
inventory = app.integrations.etsy.get_listing_inventory(listing_id: 1234567890)
```
...
```ruby
inventory = app.integrations.etsy.update_listing_inventory(listing_id: 1234567890, data: {products: [{sku: "MUG-BLUE", offerings: [{price: 28, quantity: 10, is_enabled: true}], property_values: {}}], price_on_property: {}, quantity_on_property: {}, sku_on_property: {}})
```
## Orders
...
```ruby
orders = app.integrations.etsy.list_orders(was_paid: true, was_shipped: false, limit: 10)
```
### get_receipt
...
```ruby
receipt = app.integrations.etsy.get_receipt(receipt_id: 987654321)
```
...
```ruby
items = app.integrations.etsy.list_receipt_transactions(receipt_id: 987654321)
```
## Taxonomy And User
...
```ruby
taxonomy = app.integrations.etsy.list_seller_taxonomy_nodes()
```
### get_current_user
...
```ruby
user = app.integrations.etsy.get_current_user()
```
## Generic API Helpers
...
```ruby
result = app.integrations.etsy.api_get(path: "/seller-taxonomy/nodes", params: {})
created = app.integrations.etsy.api_post(path: "/shops/12345678/listings", body: {title: "Draft Listing", description: "Safe example", price: 12, quantity: 1, shipping_profile_id: 111111})
updated = app.integrations.etsy.api_put(path: "/listings/1234567890/inventory", body: {products: {}})
deleted = app.integrations.etsy.api_delete(path: "/shops/12345678/listings/1234567890")
```
## Multi-Account Usage
