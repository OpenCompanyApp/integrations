# Fathom Analytics - Ruby API Reference
...
```ruby
account = app.integrations.fathom.get_account()
puts((account.name).to_s + " <" + (account.email).to_s + ">")
```
### get_current_user
...
```ruby
result = app.integrations.fathom.list_sites(limit: 10)
(result.data || []).each do |site|
  puts((site.id).to_s + ": " + (site.name).to_s)
end
```
### get_site
...
```ruby
site = app.integrations.fathom.create_site(name: "Example Site", sharing: "none")
```
...
```ruby
event = app.integrations.fathom.create_event(site_id: "ABCDEF", name: "Signup")
```
## Milestones
...
```ruby
milestone = app.integrations.fathom.create_milestone(site_id: "ABCDEF", name: "Launch day", milestone_date: "2026-01-15")
```
## Reports
...
```ruby
top_pages = app.integrations.fathom.get_aggregation(site_id: "ABCDEF", date_from: "2026-01-01", date_to: "2026-01-31", metrics: "pageviews,visits,uniques", group_by: "pathname", sort_by: "pageviews:desc", limit: 10)
```
...
```ruby
current = app.integrations.fathom.current_visitors(site_id: "ABCDEF", detailed: true)
```
## Multi-Account Usage
