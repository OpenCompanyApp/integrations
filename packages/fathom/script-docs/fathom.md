# Fathom Analytics - JavaScript API Reference
...
```js
var account = app.integrations.fathom.get_account({})
...
```js
var result = app.integrations.fathom.list_sites({limit: 10})
...
```js
var site = app.integrations.fathom.create_site({
  name: "Example Site",
  sharing: "none",
})
```
...
```js
var event = app.integrations.fathom.create_event({
  site_id: "ABCDEF",
  name: "Signup",
})
```
...
```js
var milestone = app.integrations.fathom.create_milestone({
  site_id: "ABCDEF",
  name: "Launch day",
...
```js
var top_pages = app.integrations.fathom.get_aggregate({
  site_id: "ABCDEF",
...
```js
var current = app.integrations.fathom.get_current_visitors({
  site_id: "ABCDEF",
  detailed: true,
})
```
