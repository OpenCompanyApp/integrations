...
/**
 * Shared executor for Feedbin operation tools.
 *
 * Concrete tools declare the operation key while this class exposes metadata,
 * merges payload fields, and delegates to FeedbinService.
 */
abstract class AbstractFeedbinTool implements Tool
{
    ...
    public function name(): string { return 'feedbin_'.static::OPERATION; }
    ...
    {
        $definition = FeedbinService::operations()[static::OPERATION] ?? null;
        if ($definition === null) {
            throw new \RuntimeException('Unknown Feedbin operation: '.static::OPERATION);
        }
