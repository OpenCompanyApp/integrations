# Fellow JavaScript API Reference
...
```js
var me = app.integrations.fellow.get_current_user({})
```
...
```js
var notes = app.integrations.fellow.list_notes({
  pagination: { page_size: 25 },
  include: { transcript: true },
  filters: { updated_at_start: "2026-05-01" },
})

var note = app.integrations.fellow.get_note({
  note_id: "note_123",
})
```
...
```js
var items = app.integrations.fellow.list_action_items({
  order_by: "due_date",
  filters: { scope: "assigned_to_me" },
})

var item = app.integrations.fellow.get_action_item({
...
```js
var recordings = app.integrations.fellow.list_recordings({
  pagination: { page_size: 10 },
  media_url: { expires_in: 3600 },
})

var recording = app.integrations.fellow.get_recording({
...
```js
var hooks = app.integrations.fellow.list_webhooks({
  page_size: 20,
})

var hook = app.integrations.fellow.create_webhook({
...
```js
var data = app.integrations.fellow.api_get({
  path: "/me",
})
