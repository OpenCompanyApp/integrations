...
- Streaming responses are returned as normal HTTP response bodies. Hosts that
  ...
  transport outside the JavaScript bridge.
...
```js
var response = fireworks_ai_chat_completions({
  body: {
...
```js
var embeddings = fireworks_ai_create_embeddings({
  body: {
...
```js
var models = fireworks_ai_list_models({
...
```js
var dataset = fireworks_ai_create_dataset({
  account_id: "example-account",
  body: {
