...
- Streaming responses are returned as normal HTTP response bodies. Hosts that
  ...
  transport outside the Ruby bridge.
...
```ruby
response = app.call("integrations.fireworks-ai.chat_completions", body: {model: "accounts/fireworks/models/deepseek-v3p1", messages: [{role: "user", content: "Write a concise changelog entry."}]})
```
Create embeddings:

```ruby
embeddings = app.call("integrations.fireworks-ai.create_embeddings", body: {model: "nomic-ai/nomic-embed-text-v1.5", input: ["hello", "world"]})
```
List account models:

```ruby
models = app.call("integrations.fireworks-ai.list_models", account_id: "example-account", query: {page_size: 20})
```
Create a dataset:

```ruby
dataset = app.call("integrations.fireworks-ai.create_dataset", account_id: "example-account", body: {datasetId: "example-dataset", dataset: {displayName: "Example dataset"}})
```
## Coverage notes
