...
/**
 * Shared executor for Fireworks AI endpoint-specific tools.
 *
 * Keeps authentication checks, path parameter validation, query/body shaping,
 * and error conversion consistent while each child maps to one API operation.
 */
abstract class AbstractFireworksAiTool implements Tool
{
    ...
    /**
     * @param  FireworksAiService  $service  Fireworks AI API client.
     */
    public function __construct(
        protected FireworksAiService $service,
    ) {}
    ...
    /**
     * Execute the mapped Fireworks AI API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Fireworks AI integration is not configured.');
            }
    ...
    /**
     * Extract the official request body from arguments.
     *
     * @param  array<string, mixed>  $args  Tool arguments.
     * @return array<string, mixed>
     */
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the Fireworks AI API request schema.');
        }
