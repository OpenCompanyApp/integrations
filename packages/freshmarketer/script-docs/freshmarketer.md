# Freshmarketer — Ruby API Reference
...
result = app.integrations.freshmarketer.list_campaigns(page: 1, limit: 20)
# Filter active campaigns only
result = app.integrations.freshmarketer.list_campaigns(status: "active")
```
---
...
```ruby
result = app.integrations.freshmarketer.get_campaign(id: 123)
```
---
...
result = app.integrations.freshmarketer.create_campaign(name: "Welcome Series", channel_list: ["email"], schedule: {type: "immediate"})
# Create a scheduled campaign
result = app.integrations.freshmarketer.create_campaign(name: "Monthly Newsletter", channel_list: ["email"], schedule: {type: "scheduled", scheduled_at: "2025-06-01T09:00:00Z"})
```
---
...
```ruby
result = app.integrations.freshmarketer.list_segments(page: 1, limit: 20)
```
---
...
```ruby
result = app.integrations.freshmarketer.get_segment(id: 456)
```
---
...
```ruby
result = app.integrations.freshmarketer.list_users()
```
---
...
```ruby
result = app.integrations.freshmarketer.get_current_user()
user = result.user
