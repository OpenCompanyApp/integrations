...
```ruby
users = app.integrations.fusionauth.retrieve_user(email: "agent-test@example.test", tenant_id: "tenant-example")
```
```ruby
created = app.integrations.fusionauth.create_user(tenant_id: "tenant-example", body: {user: {email: "agent-test@example.test", username: "agent-test", password: "example-password"}})
```