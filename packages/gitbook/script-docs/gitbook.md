...
```ruby
orgs = app.integrations.gitbook.list_organizations()
spaces = app.integrations.gitbook.list_spaces(organization_id: "org_abc", limit: 20)
results = app.integrations.gitbook.search_space(space_id: "space_abc", query: "authentication", limit: 10)
page = app.integrations.gitbook.get_page_by_path(space_id: "space_abc", page_path: "developers/api", format: "markdown")
```