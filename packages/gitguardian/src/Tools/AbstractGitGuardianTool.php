...
/**
 * Shared executor for GitGuardian endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGitGuardianTool implements Tool
{
    ...
    /**
     * @param  GitGuardianService  $service  GitGuardian API client.
     */
    public function __construct(protected GitGuardianService $service) {}
    ...
    /**
     * Execute the mapped GitGuardian API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('GitGuardian integration is not configured.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the GitGuardian API request schema.');
        }
