...
/**
 * HTTP client for the Google Admin Reports REST API.
 *
 ...
 */
class GoogleAdminReportsService
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Admin Reports scopes.
     * @param  string  $baseUrl  Google Admin Reports REST API base URL.
     */
    ...
    /**
     * Execute a Google Admin Reports REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Admin Reports.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Admin Reports access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Admin Reports API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Admin Reports API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Admin Reports API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Admin Reports API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{userKey}` and `{date}`.
     *
