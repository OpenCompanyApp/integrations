...
/**
 * Shared executor for Google Admin Reports endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleAdminReportsTool implements Tool
{
    ...
    /**
     * @param  GoogleAdminReportsService  $service  Google Admin Reports API client.
     */
    public function __construct(protected GoogleAdminReportsService $service) {}
    ...
    /**
     * Execute the mapped Google Admin Reports REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Admin Reports integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Admin Reports API request schema.');
