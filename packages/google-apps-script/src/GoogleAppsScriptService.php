...
/**
 * HTTP client for the Google Apps Script REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Apps Script scopes.
     * @param  string  $baseUrl  Google Apps Script REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://script.googleapis.com')
    {
    ...
    /**
     * Execute a Google Apps Script REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Apps Script.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Apps Script access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Apps Script API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Apps Script API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Apps Script API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Apps Script API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{scriptId}` and `{deploymentId}`.
     *
