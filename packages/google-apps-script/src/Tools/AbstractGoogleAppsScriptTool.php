...
/**
 * Shared executor for Google Apps Script endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleAppsScriptTool implements Tool
{
    ...
    /**
     * @param  GoogleAppsScriptService  $service  Google Apps Script API client.
     */
    public function __construct(protected GoogleAppsScriptService $service) {}
    ...
    /**
     * Execute the mapped Google Apps Script REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Apps Script integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Apps Script API request schema.');
