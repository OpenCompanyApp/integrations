...
/**
 * HTTP client for the Google BigQuery REST API.
 *
 * Handles OAuth bearer authentication, Discovery-style path expansion, JSON
 * request dispatch, response parsing, and Google API error normalization.
 */
class GoogleBigQueryService
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with BigQuery scopes.
     * @param  string  $baseUrl  BigQuery REST API base URL.
     */
    public function __construct(
        private string $accessToken = '',
        private string $baseUrl = 'https://bigquery.googleapis.com/bigquery/v2',
    ) {
    ...
    /**
     * Execute a Google BigQuery REST method.
     *
     * @param  array<string, mixed>  $pathParams  Path parameter values keyed by Discovery parameter name.
     * @param  string[]  $reservedPathParams  Path parameters that use `{+param}` reserved expansion.
    ...
    /**
     * Perform a raw HTTP request against BigQuery.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    private function rawRequest(string $method, string $path, array $query = [], array $body = []): Response
    {
        if (!$this->isConfigured()) {
            throw new RuntimeException('Google BigQuery access token is not configured.');
        }
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google BigQuery API error: {$method} {$path}", [
                    'status' => $response->status(),
                    'error' => $error,
                ]);

                throw new RuntimeException("Google BigQuery API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }

            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google BigQuery API connection error: {$method} {$path}", ['error' => $e->getMessage()]);

            throw new RuntimeException("Failed to connect to Google BigQuery API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{+projectId}` and `{datasetId}`.
     *
