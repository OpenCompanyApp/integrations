...
/**
 * Shared executor for Google BigQuery endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleBigQueryTool implements Tool
{
    ...
    /**
     * @param  GoogleBigQueryService  $service  BigQuery API client.
     */
    public function __construct(
        protected GoogleBigQueryService $service,
    ) {}
    ...
    /**
     * Execute the mapped BigQuery REST method.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Google BigQuery integration is not configured.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the BigQuery API request schema.');
        }
