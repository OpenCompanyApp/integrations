...
/**
 * Shared executor for Google Calendar endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleCalendarTool implements Tool
{
    ...
    /**
     * @param  GoogleCalendarService  $service  Google Calendar API client.
     */
    ...
    /**
     * Execute the mapped Google Calendar REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Calendar integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Calendar API request schema.');
