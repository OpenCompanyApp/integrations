...
/**
 * Shared executor for Google Chat endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleChatTool implements Tool
{
    ...
    /**
     * @param  GoogleChatService  $service  Google Chat API client.
     */
    public function __construct(protected GoogleChatService $service) {}
    ...
    /**
     * Execute the mapped Google Chat REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Chat integration is not configured.');
            if (static::MEDIA_UPLOAD) {
                return ToolResult::success($this->service->upload(static::MEDIA_UPLOAD_PATH, $this->pathParams($args), static::RESERVED_PATH_PARAMS, $this->query($args), $this->body($args), $this->requireScalar($args, 'file_path'), (string) ($args['mime_type'] ?? 'application/octet-stream')));
            }
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Chat API request schema.');
