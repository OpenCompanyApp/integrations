...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Classroom scopes.
     * @param  string  $baseUrl  Google Classroom REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://classroom.googleapis.com')
    {
    ...
    /**
     * Execute a Google Classroom REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Classroom.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Classroom access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Classroom API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Classroom API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Classroom API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Classroom API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{courseId}` and `{studentId}`.
     *
