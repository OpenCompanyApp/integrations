...
/**
 * Shared executor for Google Classroom endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleClassroomTool implements Tool
{
    ...
    /**
     * @param  GoogleClassroomService  $service  Google Classroom API client.
     */
    public function __construct(protected GoogleClassroomService $service) {}
    ...
    /**
     * Execute the mapped Google Classroom REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Classroom integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Classroom API request schema.');
