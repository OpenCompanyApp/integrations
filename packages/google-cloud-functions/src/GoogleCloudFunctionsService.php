...
/**
 * HTTP client for the Google Cloud Functions REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Cloud Functions scopes.
     * @param  string  $baseUrl  Cloud Functions REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://cloudfunctions.googleapis.com')
    {
    ...
    /**
     * Execute a Google Cloud Functions REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Cloud Functions.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Cloud Functions access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Cloud Functions API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Cloud Functions API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Cloud Functions API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Cloud Functions API: {$e->getMessage()}");
        }
    }
