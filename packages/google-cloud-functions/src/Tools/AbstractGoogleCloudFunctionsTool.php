...
/**
 * Shared executor for Google Cloud Functions endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleCloudFunctionsTool implements Tool
{
    ...
    /**
     * @param  GoogleCloudFunctionsService  $service  Cloud Functions API client.
     */
    public function __construct(protected GoogleCloudFunctionsService $service) {}
    ...
    /**
     * Execute the mapped Cloud Functions REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Cloud Functions integration is not configured.');
     ...
     */
    private function pathParams(array $args): array { $params=[]; foreach(static::PATH_PARAMS as $key)$params[$key]=$this->requireString($args,$key); return $params; }
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Cloud Functions API request schema.');
     ...
     * @param  array<string, mixed>  $args  Tool arguments.
     */
    private function requireString(array $args,string $key): string { $value=$args[$key]??null; if(is_int($value))return(string)$value; if(!is_string($value)||trim($value)==='') throw new InvalidArgumentException($key.' must be a non-empty string.'); return $value; }
}