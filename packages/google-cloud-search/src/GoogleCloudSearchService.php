...
/**
 * HTTP client for the Google Cloud Search REST API.
 *
 ...
 */
class GoogleCloudSearchService
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Cloud Search scopes.
     * @param  string  $baseUrl  Google Cloud Search REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://cloudsearch.googleapis.com')
    {
    ...
    /**
     * Execute a Google Cloud Search REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Cloud Search.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Cloud Search access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Cloud Search API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Cloud Search API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Cloud Search API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Cloud Search API: {$e->getMessage()}");
        }
    }
