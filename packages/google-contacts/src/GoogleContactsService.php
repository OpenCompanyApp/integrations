...
/**
 * HTTP client for the Google Contacts REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with People API scopes.
     * @param  string  $baseUrl  Google Contacts REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://people.googleapis.com')
    {
    ...
    /**
     * Execute a Google Contacts REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Contacts.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Contacts access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Contacts API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException('Google Contacts API error ('.$response->status().'): '.(is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Contacts API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
