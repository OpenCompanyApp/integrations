...
/**
 * Shared executor for Google Contacts endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleContactsTool implements Tool
{
    ...
    /**
     * @param  GoogleContactsService  $service  Google Contacts API client.
     */
    public function __construct(protected GoogleContactsService $service) {}
    ...
    /**
     * Execute the mapped Google Contacts REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Contacts integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Contacts API request schema.');
