...
/**
 * HTTP client for the Google Docs REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Docs scopes.
     * @param  string  $baseUrl  Google Docs REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://docs.googleapis.com')
    {
    ...
    /**
     * Execute a Google Docs REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Docs.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Docs access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Docs API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException('Google Docs API error ('.$response->status().'): '.(is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Docs API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
