...
/**
 * Shared executor for Google Docs endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleDocsTool implements Tool
{
    ...
    /**
     * @param  GoogleDocsService  $service  Google Docs API client.
     */
    public function __construct(protected GoogleDocsService $service) {}
    ...
    /**
     * Execute the mapped Google Docs REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Docs integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Docs API request schema.');
