...
/**
 * HTTP client for the Google Forms REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Forms scopes.
     * @param  string  $baseUrl  Google Forms REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://forms.googleapis.com')
    {
    ...
    /**
     * Execute a Google Forms REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Forms.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Forms access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Forms API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException('Google Forms API error ('.$response->status().'): '.(is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Forms API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
