...
/**
 * Shared executor for Google Forms endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleFormsTool implements Tool
{
    ...
    /**
     * @param  GoogleFormsService  $service  Google Forms API client.
     */
    public function __construct(protected GoogleFormsService $service) {}
    ...
    /**
     * Execute the mapped Google Forms REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Forms integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Forms API request schema.');
