...
```ruby
response = app.call("integrations.google-gemini.models_generate_content", model: "models/gemini-2.5-pro", body: {contents: [{parts: [{text: "Write a concise summary"}]}]})
models = app.call("integrations.google-gemini.models_list", page_size: 10)
```
