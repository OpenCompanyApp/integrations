...
/**
 * Shared executor for Gemini endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGeminiTool implements Tool
{
    ...
    /**
     * @param  GeminiService  $service  Google Gemini API client.
     */
    ...
    /**
     * Execute the mapped Gemini REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Gemini integration is not configured.');
    ...
    /** @param  array<string, mixed>  $args  Tool arguments. @return array<string, mixed> */
    private function body(array $args): array { $body=$args['body']??[]; if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Gemini API request schema.'); if($body!==[]&&!is_array($body)) throw new InvalidArgumentException('body must be an object.'); return $body; }
