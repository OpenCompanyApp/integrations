...
```ruby
notes = app.call("integrations.google-keep.notes_list", page_size: 10, filter: "trashed = false")
note = app.call("integrations.google-keep.notes_create", body: {title: "Agent checklist", body: {text: {text: "Review queued tasks"}}})
shared = app.call("integrations.google-keep.notes_permissions_batch_create", parent: "notes/example", body: {requests: [{permission: {role: "WRITER", email: "person@example.test"}}]})
```
