...
/**
 * HTTP client for the Google Keep REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Keep scopes.
     * @param  string  $baseUrl  Google Keep REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://keep.googleapis.com')
    {
    ...
    /**
     * Execute a Google Keep REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Keep.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Keep access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Keep API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Keep API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Keep API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Keep API: {$e->getMessage()}");
        }
    }
