...
/**
 * Shared executor for Google Keep endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleKeepTool implements Tool
{
    ...
    /**
     * @param  GoogleKeepService  $service  Google Keep API client.
     */
    public function __construct(protected GoogleKeepService $service) {}
    ...
    /**
     * Execute the mapped Google Keep REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Keep integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Keep API request schema.');
