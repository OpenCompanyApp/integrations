...
/**
 * Shared executor for Google Meet endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleMeetTool implements Tool
{
    ...
    /**
     * @param  GoogleMeetService  $service  Google Meet API client.
     */
    ...
    /**
     * Execute the mapped Google Meet REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Meet integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Meet API request schema.');
