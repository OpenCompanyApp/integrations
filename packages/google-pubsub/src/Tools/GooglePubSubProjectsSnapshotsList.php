 ...
 *
 * Maps to the official Pub/Sub endpoint GET /v1/{+project}/snapshots.
 */
class GooglePubSubProjectsSnapshotsList extends AbstractGooglePubSubTool
{
    protected const NAME = 'google_pubsub_projects_snapshots_list';
    protected const DESCRIPTION = 'Projects Snapshots List

Official Pub/Sub endpoint: GET /v1/{+project}/snapshots
Lists the existing snapshots. Snapshots are used in [Seek]( https://cloud.google.com/pubsub/docs/replay-overview) operations, which allow you to manage message acknowledgments in bulk. That is, you can set the acknowledgment state of messages in an existing subscription to the state captured by a snapshot.';
  ...
  array (
    ...
    protected const PATH = '/v1/{+project}/snapshots';
