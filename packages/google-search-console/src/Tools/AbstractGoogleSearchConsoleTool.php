...
/**
 * Shared executor for Google Search Console endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleSearchConsoleTool implements Tool
{
    ...
    /**
     * @param  GoogleSearchConsoleService  $service  Google Search Console API client.
     */
    public function __construct(protected GoogleSearchConsoleService $service) {}
    ...
    /**
     * Execute the mapped Google Search Console REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Search Console integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Search Console API request schema.');
