...
/**
 * Shared executor for Google Slides endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleSlidesTool implements Tool
{
    ...
    /**
     * @param  GoogleSlidesService  $service  Google Slides API client.
     */
    public function __construct(protected GoogleSlidesService $service) {}
    ...
    /**
     * Execute the mapped Google Slides REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Slides integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Slides API request schema.');
