...
/**
 * HTTP client for the Google Translate REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Cloud Translation API scopes.
     * @param  string  $baseUrl  Google Translate REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://translate.googleapis.com')
    {
    ...
    /**
     * Execute a Google Translate REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Google Translate.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Translate access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Translate API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException('Google Translate API error ('.$response->status().'): '.(is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Translate API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
