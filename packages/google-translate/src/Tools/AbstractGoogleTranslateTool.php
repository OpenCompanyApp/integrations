...
/**
 * Shared executor for Google Translate endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleTranslateTool implements Tool
{
    ...
    /**
     * @param  GoogleTranslateService  $service  Google Translate API client.
     */
    public function __construct(protected GoogleTranslateService $service) {}
    ...
    /**
     * Execute the mapped Google Translate REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Translate integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Translate API request schema.');
