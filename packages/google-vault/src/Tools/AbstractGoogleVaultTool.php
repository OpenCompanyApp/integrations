...
/**
 * Shared executor for Google Vault endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleVaultTool implements Tool
{
    ...
    /**
     * @param  GoogleVaultService  $service  Google Vault API client.
     */
    public function __construct(protected GoogleVaultService $service) {}
    ...
    /**
     * Execute the mapped Google Vault REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Vault integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Google Vault API request schema.');
