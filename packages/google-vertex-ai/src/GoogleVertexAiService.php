...
/**
 * HTTP client for the Google Vertex AI REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Vertex AI scopes.
     * @param  string  $baseUrl  Vertex AI REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://aiplatform.googleapis.com')
    {
    ...
    /**
     * Execute a Google Vertex AI REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Vertex AI.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Vertex AI access token is not configured.');
        try {
            $http = Http::withHeaders(['Authorization' => 'Bearer '.$this->accessToken, 'Content-Type' => 'application/json', 'Accept' => 'application/json'])->timeout(180);
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Vertex AI API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Vertex AI API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Vertex AI API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Vertex AI API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{+name}`, `{+parent}`, and `{+endpoint}`.
     *
