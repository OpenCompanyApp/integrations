...
/**
 * Shared executor for Google Vertex AI endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleVertexAiTool implements Tool
{
    ...
    /**
     * @param  GoogleVertexAiService  $service  Vertex AI API client.
     */
    public function __construct(protected GoogleVertexAiService $service) {}
    ...
    /**
     * Execute the mapped Vertex AI REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Vertex AI integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Vertex AI API request schema.');
