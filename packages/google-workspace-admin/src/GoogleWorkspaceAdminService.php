...
/**
 * HTTP client for the Google Workspace Admin Directory REST API.
 *
...
{
    /**
     * @param  string  $accessToken  Google OAuth 2.0 access token with Admin SDK Directory scopes.
     * @param  string  $baseUrl  Workspace Admin REST API base URL.
     */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://admin.googleapis.com')
    {
    ...
    /**
     * Execute a Google Workspace Admin Directory REST method.
     *
    ...
    /**
     * Perform a raw HTTP request against Workspace Admin.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON request body.
     */
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Google Workspace Admin access token is not configured.');
        try {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Google Workspace Admin API error: {$method} {$path}", ['status' => $response->status(), 'error' => $error]);
                throw new RuntimeException("Google Workspace Admin API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }
            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Google Workspace Admin API connection error: {$method} {$path}", ['error' => $e->getMessage()]);
            throw new RuntimeException("Failed to connect to Google Workspace Admin API: {$e->getMessage()}");
        }
    }

    /**
     * Expand Discovery path templates such as `{userKey}` and `{customer}`.
     *
