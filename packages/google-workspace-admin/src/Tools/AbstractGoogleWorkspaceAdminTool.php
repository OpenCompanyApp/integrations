...
/**
 * Shared executor for Google Workspace Admin endpoint-specific tools.
 *
 ...
 */
abstract class AbstractGoogleWorkspaceAdminTool implements Tool
{
    ...
    /**
     * @param  GoogleWorkspaceAdminService  $service  Workspace Admin API client.
     */
    public function __construct(protected GoogleWorkspaceAdminService $service) {}
    ...
    /**
     * Execute the mapped Workspace Admin REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('Google Workspace Admin integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Workspace Admin API request schema.');
