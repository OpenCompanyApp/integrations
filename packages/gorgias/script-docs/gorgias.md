...
```ruby
# List open tickets
result = app.integrations.gorgias.list_tickets(status: "open", limit: 10)
# Search tickets
result = app.integrations.gorgias.list_tickets(q: "billing", limit: 5)
# Paginate through all tickets
result = app.integrations.gorgias.list_tickets(page: 2, limit: 50)
```
---
...
```ruby
result = app.integrations.gorgias.get_ticket(id: "12345")
puts(result.subject)
puts(result.status)
```
---
...
```ruby
result = app.integrations.gorgias.create_ticket(subject: "Order !received", body: "<p>Customer reports order #5678 was never delivered.</p>", from_email: "customer@example.com", to_email: "support@myshop.com", channel: "email", priority: "high")
...
```ruby
# Search customers
result = app.integrations.gorgias.list_customers(q: "john", limit: 10)
# List all customers (paginated)
result = app.integrations.gorgias.list_customers(page: 1, limit: 100)
```
---
...
```ruby
result = app.integrations.gorgias.get_customer(id: "67890")
puts(result.name)
puts(result.email)
```
---
...
result = app.integrations.gorgias.list_satisfaction_surveys(limit: 20)
# Surveys for a specific ticket
result = app.integrations.gorgias.list_satisfaction_surveys(ticket_id: "12345")
```
---
...
```ruby
result = app.integrations.gorgias.get_current_user()
