...
```ruby
sent = app.integrations.gotify.create_message(title: "Deploy Complete", message: "Version 2.1.0 deployed successfully.", priority: 5)
puts(sent.id)
```
Use `extras` for Gotify client display hints such as markdown rendering:

```ruby
app.integrations.gotify.create_message(title: "Weekly Report", message: "## Summary\n\n- Pageviews: 12450", extras: {"client::display" => {contentType: "text/markdown"}})
```
### list_messages
...
```ruby
result = app.integrations.gotify.list_messages(limit: 25)
(result.messages || []).each do |msg|
  puts("[" + (msg.id).to_s + "] " + (msg.title).to_s)
end
```
### delete_message
...
```ruby
result = app.integrations.gotify.list_application_messages(application_id: 7, limit: 50)
```
...
```ruby
app.integrations.gotify.delete_application_messages(application_id: 7)
```
## Applications
...
```ruby
apps = app.integrations.gotify.list_applications()
```
### create_application
...
```ruby
app = app.integrations.gotify.create_application(name: "CI", description: "Build notifications")
puts(app.token)
```
### update_application
...
```ruby
app.integrations.gotify.update_application(id: 7, name: "CI Alerts", description: "Build && deploy notifications")
```
### delete_application
...
```ruby
clients = app.integrations.gotify.list_clients()
```
### create_client
...
```ruby
client = app.integrations.gotify.create_client(name: "Automation")
puts(client.token)
```
### update_client
...
```ruby
app.integrations.gotify.update_client(id: 12, name: "Automation Worker")
```
### delete_client
...
```ruby
health = app.integrations.gotify.get_health()
puts(health.health)
```
### get_version
...
```ruby
version = app.integrations.gotify.get_version()
...
```ruby
user = app.integrations.gotify.get_current_user()
puts(user.name)
```
## Scope Notes
