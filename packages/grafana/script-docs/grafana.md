...
```ruby
dashboards = app.integrations.grafana.search(query: "latency", type: "dash-db", limit: 25)
```
```ruby
dashboard = app.integrations.grafana.get_dashboard_by_uid(uid: "service-latency")
```
```ruby
result = app.integrations.grafana.create_update_dashboard(body: {dashboard: {uid: "agent-demo", title: "Agent Demo", panels: {}}, overwrite: true})
```
```ruby
rules = app.integrations.grafana.get_all_alert_rules()
```
## Return Shapes
...
```ruby
example = {body: "...", content_type: "text/csv"}
```
