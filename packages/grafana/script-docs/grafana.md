# Grafana JavaScript API Reference
...
```js
var dashboards = grafana.grafana_list_dashboards({
  query: "latency",
  type: "dash-db",
  limit: 25,
})
```
```js
var dashboard = grafana.grafana_get_dashboard({
  uid: "service-latency",
})
```
```js
var result = grafana.grafana_create_dashboard({
  body: {
    dashboard: {
      uid: "agent-demo",
      title: "Agent Demo",
...
```
```js
var rules = grafana.grafana_list_alerts({})
```
## Return Shapes
