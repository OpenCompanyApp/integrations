...
```ruby
token = app.integrations.greenhouse.generate_access_token(body: {sub: 123456})
candidates = app.integrations.greenhouse.list_candidates(per_page: 50, created_at: {gte: "2024-01-01T00:00:00Z"})
candidate = app.integrations.greenhouse.create_candidate(body: {first_name: "Ada", last_name: "Lovelace", email_addresses: [{value: "ada@example.test", type: "personal"}]})
```
Use fake names, email addresses, IDs, and tokens in tests and examples. Never store real Greenhouse candidate, employee, job, offer, interview, webhook, or OAuth credential data in fixtures or Ruby examples.
