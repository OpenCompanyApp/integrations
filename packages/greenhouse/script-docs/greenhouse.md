...
```js
var token = app.integrations.greenhouse.post_auth_token({
  body: { sub: 123456 },
})

var candidates = app.integrations.greenhouse.get_v3_candidates({
  per_page: 50,
  created_at: { gte: "2024-01-01T00:00:00Z" },
})

var candidate = app.integrations.greenhouse.post_v3_candidates({
  body: {
    first_name: "Ada",
    last_name: "Lovelace",
...
```
Use fake names, email addresses, IDs, and tokens in tests and examples. Never store real Greenhouse candidate, employee, job, offer, interview, webhook, or OAuth credential data in fixtures or JavaScript examples.
