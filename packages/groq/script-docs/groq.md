...
```ruby
result = app.integrations.groq.create_chat_completion(model: "llama-3.3-70b-versatile", messages: [{role: "system", content: "You are concise."}, {role: "user", content: "Summarize this paragraph."}], temperature: 0.2, max_tokens: 120)
puts(result.choices[0].message.content)
```
Use payload-based tools when the upstream endpoint has a richer request body:

```ruby
response = app.integrations.groq.create_response(payload: {model: "openai/gpt-oss-20b", input: "Draft a release note."})
transcription = app.integrations.groq.create_transcription(payload: {model: "whisper-large-v3", url: "https://example.test/audio/sample.wav", response_format: "json"})
```
...
```ruby
example = {content_type: "audio/wav", body_base64: "..."}
```
### Batches and Files
