 ...
 * "newest" stories. This tool fetches the list and then retrieves the
 * full item data for the top N stories.
 *
 * @see https://github.com/HackerNews/API#new-top-and-best-stories
 */
class HackerNewsListNewStories implements Tool
{
    /**
    ...
    {
        return 'hackernews_list_new_stories';
    }
    ...
    {
        return 'Fetch the newest stories from Hacker News. Returns up to N stories with full item data (title, URL, score, author, comment count). These are the most recently submitted stories, not yet ranked by the HN algorithm.';
    }
    ...
    /**
     * Execute the tool — fetch new story IDs and resolve to full items.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            $ids = $this->service->newStories();

            if (empty($ids)) {
                return ToolResult::error('Failed to fetch new stories from Hacker News. The API may be temporarily unavailable.');
            }
