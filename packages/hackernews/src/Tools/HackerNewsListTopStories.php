 ...
 *
 * The HN API returns an ordered list of up to ~500 item IDs. This tool
 * fetches the list and then retrieves the full item data for the top N
 * stories (configurable via the `limit` parameter).
 *
 * @see https://github.com/HackerNews/API#new-top-and-best-stories
 */
class HackerNewsListTopStories implements Tool
{
    /**
    ...
    {
        return 'hackernews_list_top_stories';
    }
    ...
    {
        return 'Fetch the current top stories from Hacker News. Returns up to N stories with full item data (title, URL, score, author, comment count). The "top" list is ranked by the HN algorithm (a combination of score, recency, and flags).';
    }
    ...
    /**
     * Execute the tool — fetch top story IDs and resolve to full items.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            $ids = $this->service->topStories();

            if (empty($ids)) {
                return ToolResult::error('Failed to fetch top stories from Hacker News. The API may be temporarily unavailable.');
            }
