# Client for the Harvest REST API v2 — Ruby API Reference
...
```ruby
result = app.integrations.harvest.create_time_entry(project_id: 0, task_id: 0, spent_date: "")
```
...
```ruby
result = app.integrations.harvest.delete_time_entry(id: 0)
```
...
```ruby
result = app.integrations.harvest.get_current_user()
```
## harvest_get_project
...
```ruby
result = app.integrations.harvest.get_project(id: 0)
```
...
```ruby
result = app.integrations.harvest.get_time_entry(id: 0)
```
## harvest_get_user
...
```ruby
result = app.integrations.harvest.get_user(id: 0)
```
...
```ruby
result = app.integrations.harvest.list_clients(is_active: true, page: 0)
```
...
```ruby
result = app.integrations.harvest.list_projects(client_id: 0, is_active: true, page: 0)
```
## harvest_list_tasks
...
```ruby
result = app.integrations.harvest.list_tasks(is_active: true, page: 0)
```
...
```ruby
result = app.integrations.harvest.list_time_entries(user_id: 0, client_id: 0, project_id: 0)
```
## harvest_list_users
...
```ruby
result = app.integrations.harvest.list_users(is_active: true, page: 0, per_page: 0)
```
...
```ruby
result = app.integrations.harvest.update_time_entry(id: 0, hours: 0, notes: "")
```
---
