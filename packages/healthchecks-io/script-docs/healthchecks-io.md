...
```ruby
checks = app.call("integrations.healthchecks-io.list", tag: ["backup", "prod"])
created = app.call("integrations.healthchecks-io.create", body: {name: "Database Backup", slug: "database-backup", tags: "backup prod", timeout: 3600, grace: 600})
ping = app.call("integrations.healthchecks-io.ping_success_slug", ping_key: "example-ping-key", slug: "database-backup", create: 1, body_text: "backup completed")
```
Use fake UUIDs, slugs, ping keys, check names, and diagnostic text in tests and examples. Do not store real ping keys, project API keys, customer details, or production incident logs in fixtures or Ruby examples.
