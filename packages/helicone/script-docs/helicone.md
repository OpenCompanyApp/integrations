...
```ruby
rows = app.integrations.helicone.query_requests(body: {filter: {}, limit: 10, offset: 0, sort: {created_at: "desc"}})
```
...
```ruby
app.integrations.helicone.submit_feedback(request_id: "request-uuid", body: {rating: true})
```
...
```ruby
response = app.integrations.helicone.gateway_chat_completions(body: {model: "openai/gpt-4o-mini", messages: [{role: "user", content: "Summarize this trace."}]})
```
## Coverage Notes
