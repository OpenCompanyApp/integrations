# HelpScout — JavaScript API Reference
...
```js
var result = app.integrations.helpscout.list_conversations({
  status: "open",
  mailbox: 123,
  per_page: 10,
})
...
for (const conv of (result._embedded.conversations)) {
  console.log(conv.id + ": " + conv.subject + " (" + conv.status + ")")
}
```
---
...
```js
var conv = app.integrations.helpscout.get_conversation({ id: 456 })
console.log("Subject: " + conv.subject)
console.log("Status: " + conv.status)
console.log("Customer: " + conv._embedded.customer.firstName + " " + conv._embedded.customer.lastName)
```
---
...
```js
var result = app.integrations.helpscout.create_conversation({
...
```js
var result = app.integrations.helpscout.list_customers({
  email: "jane@example.com",
})

for (const customer of (result._embedded.customers)) {
...
```js
var customer = app.integrations.helpscout.get_customer({ id: 789 })
console.log(customer.firstName + " " + customer.lastName)
console.log("Email: " + customer._embedded.emails[0].value)
```
---
...
```js
var result = app.integrations.helpscout.create_customer({
  first_name: "Jane",
  last_name: "Doe",
  email: "jane@example.com",
...
```js
var result = app.integrations.helpscout.list_mailboxes({})

for (const mailbox of (result._embedded.mailboxes)) {
  console.log(mailbox.id + ": " + mailbox.name + " (" + mailbox.email + ")")
}
```
---
...
```js
var mailbox = app.integrations.helpscout.get_mailbox({ id: 123 })
console.log("Mailbox: " + mailbox.name)
...
```js
var user = app.integrations.helpscout.get_current_user({})
...
app.integrations.helpscout.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.helpscout.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.helpscout.work.function_name({ /* parameters */ })
app.integrations.helpscout.support.function_name({ /* parameters */ })
```
