# Hetzner Cloud Ruby API Reference
...
```ruby
servers = app.integrations.hetzner.list_servers(per_page: 25, page: 1)
```
```ruby
server = app.integrations.hetzner.get_server(id: 123456)
```
```ruby
created = app.integrations.hetzner.create_server(body: {name: "agent-demo", server_type: "cx22", image: "ubuntu-24.04", location: "fsn1", ssh_keys: ["agent-key"]})
```
```ruby
action = app.integrations.hetzner.power_server(id: 123456)
```
## Return Shapes
