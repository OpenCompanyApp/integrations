# Hetzner Cloud JavaScript API Reference
...
```js
var servers = hetzner.hetzner_list_servers({
  per_page: 25,
  page: 1,
})
```
```js
var server = hetzner.hetzner_get_server({
  id: 123456,
})
```
```js
var created = hetzner.hetzner_create_server({
  body: {
    name: "agent-demo",
    server_type: "cx22",
    image: "ubuntu-24.04",
...
```
```js
var action = hetzner.hetzner_poweron_server({
  id: 123456,
})
```
## Return Shapes
