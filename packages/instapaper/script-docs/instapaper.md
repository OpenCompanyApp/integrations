...
```ruby
app.integrations.instapaper.get_access_token(x_auth_username: "reader@example.test", x_auth_password: "fake-password")
```
...
```ruby
app.integrations.instapaper.add_bookmark(url: "https://example.test/article", payload: {title: "Example Article", selection: "Short note", folder_id: "12345"})
app.integrations.instapaper.list_bookmarks(payload: {folder_id: "unread", limit: 25})
```
...
```ruby
app.integrations.instapaper.add_folder(title: "Research")
app.integrations.instapaper.set_folder_order(folder_ids: "12345,67890")
```
## Highlights
...
```ruby
app.integrations.instapaper.list_highlights(bookmark_id: "111")
app.integrations.instapaper.create_highlight(bookmark_id: "111", text: "Important passage", payload: {note: "Use in brief"})
app.integrations.instapaper.delete_highlight(highlight_id: "222")
```
## Simple API
...
```ruby
app.integrations.instapaper.simple_authenticate(username: "reader@example.test", password: "fake-password")
app.integrations.instapaper.simple_add_url(username: "reader@example.test", password: "fake-password", url: "https://example.test/post", payload: {title: "Example Post", selection: "Save for later"})
```
...
```ruby
app.integrations.instapaper.api_post(path: "/api/1/bookmarks/list", payload: {limit: 10})
```