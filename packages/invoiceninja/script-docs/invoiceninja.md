# Invoice Ninja Ruby API Reference
...
```ruby
clients = app.integrations.invoiceninja.list_clients(per_page: 25, sort: "name|desc")
quote = app.integrations.invoiceninja.create_quote(payload: {client_id: "client_123", line_items: [{product_key: "consulting", notes: "Planning call", quantity: 1, cost: 250}]})
task = app.integrations.invoiceninja.update_task(id: "task_123", payload: {description: "Follow up with client", project_id: "project_123"})
```
## Payload Tools
...
```ruby
app.integrations.invoiceninja.bulk(payload: {action: "archive", ids: ["invoice_123"]})
```
## Payments
...
```ruby
result = app.integrations.invoiceninja.refund_payment(id: "payment_123", payload: {amount: 25, reason: "Duplicate charge"})
```
...
```ruby
app.integrations.invoiceninja.list(per_page: 20)
app.integrations.invoiceninja.default.list(per_page: 20)
app.integrations.invoiceninja.production.list(per_page: 20)
```
