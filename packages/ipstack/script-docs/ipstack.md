...
```ruby
result = app.integrations.ipstack.standard_lookup(ip: "134.201.250.155", fields: ["main", "location", "timezone"], language: "en")
```
Optional parameters:
...
```ruby
result = app.integrations.ipstack.bulk_lookup(ips: ["134.201.250.155", "72.229.28.185"], fields: ["main", "connection"])
```
...
```ruby
result = app.integrations.ipstack.requester_lookup(fields: ["main", "location"])
```
## Notes
