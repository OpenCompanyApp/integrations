...
```ruby
users = app.integrations.keycloak.keycloak_get_admin_realms_realm_users(realm: "master", username: "alice", exact: true, brief_representation: true)
```
```ruby
created = app.integrations.keycloak.create_new_user_username_must_be_unique(realm: "master", body: {username: "agent-test-user", enabled: true, email: "agent-test@example.test"})
```
