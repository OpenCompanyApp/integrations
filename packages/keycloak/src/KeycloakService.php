 ...
 *
 * Handles bearer authentication, base URL normalization, request dispatch,
 * path/query/header construction, and API error handling for generated tools.
 */
class KeycloakService
{
    /** @param  string  $accessToken  Keycloak bearer access token. @param  string  $baseUrl  Keycloak server base URL, for example https://keycloak.example.test. */
    public function __construct(private string $accessToken = '', private string $baseUrl = 'https://keycloak.example.test') { $this->baseUrl = rtrim($this->baseUrl, '/'); }
    public function isConfigured(): bool { return $this->accessToken !== ''; }
    /** @return array<string, array<string, mixed>> */ public static function operations(): array { return KeycloakOperations::all(); }
    ...
    public function executeOperation(array $operation, array $args = []): array
    {
        ...
        if (($operation['body_required'] ?? false) && ($body === null || $body === '' || $body === [])) { throw new RuntimeException('body must be a non-empty object matching the Keycloak Admin REST API request schema.'); }
    ...
    {
        if (! $this->isConfigured()) { throw new RuntimeException('Keycloak access token is not configured.'); }
        $method = strtoupper($method); $url = $this->urlWithQuery($this->baseUrl.$path, $query);
        try { $http = Http::withHeaders(array_merge(['Authorization' => 'Bearer '.$this->accessToken, 'Accept' => 'application/json'], $headers))->timeout(120); if ($body !== null) { if (is_array($body) && is_string($contentType) && str_contains($contentType, 'application/x-www-form-urlencoded')) { $response = match($method){'POST'=>$http->asForm()->post($url,$body),'PUT'=>$http->asForm()->put($url,$body),'PATCH'=>$http->asForm()->patch($url,$body),default=>$http->asForm()->send($method,$url,['form_params'=>$body])}; } elseif (is_array($body)) { $response = $http->withHeaders(['Content-Type' => $contentType ?: 'application/json'])->send($method, $url, ['json' => $body]); } else { $response = $http->withBody((string)$body, $contentType ?: 'application/octet-stream')->send($method, $url); } } else { $response = match($method){'GET'=>$http->get($url),'POST'=>$http->post($url,[]),'PUT'=>$http->put($url,[]),'PATCH'=>$http->patch($url,[]),'DELETE'=>$http->delete($url,[]),default=>$http->send($method,$url)}; } } catch (\Illuminate\Http\Client\ConnectionException $e) { Log::error("Keycloak API connection error: {$method} {$path}", ['error'=>$e->getMessage()]); throw new RuntimeException('Failed to connect to Keycloak API: '.$e->getMessage()); }
        if (! $response->successful()) { $error = $response->json('error_description') ?? $response->json('errorMessage') ?? $response->json('message') ?? $response->json('error') ?? $response->body(); Log::error("Keycloak API error: {$method} {$path}", ['status'=>$response->status(),'error'=>$error]); throw new RuntimeException('Keycloak API error ('.$response->status().'): '.(is_string($error)?$error:json_encode($error))); }
        return $response;
    }
