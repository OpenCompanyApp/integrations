# LaunchDarkly Ruby API Reference
...
```ruby
projects = app.integrations.launchdarkly.api_get(path: "/projects", query: {limit: 20})
flag = app.integrations.launchdarkly.api_get(path: "/flags/default/checkout-flow")
```
## Projects
...
```ruby
created = app.integrations.launchdarkly.create_project(key: "web-app", name: "Web App", tags: ["example"])
updated = app.integrations.launchdarkly.update_project(project_key: "web-app", patch: [{op: "replace", path: "/name", value: "Web Application"}])
```
...
```ruby
env = app.integrations.launchdarkly.create_environment(project_key: "web-app", key: "qa", name: "QA", color: "DADBEE", require_comments: true)
```
## Feature Flags
...
```ruby
flags = app.integrations.launchdarkly.list_flags(project_key: "web-app", env: "production", limit: 50)
flag = app.integrations.launchdarkly.create_feature_flag(project_key: "web-app", key: "checkout-flow", name: "Checkout Flow", kind: "boolean", temporary: true, tags: ["release"])
toggled = app.integrations.launchdarkly.toggle_flag(project_key: "web-app", feature_flag_key: "checkout-flow", environment_key: "production", enabled: true)
```
For advanced flag changes, `update_feature_flag` accepts either a JSON Patch list or a semantic patch-style body:

```ruby
patched = app.integrations.launchdarkly.update_feature_flag(project_key: "web-app", feature_flag_key: "checkout-flow", patch: [{op: "replace", path: "/environments/production/on", value: false}])
semantic = app.integrations.launchdarkly.update_feature_flag(project_key: "web-app", feature_flag_key: "checkout-flow", body: {environmentKey: "production", instructions: [{kind: "turnFlagOn"}]})
```
...
```ruby
segment = app.integrations.launchdarkly.create_segment(project_key: "web-app", environment_key: "production", key: "beta-users", name: "Beta Users", tags: ["example"])
```
...
```ruby
members = app.integrations.launchdarkly.list_members(limit: 20, filter: "query:alex")
invited = app.integrations.launchdarkly.invite_members(body: [{email: "person@example.test", role: "reader"}])
```
...
```ruby
teams = app.integrations.launchdarkly.list_teams(filter: "query:platform", expand: "members,maintainers")
team = app.integrations.launchdarkly.create_team(key: "platform", name: "Platform", description: "Platform engineering")
```
...
```ruby
result = app.integrations.launchdarkly.accounts.production.list_flags(project_key: "web-app", env: "production")
```
## Safety Notes
