# LINE Messaging - Ruby API Reference
...
```ruby
app.integrations.line.reply_message(reply_token: "reply-token-from-webhook", messages: [{type: "text", text: "Thanks."}])
app.integrations.line.send_push_message(to: "U0000000000", messages: [{type: "text", text: "Your order is ready."}], notification_disabled: false, custom_aggregation_units: "orders")
app.integrations.line.multicast_message(to: ["U0000000000", "U1111111111"], messages: [{type: "text", text: "A shared update."}])
app.integrations.line.narrowcast_message(messages: [{type: "text", text: "Segment update."}], recipient: {type: "operator", and: [{type: "audience", audienceGroupId: 1234567890}]})
app.integrations.line.broadcast_message(messages: [{type: "text", text: "Announcement for all followers."}])
```
...
```ruby
app.integrations.line.set_webhook_endpoint(endpoint: "https://example.test/line/webhook")
webhook = app.integrations.line.get_webhook_endpoint()
test = app.integrations.line.test_webhook_endpoint(endpoint: "https://example.test/line/webhook")
```
...
```ruby
profile = app.integrations.line.get_profile(user_id: "U0000000000")
followers = app.integrations.line.list_followers(limit: 100, start: nil)
bot = app.integrations.line.get_bot_info()
```
...
```ruby
summary = app.integrations.line.get_group_summary(group_id: "C0000000000")
count = app.integrations.line.get_group_member_count(group_id: "C0000000000")
members = app.integrations.line.list_group_member_ids(group_id: "C0000000000", start: nil)
member = app.integrations.line.get_group_member_profile(group_id: "C0000000000", user_id: "U0000000000")
```
...
```ruby
rich_menu = {size: {width: 2500, height: 843}, selected: false, name: "main-menu", chatBarText: "Menu", areas: {}}
app.integrations.line.validate_rich_menu(rich_menu: rich_menu)
created = app.integrations.line.create_rich_menu(rich_menu: rich_menu)
list = app.integrations.line.list_rich_menus()
one = app.integrations.line.get_rich_menu(rich_menu_id: "richmenu-123")
```
...
```ruby
token = app.integrations.line.issue_link_token(user_id: "U0000000000")
```
...
app.integrations.line.default.send_push_message()
app.integrations.line.production.send_push_message()
```
