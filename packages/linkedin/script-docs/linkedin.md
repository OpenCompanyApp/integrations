# Client for the LinkedIn REST API v2 covering posts, organizations, and ad accounts — Ruby API Reference
...
```ruby
result = app.integrations.linkedin.list_posts(author: "urn:li:person:ABC123", count: 10, start: 0)
```
## linkedin_get_post
...
```ruby
result = app.integrations.linkedin.get_post(post_id: "urn:li:ugcPost:123456789")
```
...
```ruby
result = app.integrations.linkedin.create_post(author: "urn:li:person:ABC123", text: "Hello from the LinkedIn API!", visibility: "PUBLIC")
```
...
```ruby
result = app.integrations.linkedin.list_organizations(count: 10, start: 0)
```
...
```ruby
result = app.integrations.linkedin.get_organization(organization_id: "12345")
```
...
```ruby
result = app.integrations.linkedin.list_ad_accounts(count: 10, start: 0)
```
...
```ruby
result = app.integrations.linkedin.get_current_user()
```
---
