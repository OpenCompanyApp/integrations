...
```ruby
users = app.integrations.logto.get_users(page: 1, page_size: 20)
```
```ruby
created = app.integrations.logto.create_user(body: {primaryEmail: "agent-test@example.test", name: "Agent Test"})
```