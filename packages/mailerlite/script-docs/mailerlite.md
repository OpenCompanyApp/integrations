...
```ruby
created = app.integrations.mailerlite.create_or_upsert_subscriber(email: "reader@example.test", fields: {name: "Ada Example", company: "Example Co"}, groups: ["1234567890"], status: "active")
```
```ruby
activity = app.integrations.mailerlite.list_campaign_subscriber_activity(campaign_id: "66200823885989563", limit: 50)
```
```ruby
webhook = app.integrations.mailerlite.create_webhook(name: "Subscriber events", url: "https://example.test/mailerlite", events: ["subscriber.created", "subscriber.updated"], enabled: true)
```