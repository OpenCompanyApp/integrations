...
```ruby
app.integrations.mailgun.send_email(from: "Example <noreply@example.test>", to: ["user@example.test"], subject: "Hello", text: "Hello from Mailgun")
events = app.integrations.mailgun.list_events(event: "delivered", limit: 100)
bounce = app.integrations.mailgun.create_bounce(address: "bad@example.test", code: 550, error: "Suppressed by support request")
```
## Coverage Notes
