...
/**
 * Base class for focused Mailgun API endpoint tools.
 *
 * Handles configuration checks, default-domain substitution, required
 * arguments, path interpolation, and query/body shaping.
 */
abstract class AbstractMailgunEndpointTool implements Tool
{
    ...
    public function __construct(
        protected MailgunService $service,
    ) {}
    ...
    /**
     * Execute one Mailgun API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Mailgun integration is not configured. API key is required.');
            }
            ...
            $result = match ($this->method) {
                ...
                default => throw new RuntimeException("Unsupported Mailgun tool method: {$this->method}"),
            };
    ...
    private function interpolatePath(array $args): string
    {
        return preg_replace_callback('/\{([a-zA-Z0-9_]+)\}/', function (array $matches) use ($args): string {
            ...
            if (($value === null || $value === '') && $key === 'domain') {
                $value = $this->service->getConfiguredDomain();
            }
