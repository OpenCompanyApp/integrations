...
```ruby
users = app.integrations.mattermost.api_get(path: "/users", query: {page: 0, per_page: 20})
```
## Users

```ruby
me = app.integrations.mattermost.get_current_user()
results = app.integrations.mattermost.search_users(term: "alex", team_id: "team_123")
```
User tools:
...
```ruby
teams = app.integrations.mattermost.list_teams(page: 0, per_page: 60)
app.integrations.mattermost.add_team_member(team_id: "team_123", user_id: "user_123")
```
Team tools:
...
```ruby
channels = app.integrations.mattermost.list_team_channels(team_id: "team_123", page: 0, per_page: 50)
created = app.integrations.mattermost.create_channel(team_id: "team_123", name: "release-updates", display_name: "Release Updates", type: "O")
```
Channel tools:
...
```ruby
post = app.integrations.mattermost.create_post(channel_id: "channel_123", message: "Deployment finished.")
thread = app.integrations.mattermost.get_post_thread(post_id: post.id)
```
