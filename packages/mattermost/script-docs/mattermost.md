...
```js
var users = app.integrations.mattermost.mattermost_api_get({
  path: "/users",
  query: {
    page: 0,
    per_page: 20,
...
```js
var me = app.integrations.mattermost.mattermost_get_current_user({})

var results = app.integrations.mattermost.mattermost_search_users({
  term: "alex",
  team_id: "team_123",
})
```
...
```js
var teams = app.integrations.mattermost.mattermost_list_teams({
  page: 0,
  per_page: 60,
})
...
```js
var channels = app.integrations.mattermost.mattermost_list_team_channels({
  team_id: "team_123",
  page: 0,
  per_page: 50,
})

var created = app.integrations.mattermost.mattermost_create_channel({
  team_id: "team_123",
...
```js
var post = app.integrations.mattermost.mattermost_create_post({
  channel_id: "channel_123",
  message: "Deployment finished.",
})

var thread = app.integrations.mattermost.mattermost_get_post_thread({
  post_id: post.id,
})
```
