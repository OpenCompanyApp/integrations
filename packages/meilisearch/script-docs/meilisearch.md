# Meilisearch JavaScript Reference
...
```js
var indexes = app.integrations.meilisearch.list_indexes({
  limit: 20,
  offset: 0,
})
```
...
```js
var task = app.integrations.meilisearch.create_index({
  uid: "books",
  primaryKey: "id",
})
```
...
```js
var task = app.integrations.meilisearch.add_documents({
  index_uid: "books",
  body: [
...
```js
var results = app.integrations.meilisearch.search_documents({
  index_uid: "books",
  q: "ranking",
  limit: 10,
...
```js
var task = app.integrations.meilisearch.update_all({
  index_uid: "books",
  body: {
...
```js
var tasks = app.integrations.meilisearch.get_tasks({
  indexUids: [ "books" ],
