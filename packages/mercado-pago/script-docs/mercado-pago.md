# Mercado Pago — JavaScript API Reference
...
```js
var result = app.integrations["mercado-pago"].list_payments({
  status: "approved",
  limit: 10,
})

for (const payment of (result.results)) {
  console.log(payment.id + ": " + payment.transaction_amount + " " + payment.currency_id)
}
```
---
...
```js
var result = app.integrations["mercado-pago"].get_payment({
  id: "1234567890",
})

console.log("Status: " + result.status)
console.log("Amount: " + result.transaction_amount)
console.log("Payer: " + result.payer.email)
```
---
...
```js
var result = app.integrations["mercado-pago"].create_payment({
...
```js
var result = app.integrations["mercado-pago"].list_customers({
...
```js
var result = app.integrations["mercado-pago"].get_customer({
...
```js
var result = app.integrations["mercado-pago"].list_preferences({
  limit: 10,
})

for (const pref of (result.elements)) {
  console.log(pref.id + ": " + pref.items[0].title)
}
```
---
...
```js
var result = app.integrations["mercado-pago"].get_current_user({})
...
app.integrations["mercado-pago"].list_payments({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations["mercado-pago"].default.list_payments({ /* parameters */ })

// Named accounts
app.integrations["mercado-pago"].brazil.list_payments({ /* parameters */ })
app.integrations["mercado-pago"].argentina.list_payments({ /* parameters */ })
```
