...
```ruby
result = app.integrations.mermaid.render(syntax: "graph TD\n  A[Start] // > B{Decision}\n  B -->|Yes| C[Action]\n  B -->|No| D[End]", title: "Simple Flowchart", theme: "default")
```
#### Render a sequence diagram with dark theme

```ruby
result = app.integrations.mermaid.render(syntax: "sequenceDiagram\n  participant Client\n  participant Server\n  Client->>Server: Request\n  Server-->>Client: Response", title: "Client-Server Sequence", theme: "dark")
```
#### Render an ER diagram

```ruby
result = app.integrations.mermaid.render(syntax: "erDiagram\n  CUSTOMER ||--o{ ORDER : places\n  ORDER ||--|{ LINE-ITEM : contains", title: "Entity Relationship", width: 1200)
```