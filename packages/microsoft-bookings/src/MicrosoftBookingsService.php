...
/**
 * HTTP client for Microsoft Bookings through Microsoft Graph v1.0.
 *
 * Handles bearer-token authentication, OData query serialization, path expansion,
 * JSON request bodies, optional Graph headers, error logging, and response parsing.
 */
class MicrosoftBookingsService
{
    ...
    /**
     * Execute a Microsoft Bookings request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Bookings access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Bookings Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Bookings request failed.';
            throw new RuntimeException('Microsoft Bookings API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
    ...
    private function urlWithQuery(string $url, array $query): string
    {
        ...
        foreach ($query as $key => $value) {
            ...
            if (is_array($value)) { $parts[] = rawurlencode((string) $key).'='.rawurlencode(implode(',', array_map(static fn (mixed $item): string => is_scalar($item) ? (string) $item : json_encode($item), $value))); continue; }
