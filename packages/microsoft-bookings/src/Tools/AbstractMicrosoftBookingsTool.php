...
/**
 * Shared executor for generated Microsoft Bookings tools.
 *
 ...
 */
abstract class AbstractMicrosoftBookingsTool implements Tool
{
    ...
    /** @param  MicrosoftBookingsService  $service  Microsoft Bookings API client. */
    public function __construct(protected MicrosoftBookingsService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Bookings access token is required.'); }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Bookings request schema.'); }
        return $body;
    }
}
