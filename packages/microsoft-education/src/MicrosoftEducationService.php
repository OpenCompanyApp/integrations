...
/**
 * HTTP client for Microsoft Education resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * education request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftEducationService
{
    /**
    ...
    /**
     * Execute a Microsoft Education request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Education access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Education Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Education request failed.';
            throw new RuntimeException('Microsoft Education API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
