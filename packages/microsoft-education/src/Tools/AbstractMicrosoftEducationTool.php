...
/**
 * Shared executor for generated Microsoft Education tools.
 *
 ...
 */
abstract class AbstractMicrosoftEducationTool implements Tool
{
    ...
    /** @param  MicrosoftEducationService  $service  Microsoft Education API client. */
    public function __construct(protected MicrosoftEducationService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Education access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph education request schema.'); } return $body; }
}
