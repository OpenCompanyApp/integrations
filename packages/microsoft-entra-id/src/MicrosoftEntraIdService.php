...
/**
 * HTTP client for Microsoft Entra ID resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * directory request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftEntraIdService
{
    ...
    /**
     * Execute a Microsoft Entra ID request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Entra ID access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Entra ID Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Entra ID request failed.';
            throw new RuntimeException('Microsoft Entra ID API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
