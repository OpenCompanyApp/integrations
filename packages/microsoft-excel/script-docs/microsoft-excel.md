...
- Call the exact generated tool slug. Workbook paths have colliding display names, and the full slug (including its collision suffix when present) is the stable callable name.
...
- JSON responses are decoded Graph payloads. Worksheet lists use `result["value"]`; a range response is the range object itself (for example `range["values"]`). Empty `202` or `204` responses are `{"success" => true, "status" => status}`.

## Example

```ruby
worksheets = app.call('integrations.microsoft-excel.microsoft_excel_drives_items_workbook_list_worksheets', drive_id: "drive-id", drive_item_id: "item-id")
worksheet_rows = worksheets["value"] || []
range = app.call('integrations.microsoft-excel.microsoft_excel_drives_drive_items_drive_item_workbook_worksheets_workbook_worksheet_range_b0fa', drive_id: "drive-id", drive_item_id: "item-id", workbook_worksheet_id: "sheet-id", address: "A1:D10")
values = range["values"]
```
