...
```js
var incidents = microsoft_graph_security_list_incidents({ top: 10, filter: "status ne 'resolved'" })
var alert = microsoft_graph_security_get_alerts_v2({ alert_id: "alert-id", select: "id,title,severity,status" })
var scores = microsoft_graph_security_list_secure_scores({ top: 5 })
```