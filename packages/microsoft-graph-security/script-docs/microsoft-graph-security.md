...
```ruby
incidents = app.call("integrations.microsoft-graph-security.list_incidents", top: 10, filter: "status ne 'resolved'")
alert = app.call("integrations.microsoft-graph-security.get_alert", alert_id: "alert-id", select: "id,title,severity,status")
scores = app.call("integrations.microsoft-graph-security.list_securescores", top: 5)
```