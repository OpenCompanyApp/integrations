...
/**
 * HTTP client for Microsoft Graph Security v1.0 APIs.
 *
 * Handles bearer-token authentication, OData query serialization, path expansion,
 * JSON request bodies, error logging, and response parsing for security endpoints.
 */
class MicrosoftGraphSecurityService
{
    ...
    /**
     * Execute a Microsoft Graph Security request and return decoded response data.
     *
    ...
    private function rawRequest(string $method, string $path, array $query = [], array $headers = [], array $body = []): Response
    {
        if (!$this->isConfigured()) {
            throw new RuntimeException('Microsoft Graph Security access token is required.');
        }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Graph Security request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Graph Security request failed.';
            throw new RuntimeException('Microsoft Graph Security API error: ' . (is_string($message) ? $message : json_encode($message)));
        }
