...
/**
 * Shared executor for generated Microsoft Graph Security tools.
 *
 ...
 */
abstract class AbstractMicrosoftGraphSecurityTool implements Tool
{
    ...
    /**
     * @param  MicrosoftGraphSecurityService  $service  Microsoft Graph Security API client.
     */
    public function __construct(protected MicrosoftGraphSecurityService $service) {}
    ...
    /**
     * Execute the mapped Microsoft Graph Security operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Microsoft Graph Security access token is required.');
            }
    ...
    {
        $headers = [];
        if (isset($args['consistency_level']) && $args['consistency_level'] !== '') {
            $headers['ConsistencyLevel'] = (string) $args['consistency_level'];
        }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph Security request schema.');
        }
