...
/**
 * HTTP client for Microsoft Intune resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * Intune request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftIntuneService
{
    ...
    /**
     * Execute a Microsoft Intune request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Intune access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Intune Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Intune request failed.';
            throw new RuntimeException('Microsoft Intune API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
