...
/**
 * HTTP client for Microsoft OneNote through Microsoft Graph v1.0.
 *
...
class MicrosoftOneNoteService
{
    ...
    /**
     * Execute a Microsoft OneNote request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft OneNote access token is required.'); }
        $method = strtoupper($method);
        $pending = Http::withHeaders(array_merge(['Accept' => 'application/json', 'Authorization' => 'Bearer '.$this->accessToken], $headers))->timeout(128);
        if ($bodyMode === 'raw') {
            ...
            $response = $pending->send($method, $this->urlWithQuery($this->baseUrl.$path, $query));
        } else {
            $response = $pending->withHeaders(['Content-Type' => 'application/json'])->send($method, $this->urlWithQuery($this->baseUrl.$path, $query), $body === [] ? [] : ['json' => $body]);
        }
        if (!$response->successful()) {
            Log::error('Microsoft OneNote Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft OneNote request failed.';
            throw new RuntimeException('Microsoft OneNote API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
