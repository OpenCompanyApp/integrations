...
/**
 * HTTP client for Microsoft People resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * people request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftPeopleService
{
    /**
    ...
    /**
     * Execute a Microsoft People request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft People access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft People Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft People request failed.';
            throw new RuntimeException('Microsoft People API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
