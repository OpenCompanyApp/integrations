...
/**
 * Shared executor for generated Microsoft People tools.
 *
 ...
 */
abstract class AbstractMicrosoftPeopleTool implements Tool
{
    ...
    /** @param  MicrosoftPeopleService  $service  Microsoft People API client. */
    public function __construct(protected MicrosoftPeopleService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft People access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph people request schema.'); } return $body; }
}
