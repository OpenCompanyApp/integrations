...
/**
 * HTTP client for Microsoft Places resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * places request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftPlacesService
{
    /**
    ...
    /**
     * Execute a Microsoft Places request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Places access token is required.'); }
        ...
        if (!$response->successful() && !in_array($response->status(), [301, 302, 303, 307, 308], true)) {
            Log::error('Microsoft Places Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Places request failed.';
            throw new RuntimeException('Microsoft Places API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
