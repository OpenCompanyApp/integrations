...
/**
 * Shared executor for generated Microsoft Places tools.
 *
 ...
 */
abstract class AbstractMicrosoftPlacesTool implements Tool
{
    ...
    /** @param  MicrosoftPlacesService  $service  Microsoft Places API client. */
    public function __construct(protected MicrosoftPlacesService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Places access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph Places request schema.'); } return $body; }
}
