...
```lua
local tasks = microsoft_planner_list_tasks({ top = 10 })
local task = microsoft_planner_get_tasks({ planner_task_id = "task-id", select = "id,title,percentComplete" })
local updated = microsoft_planner_update_tasks({
