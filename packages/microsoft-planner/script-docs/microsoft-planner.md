...
```ruby
tasks = app.call("integrations.microsoft-planner.list_plannertask_objects", top: 10)
task = app.call("integrations.microsoft-planner.get_plannertask", planner_task_id: "task-id", select: "id,title,percentComplete")
updated = app.call("integrations.microsoft-planner.update_plannertask", planner_task_id: "task-id", if_match: "W/\"etag\"", body: {percentComplete: 100})
```