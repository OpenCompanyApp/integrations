...
/**
 * HTTP client for Microsoft Planner through Microsoft Graph v1.0.
 *
 * Handles bearer-token authentication, OData query serialization, path expansion,
 * Planner ETag headers, JSON bodies, error logging, and response parsing.
 */
class MicrosoftPlannerService
{
    ...
    /**
     * Execute a Microsoft Planner request and return decoded response data.
     *
    ...
    private function rawRequest(string $method, string $path, array $query = [], array $headers = [], array $body = []): Response
    {
        if (!$this->isConfigured()) {
            throw new RuntimeException('Microsoft Planner access token is required.');
        }
        ...
        $response = Http::withHeaders($baseHeaders)->timeout(128)->withHeaders(['Content-Type' => 'application/json'])->send($method, $url, $body === [] ? [] : ['json' => $body]);

        if (!$response->successful()) {
            Log::error('Microsoft Planner Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Planner request failed.';
            throw new RuntimeException('Microsoft Planner API error: ' . (is_string($message) ? $message : json_encode($message)));
        }
