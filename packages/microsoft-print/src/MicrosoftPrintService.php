...
/**
 * HTTP client for Microsoft Universal Print resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * print request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftPrintService
{
    /**
    ...
    /**
     * Execute a Microsoft Universal Print request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Universal Print access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Universal Print Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Universal Print request failed.';
            throw new RuntimeException('Microsoft Universal Print API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
