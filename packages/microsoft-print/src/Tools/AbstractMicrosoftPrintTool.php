...
/**
 * Shared executor for generated Microsoft Universal Print tools.
 *
 ...
 */
abstract class AbstractMicrosoftPrintTool implements Tool
{
    ...
    /** @param  MicrosoftPrintService  $service  Microsoft Universal Print API client. */
    public function __construct(protected MicrosoftPrintService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Universal Print access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph Universal Print request schema.'); } return $body; }
}
