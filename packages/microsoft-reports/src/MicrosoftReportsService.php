...
/**
 * HTTP client for Microsoft Reports resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * report request headers, JSON request bodies, redirect handling, error logging, and response parsing.
 */
class MicrosoftReportsService
{
    /**
    ...
    /**
     * Execute a Microsoft Reports request and return decoded response data.
     *
    ...
    public function request(string $method, string $pathTemplate, array $pathParams = [], array $query = [], array $headers = [], array $body = []): array
    {
        ...
        if ($response->status() === 202 || $response->status() === 204 || $response->body() === '') { return ['success' => true, 'status' => $response->status(), 'location' => $response->header('Location')]; }
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Reports access token is required.'); }
        $method = strtoupper($method);
        $response = Http::withHeaders(array_merge(['Accept' => 'application/json', 'Authorization' => 'Bearer '.$this->accessToken], $headers))->timeout(128)->withOptions(['allow_redirects' => false])->withHeaders(['Content-Type' => 'application/json'])->send($method, $this->urlWithQuery($this->baseUrl.$path, $query), $body === [] ? [] : ['json' => $body]);
        if (!$response->successful() && !in_array($response->status(), [301, 302, 303, 307, 308], true)) {
            Log::error('Microsoft Reports Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Reports request failed.';
            throw new RuntimeException('Microsoft Reports API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
