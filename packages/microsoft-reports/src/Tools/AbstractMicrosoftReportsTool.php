...
/**
 * Shared executor for generated Microsoft Reports tools.
 *
 ...
 */
abstract class AbstractMicrosoftReportsTool implements Tool
{
    ...
    /** @param  MicrosoftReportsService  $service  Microsoft Reports API client. */
    public function __construct(protected MicrosoftReportsService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Reports access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph reports request schema.'); } return $body; }
}
