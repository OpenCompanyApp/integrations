...
/**
 * HTTP client for Microsoft Search resources through Microsoft Graph v1.0.
 *
 * Handles bearer authentication, OData query serialization, path expansion,
 * search request headers, JSON request bodies, error logging, and response parsing.
 */
class MicrosoftSearchService
{
    /**
    ...
    /**
     * Execute a Microsoft Search request and return decoded response data.
     *
    ...
    {
        if (!$this->isConfigured()) { throw new RuntimeException('Microsoft Search access token is required.'); }
        ...
        if (!$response->successful()) {
            Log::error('Microsoft Search Graph request failed', ['method' => $method, 'path' => $path, 'status' => $response->status(), 'body' => $response->body()]);
            $message = $response->json('error.message') ?? $response->json('message') ?? $response->body() ?: 'Microsoft Search request failed.';
            throw new RuntimeException('Microsoft Search API error: '.(is_string($message) ? $message : json_encode($message)));
        }
        return $response;
    }
