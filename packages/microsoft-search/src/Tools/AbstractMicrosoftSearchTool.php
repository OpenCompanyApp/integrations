...
/**
 * Shared executor for generated Microsoft Search tools.
 *
 ...
 */
abstract class AbstractMicrosoftSearchTool implements Tool
{
    ...
    /** @param  MicrosoftSearchService  $service  Microsoft Search API client. */
    public function __construct(protected MicrosoftSearchService $service) {}
    ...
    {
        try {
            if (!$this->service->isConfigured()) { return ToolResult::error('Microsoft Search access token is required.'); }
    ...
    private function body(array $args): array { $body = $args['body'] ?? []; if ($body !== [] && !is_array($body)) { throw new InvalidArgumentException('body must be an object.'); } if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) { throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph search request schema.'); } return $body; }
}
