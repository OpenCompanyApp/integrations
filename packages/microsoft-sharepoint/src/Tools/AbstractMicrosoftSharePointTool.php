...
/**
 * Shared executor for generated Microsoft SharePoint Graph tools.
 *
 ...
 */
abstract class AbstractMicrosoftSharePointTool implements Tool
{
    ...
    /**
     * @param  MicrosoftSharePointService  $service  Microsoft Graph API client.
     */
    public function __construct(protected MicrosoftSharePointService $service) {}
    ...
    /**
     * Execute the mapped Microsoft Graph operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Microsoft SharePoint access token is required.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the Microsoft Graph request schema.');
        }
