# Milvus — JavaScript API Reference
...
```js
var result = app.integrations.milvus.list_collections({
  limit: 50,
})
...
```js
var result = app.integrations.milvus.get_collection({
  collection_name: "knowledge_base",
})

console.log("Name: " + result.collectionName)
...
```js
var result = app.integrations.milvus.create_collection({
  name: "knowledge_base",
  dimension: 1536,
  description: "Product documentation embeddings",
})

console.log("Created collection: " + result.collectionName)
```
...
```js
var result = app.integrations.milvus.insert_documents({
  collection_name: "knowledge_base",
  data: [
    {
      id: "doc1",
...
console.log("Inserted " + String(result.insertCount) + " documents")
```
---
...
```js
var result = app.integrations.milvus.search_documents({
  collection_name: "knowledge_base",
  vector: [ 0.1, 0.2, 0.3, 0.4 ],
  limit: 5,
...
for (const [__index, match] of Array.from(result.data).entries()) {
  const i = __index + 1;
  console.log(i + ": " + match.text + " (distance: " + String(match.distance) + ")")
}
```
### Example with filter
...
```js
var result = app.integrations.milvus.get_collection_stats({
  collection_name: "knowledge_base",
})

console.log("Row count: " + String(result.rowCount))
```
---
...
```js
var result = app.integrations.milvus.get_health({})
...
app.integrations.milvus.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.milvus.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.milvus.production.function_name({ /* parameters */ })
app.integrations.milvus.staging.function_name({ /* parameters */ })
```
