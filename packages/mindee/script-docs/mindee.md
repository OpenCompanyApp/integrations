...
```ruby
result = app.integrations.mindee.predict_document(account: "mindee", api_name: "expense_receipts", api_version: "v5", document: "/tmp/receipt.jpg")
```
Parameters:
...
```ruby
queued = app.integrations.mindee.predict_document_async(account: "mindee", api_name: "invoices", api_version: "v4", document: "/tmp/invoice.pdf")
status = app.integrations.mindee.get_async_prediction(account: "mindee", api_name: "invoices", api_version: "v4", job_id: queued.job.id)
```
...
```ruby
invoice = app.integrations.mindee.parse_invoice(document: "/tmp/invoice.pdf")
receipt = app.integrations.mindee.parse_receipt(document: "/tmp/receipt.jpg")
passport = app.integrations.mindee.parse_passport(document: "/tmp/passport.png")
custom = app.integrations.mindee.parse_custom_document(endpoint_id: "acme/purchase_orders/v1", document: "/tmp/purchase_order.pdf")
```
...
app.integrations.mindee.production.predict_document(account: "mindee", api_name: "invoices", api_version: "v4", document: "/tmp/invoice.pdf")
```