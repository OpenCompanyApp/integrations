...
- Stream-style endpoints are exposed as normal request/response tools that
  ...
  those endpoints with a streaming transport outside the JavaScript bridge.
...
```js
var response = mistral_chat_completions({
  body: {
...
```js
var uploaded = mistral_upload_file({
...
```js
var job = mistral_create_batch_job({
  body: {
    input_files: [ "file-id" ],
...
```js
var document = mistral_upload_library_document({
