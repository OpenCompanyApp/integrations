...
- Stream-style endpoints are exposed as normal request/response tools that
  ...
  those endpoints with a streaming transport outside the Ruby bridge.
...
```ruby
response = app.integrations.mistral.chat_completions(body: {model: "mistral-small-latest", messages: [{role: "user", content: "Write a concise status update."}]})
```
OCR from a document URL:

```ruby
result = app.integrations.mistral.ocr(body: {model: "mistral-ocr-latest", document: {type: "document_url", document_url: "https://example.test/sample.pdf"}})
```
Upload a batch JSONL file:

```ruby
uploaded = app.integrations.mistral.upload_file(file_path: "/tmp/batch.jsonl", body: {purpose: "batch"})
```
Create a batch job:

```ruby
job = app.integrations.mistral.create_batch_job(body: {input_files: ["file-id"], endpoint: "/v1/chat/completions", model: "mistral-small-latest"})
```
Upload a library document:

```ruby
document = app.integrations.mistral.upload_library_document(library_id: "library-id", file_path: "/tmp/knowledge.pdf", body: {name: "Knowledge PDF"})
```
## Coverage notes
