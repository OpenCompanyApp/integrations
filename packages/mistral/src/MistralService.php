...
/**
 * HTTP client for the Mistral AI REST API.
 *
 * Handles bearer authentication, JSON and multipart dispatch, binary-safe
 * response parsing, API error logging, and configurable API base URLs.
 */
class MistralService
{
    /**
     * @param  string  $apiKey  Mistral API key.
     * @param  string  $baseUrl  Mistral API base URL.
     */
    ...
    /**
     * Execute a Mistral REST operation.
     *
    ...
    /**
     * Perform a raw Mistral HTTP request.
     *
     * @param  array<string, mixed>  $query  Query string parameters.
     * @param  array<string, mixed>  $body  JSON body or multipart fields.
     */
        ...
        try {
            ...
            if ($filePath !== null) {
                ...
                $response = match ($method) {
                    ...
                    default => throw new RuntimeException("Multipart uploads only support POST or PUT, got {$method}."),
                };
            } else {
            ...
            if (!$response->successful()) {
                ...
                Log::error("Mistral API error: {$method} {$path}", [
                    'status' => $response->status(),
                    'error' => $error,
                ]);

                throw new RuntimeException("Mistral API error ({$response->status()}): " . (is_string($error) ? $error : json_encode($error)));
            }

            return $response;
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            Log::error("Mistral API connection error: {$method} {$path}", [
                'error' => $e->getMessage(),
            ]);

            throw new RuntimeException("Failed to connect to Mistral API: {$e->getMessage()}");
        }
    }
