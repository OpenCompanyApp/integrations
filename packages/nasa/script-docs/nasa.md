...
```ruby
apod = app.integrations.nasa.get_apod(date: "2026-01-15", thumbs: true)
puts(apod.title)
puts(apod.url)
```
## Mars Rover Photos
...
```ruby
photos = app.integrations.nasa.mars_rover_photos(rover: "curiosity", sol: 1000, camera: "MAST")
photos.photos.each do |photo|
  puts((photo.earth_date).to_s + " " + (photo.img_src).to_s)
end
```
## Asteroids
...
```ruby
feed = app.integrations.nasa.asteroid_feed(start_date: "2026-01-01", end_date: "2026-01-07")
puts(feed.total_asteroids)
```
## DONKI
...
```ruby
flares = app.integrations.nasa.donki_events(type: "FLR", start_date: "2026-01-01", end_date: "2026-01-07")
```
## EPIC
...
```ruby
latest = app.integrations.nasa.epic_images(collection: "natural")
dates = app.integrations.nasa.epic_images(collection: "enhanced", all_dates: true)
```
## Earth
...
```ruby
assets = app.integrations.nasa.earth_assets(lon: (-122.4194), lat: 37.7749, date: "2026-01-01")
```
## Image Library
...
```ruby
results = app.integrations.nasa.search_images(q: "apollo 11", media_type: "image")
first = results.items[0]
asset = app.integrations.nasa.image_asset(nasa_id: first.nasa_id)
```
## EONET
...
```ruby
events = app.integrations.nasa.eonet_events(status: "open", category: "wildfires", limit: 10)
(events.events || []).each do |event|
  puts((event.id).to_s + " " + (event.title).to_s)
end
```