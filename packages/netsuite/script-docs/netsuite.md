# NetSuite ERP — Ruby API Reference
...
```ruby
result = app.integrations.netsuite.list_customers(limit: 10, offset: 0)
result.items.each do |customer|
  puts((customer.id).to_s + ": " + ((customer.companyname || customer.entityid)).to_s)
end
```
---
...
```ruby
result = app.integrations.netsuite.get_customer(id: "12345")
puts("Company: " + (result.companyname).to_s)
...
```ruby
result = app.integrations.netsuite.create_customer(companyname: "Acme Corp", email: "billing@acme.com", phone: "+1-555-0123", currency: "1", terms: "2")
...
```ruby
result = app.integrations.netsuite.list_invoices(limit: 20, offset: 0)
result.items.each do |invoice|
  puts((invoice.tranid).to_s + ": $" + ((invoice.amountremaining || invoice.total)).to_s)
end
```
---
...
```ruby
result = app.integrations.netsuite.list_sales_orders(limit: 20, offset: 0)
result.items.each do |order|
  puts((order.tranid).to_s + ": " + ((order.status || "Unknown")).to_s)
end
```
---
...
```ruby
result = app.integrations.netsuite.list_items(limit: 50, offset: 0)
result.items.each do |item|
  puts((item.itemid).to_s + ": " + (((item.displayname || item.description) || "")).to_s)
end
```
---
...
```ruby
result = app.integrations.netsuite.get_current_user()
