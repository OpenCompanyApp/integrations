...
```ruby
result = app.integrations.nocodb.list_records(table_id: "tbxyz123", where: "(Status,eq,Active)", sort: "[{\"field\":\"Name\",\"direction\":\"asc\"}]", limit: 10)
result.records.each do |record|
  puts((record.Name).to_s + " - " + (record.Status).to_s)
end
```
### Create a record

```ruby
result = app.integrations.nocodb.create_record(table_id: "tbxyz123", data: "{\"Name\":\"John\",\"Email\":\"john@example.com\",\"Status\":\"Active\"}")
puts("Created record ID: " + (result.Id).to_s)
```
### Batch create records

```ruby
result = app.integrations.nocodb.batch_create_records(table_id: "tbxyz123", records: "[{\"Name\":\"Alice\"},{\"Name\":\"Bob\"},{\"Name\":\"Charlie\"}]")
```
---
