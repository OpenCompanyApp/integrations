# Notion — JavaScript API Reference

Notion is a workspace for wikis, docs, and databases. This integration lets you search, create, update, and archive pages; manage databases and query rows; manipulate content blocks; read users; and work with comments — all from JavaScript programs.
...
All tools are called via `app.integrations.notion.<tool_name>({ ... })`. Every function takes a single JavaScript object of named parameters and returns a result table.

```js
var result = app.integrations.notion.search({ query: "meeting notes" })
```
...
var result = app.integrations.notion.search({
  query: "meeting notes",
  filter_type: "page",
  page_size: 5,
})
...
var result = app.integrations.notion.create_page({
...
```js
var result = app.integrations.notion.get_page({
  page_id: "abc123def456",
})

console.log(result.url, result.created_time)
...
var result = app.integrations.notion.update_page({
  page_id: "abc123def456",
  properties: '{"Status": {"select": {"name": "Done"}}}',
})

console.log(result.id, result.last_edited_time)
```
```js
...
```js
var result = app.integrations.notion.archive_page({
  page_id: "abc123def456",
})

console.log(result.id, result.archived)
```
---
...
```js
var result = app.integrations.notion.create_database({
  parent_id: "page123abc",
...
```js
var result = app.integrations.notion.get_database({
...
var result = app.integrations.notion.update_database({
  database_id: "db123abc456",
...
var result = app.integrations.notion.query_database({
  database_id: "db123abc456",
...
var result = app.integrations.notion.query_database({
  database_id: "db123abc456",
  filter: '{"&&": [{"property": "Status", "select": {"equals": "In Progress"}}, {"property": "Priority", "select": {"equals": "High"}}]}',
})
```
...
```js
var result = app.integrations.notion.list_databases({
  query: "projects",
})

for (const db of (result.databases)) {
  console.log(db.id, db.title, db.url)
}
```
---
...
var result = app.integrations.notion.get_block_children({
  block_id: "page123abc",
  page_size: 100,
})

for (const block of (result.results)) {
  console.log(block.id, block.type, block.has_children)
}
...
var result = app.integrations.notion.append_block_children({
  block_id: "page123abc",
...
console.log("Appended " + result.count + " blocks")
```
```js
// Add a to-do list
var result = app.integrations.notion.append_block_children({
...
```js
var result = app.integrations.notion.get_block({
  block_id: "block123abc",
})

console.log(result.id, result.type, result.has_children)
...
var result = app.integrations.notion.update_block({
  block_id: "block123abc",
  type: "paragraph",
  content: '{"rich_text": [{"text": {"content": "Updated paragraph text."}}]}',
})

console.log(result.id, result.type, result.updated)
```
```js
// Check a to-do item
var result = app.integrations.notion.update_block({
...
```js
var result = app.integrations.notion.delete_block({
  block_id: "block123abc",
})

console.log(result.id, result.deleted)
```
---
...
```js
var result = app.integrations.notion.get_current_user({})

console.log(result.name, result.type, result.avatar_url)
...
```js
var result = app.integrations.notion.list_users({
  page_size: 50,
})
...
```js
var result = app.integrations.notion.get_user({
  user_id: "user123abc",
})
...
var result = app.integrations.notion.create_comment({
  parent_id: "page123abc",
  body_text: "Looks good to me! Ready to merge.",
})

console.log(result.id, result.created_time)
```
```js
...
```js
var result = app.integrations.notion.get_comments({
  block_id: "page123abc",
})

for (const comment of (result.comments)) {
  console.log(comment.created_time, comment.text, comment.created_by.id)
}
```
---
...
do {
  ...
  var result = app.integrations.notion.query_database(params)
...
console.log("Total rows: " + all_results.length)
```
...
- **JSON parameters** — Complex structures (`properties`, `filter`, `sorts`, `children`, `content`) are passed as JSON strings. Use single-quoted JavaScript strings to wrap JSON.
...
app.integrations.notion.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.notion.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.notion.work.function_name({ /* parameters */ })
app.integrations.notion.personal.function_name({ /* parameters */ })
```
