...
```js
// List all users
var result = app.integrations.okta.list_users({})

for (const user of (result)) {
  console.log(user.profile.login + " — " + user.profile.firstName + " " + user.profile.lastName + " (" + user.status + ")")
}

// Search for a user
var result = app.integrations.okta.list_users({ q: "john" })
```
---
...
```js
var user = app.integrations.okta.get_user({ id: "00u1a2b3c4d5e6f7g8h9" })
...
```js
var me = app.integrations.okta.get_current_user({})
console.log("Connected as: " + me.profile.login)
```
---
...
```js
var user = app.integrations.okta.create_user({
  profile: {
    firstName: "Jane",
    lastName: "Doe",
...
```js
var user = app.integrations.okta.update_user({
...
```js
// List all groups
var groups = app.integrations.okta.list_groups({})
for (const group of (groups)) {
  console.log(group.id + ": " + group.profile.name)
}

// Search for a group
var groups = app.integrations.okta.list_groups({ q: "Engineering" })
```
---
...
```js
var group = app.integrations.okta.get_group({ id: "00g1a2b3c4d5e6f7g8h9" })
console.log(group.profile.name)
console.log(group.profile.description || "(no description)")
```
---
...
```js
var apps = app.integrations.okta.list_applications({})
for (const app of (apps)) {
  console.log(app.id + ": " + app.label + " (" + app.status + ")")
}
```
---
