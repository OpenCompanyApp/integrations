...
```ruby
# List all users
result = app.integrations.okta.list_users()
result.each do |user|
  puts((user.profile.login).to_s + " — " + (user.profile.firstName).to_s + " " + (user.profile.lastName).to_s + " (" + (user.status).to_s + ")")
end
# Search for a user
result = app.integrations.okta.list_users(q: "john")
```
---
...
```ruby
user = app.integrations.okta.get_user(id: "00u1a2b3c4d5e6f7g8h9")
puts(user.profile.login)
...
```ruby
me = app.integrations.okta.get_current_user()
puts("Connected as: " + (me.profile.login).to_s)
```
---
...
```ruby
user = app.integrations.okta.create_user(profile: {firstName: "Jane", lastName: "Doe", email: "jane.doe@example.com", login: "jane.doe@example.com", title: "Software Engineer", department: "Engineering"}, activate: true)
puts("Created user: " + (user.id).to_s)
```
### With password

```ruby
user = app.integrations.okta.create_user(profile: {firstName: "Jane", lastName: "Doe", email: "jane.doe@example.com", login: "jane.doe@example.com"}, credentials: {password: {value: "TempPass123!"}}, activate: true)
```
---
...
```ruby
user = app.integrations.okta.update_user(id: "00u1a2b3c4d5e6f7g8h9", profile: {title: "Senior Engineer", department: "Platform"})
```
---
...
```ruby
# List all groups
groups = app.integrations.okta.list_groups()
groups.each do |group|
  puts((group.id).to_s + ": " + (group.profile.name).to_s)
end
# Search for a group
groups = app.integrations.okta.list_groups(q: "Engineering")
```
---
...
```ruby
group = app.integrations.okta.get_group(id: "00g1a2b3c4d5e6f7g8h9")
puts(group.profile.name)
puts((group.profile.description || "(no description)"))
```
---
...
```ruby
app.integrations.okta.add_user_group(group_id: "00g1a2b3c4d5e6f7g8h9", user_id: "00u1a2b3c4d5e6f7g8h9")
...
```ruby
apps = app.integrations.okta.list_applications()
apps.each do |app|
  puts((app.id).to_s + ": " + (app.label).to_s + " (" + (app.status).to_s + ")")
end
```
---
