# Microsoft OneDrive JavaScript API Reference
...
```js
var drive = app.integrations["one-drive"].onedrive_get_drive({})

var root = app.integrations["one-drive"].onedrive_list_files({
  top: 50,
})

var children = app.integrations["one-drive"].onedrive_list_children({
  parent_id: "01ABCDEFOLDERID",
  top: 25,
})

var item = app.integrations["one-drive"].onedrive_get_file({
  id: "01ABCDEITEMID",
})
```
...
```js
var folder = app.integrations["one-drive"].onedrive_create_folder({
  name: "Reports",
  conflict_behavior: "rename",
})

var renamed = app.integrations["one-drive"].onedrive_update_item({
  id: "01ABCDEITEMID",
  name: "Q4 report.xlsx",
})

var copy = app.integrations["one-drive"].onedrive_copy_item({
  id: "01ABCDEITEMID",
  name: "Q4 report copy.xlsx",
})

var deleted = app.integrations["one-drive"].onedrive_delete_item({
  id: "01ABCDEITEMID",
})
```
...
```js
var uploaded = app.integrations["one-drive"].onedrive_upload_file({
  path: "Reports/summary.txt",
  content: "Quarterly summary",
  content_type: "text/plain",
})

var file = app.integrations["one-drive"].onedrive_download_file({
  id: uploaded.id,
})
```
...
```js
var matches = app.integrations["one-drive"].onedrive_search({
  query: "quarterly report",
})

var changes = app.integrations["one-drive"].onedrive_delta({})

var thumbs = app.integrations["one-drive"].onedrive_list_thumbnails({
  id: "01ABCDEITEMID",
})
```
...
```js
var link = app.integrations["one-drive"].onedrive_create_sharing_link({
  id: "01ABCDEITEMID",
  type: "view",
  scope: "organization",
})

var permissions = app.integrations["one-drive"].onedrive_list_permissions({
  id: "01ABCDEITEMID",
})
...
```js
var raw = app.integrations["one-drive"].onedrive_api_get({
  path: "/me/drive/recent",
})

var created = app.integrations["one-drive"].onedrive_api_post({
