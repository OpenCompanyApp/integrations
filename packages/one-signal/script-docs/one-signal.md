...
```ruby
message = app.call("integrations.one-signal.create_message", contents: {en: "Your report is ready."}, include_aliases: {external_id: ["user-123"]}, target_channel: "push")
```
```ruby
user = app.call("integrations.one-signal.update_user", alias_label: "external_id", alias_id: "user-123", payload: {properties: {tags: {plan: "pro"}}})
```
```ruby
segment = app.call("integrations.one-signal.create_segment", payload: {name: "Active Pro Users", filters: [{field: "tag", key: "plan", relation: "=", value: "pro"}, {operator: "AND"}, {field: "session_count", relation: ">", value: "5"}]})
```