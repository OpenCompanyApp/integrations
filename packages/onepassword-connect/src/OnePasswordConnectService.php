...
/**
 * HTTP client for the 1Password Connect Server REST API.
 *
 * Handles bearer-token authentication, path and query construction, JSON
 * request dispatch, response parsing, and API error normalization.
 */
class OnePasswordConnectService
{
    /**
     * @param  string  $apiToken  1Password Connect access token.
     * @param  string  $baseUrl  Connect Server API base URL, usually ending in /v1.
     */
    public function __construct(private string $apiToken = '', private string $baseUrl = 'http://localhost:8080/v1') { $this->baseUrl = rtrim($this->baseUrl, '/'); }
    ...
    {
        if(!$this->isConfigured()) throw new RuntimeException('1Password Connect token and base URL are required.');
        try { $method=strtoupper($method); $baseHeaders=['Authorization'=>'Bearer '.$this->apiToken,'Accept'=>'application/json']; if($body!==[]||in_array($method,['POST','PUT','PATCH','DELETE'],true)) $baseHeaders['Content-Type']='application/json'; $http=Http::withHeaders(array_merge($baseHeaders,$headers))->timeout(120); $url=$this->urlWithQuery($this->baseUrl.$path,$query); $response=match($method){'GET'=>$http->get($url),'POST'=>$http->post($url,$body),'PUT'=>$http->put($url,$body),'PATCH'=>$http->patch($url,$body),'DELETE'=>$http->delete($url,$body),default=>throw new RuntimeException("Unsupported HTTP method: {$method}"),}; if(!$response->successful()){ $error=$response->json('message') ?? $response->json('error') ?? $response->body(); Log::error("1Password Connect API error: {$method} {$path}",['status'=>$response->status(),'error'=>$error]); throw new RuntimeException('1Password Connect API error ('.$response->status().'): '.(is_string($error)?$error:json_encode($error))); } return $response; } catch(\Illuminate\Http\Client\ConnectionException $e){ Log::error("1Password Connect API connection error: {$method} {$path}",['error'=>$e->getMessage()]); throw new RuntimeException("Failed to connect to 1Password Connect API: {$e->getMessage()}"); }
    }
