...
```ruby
result = app.integrations.openai.chat_completion(model: "gpt-4o", messages: [{role: "system", content: "You are a helpful assistant."}, {role: "user", content: "Explain quantum computing in one sentence."}], temperature: 0.7, max_tokens: 100)
puts(result.choices[0].message.content)
```
### Generate an image

```ruby
result = app.integrations.openai.create_image(model: "dall-e-3", prompt: "A sunset over a mountain lake in watercolor style", size: "1024x1024", quality: "hd")
```
### Create an embedding

```ruby
result = app.integrations.openai.create_embedding(model: "text-embedding-3-small", input: "The quick brown fox jumps over the lazy dog")
```
---
