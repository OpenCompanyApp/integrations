...
var author = openalex.get_author({
  id: "https://orcid.org/0000-0001-6187-6610",
  select: [ "id", "display_name", "works_count", "cited_by_count" ],
})
```
List tools support `search`, `filter`, `sort`, `per_page`, `page`, `cursor`, `sample`, `seed`, `select`, and `group_by`. `filter` can be a raw OpenAlex filter string or a JavaScript object; tables are converted to OpenAlex comma-separated `field:value` syntax. `select` arrays are sent comma-separated.
...
```js
var suggestions = openalex.autocomplete({
  entity: "authors",
  q: "Ada Lovelace",
})
```
...
```js
var usage = openalex.rate_limit({})

var changefile = openalex.get_changefile({
  date: "2026-01-01",
})
```
## Return Shape
