...
```ruby
works = app.integrations.openalex.list_works(search: "agentic retrieval", filter: {publication_year: 2024, is_oa: true}, sort: "cited_by_count:desc", per_page: 10, select: ["id", "display_name", "doi", "cited_by_count"])
author = app.integrations.openalex.get_author(id: "https://orcid.org/0000-0001-6187-6610", select: ["id", "display_name", "works_count", "cited_by_count"])
```
List tools support `search`, `filter`, `sort`, `per_page`, `page`, `cursor`, `sample`, `seed`, `select`, and `group_by`. `filter` can be a raw OpenAlex filter string or a Ruby object; tables are converted to OpenAlex comma-separated `field:value` syntax. `select` arrays are sent comma-separated.
...
```ruby
suggestions = app.integrations.openalex.autocomplete(entity: "authors", q: "Ada Lovelace")
```
## Utilities
...
```ruby
usage = app.integrations.openalex.rate_limit()
changefile = app.integrations.openalex.get_changefile(date: "2026-01-01")
```
## Return Shape
