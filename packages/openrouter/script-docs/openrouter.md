...
```ruby
result = app.integrations.openrouter.create_completion(model: "openai/gpt-4o", messages: [{role: "user", content: "Summarize this in one sentence."}], max_tokens: 120, temperature: 0.2)
...
```ruby
response = app.integrations.openrouter.create_response(payload: {model: "openai/gpt-4o-mini", input: "Write a release note."})
embedding = app.integrations.openrouter.create_embedding(payload: {model: "openai/text-embedding-3-small", input: "search text"})
```
...
```ruby
generations = app.integrations.openrouter.list_generations(limit: 10)
activity = app.integrations.openrouter.get_activity(query: {limit: 25})
```
## Tool Groups
...
```ruby
zdr = app.integrations.openrouter.api_get(path: "/endpoints/zdr", query: {})
```
