...
```ruby
places = app.integrations.openweather.direct_geocoding(q: "Berlin,DE", limit: 1)
weather = app.integrations.openweather.current(lat: 52.52, lon: 13.41, units: "metric")
```
...
```ruby
daily = app.integrations.openweather.one_call(lat: 52.52, lon: 13.41, exclude: ["current", "minutely", "hourly"], units: "metric")
```
## Air Pollution
...
```ruby
pollution = app.integrations.openweather.air_pollution_history({lat: 52.52, lon: 13.41, start: 1704067200, "end" => 1704153600})
```
## Return Shape
