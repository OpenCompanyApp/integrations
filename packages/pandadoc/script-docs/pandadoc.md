# PandaDoc — JavaScript API Reference
...
```js
var result = app.integrations.pandadoc.list_documents({
  page: 1,
  count: 20,
})
...
```js
var result = app.integrations.pandadoc.get_document({
  id: "abc123-def456-...",
})
...
```js
var result = app.integrations.pandadoc.create_document({
  name: "NDA - Acme Corp",
...
```js
var result = app.integrations.pandadoc.send_document({
  id: "document-uuid-here",
  message: "Please review && sign this document at your earliest convenience.",
})
...
```js
var result = app.integrations.pandadoc.list_templates({
  page: 1,
})

for (const tmpl of (result.results)) {
...
```js
var result = app.integrations.pandadoc.get_template({
...
```js
var result = app.integrations.pandadoc.download_document({
  id: "document-uuid-here",
})
...
```js
var result = app.integrations.pandadoc.create_link({
  id: "document-uuid-here",
  lifetime: 7200,
})

console.log("Sharing link: " + result.session_url)
```
---
...
```js
var result = app.integrations.pandadoc.get_current_user({})
...
app.integrations.pandadoc.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.pandadoc.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.pandadoc.sales.function_name({ /* parameters */ })
app.integrations.pandadoc.legal.function_name({ /* parameters */ })
```
...
```js
// 1. Find a template
var templates = app.integrations.pandadoc.list_templates({})
var template_id = templates.results[0].id

// 2. Create a document from the template
var doc = app.integrations.pandadoc.create_document({
...
app.integrations.pandadoc.send_document({
  id: doc.id,
  message: "Please review && sign at your convenience.",
})

// 4. Share a link for viewing
var link = app.integrations.pandadoc.create_link({
  id: doc.id,
  lifetime: 86400,
})
console.log("View link: " + link.session_url)
```