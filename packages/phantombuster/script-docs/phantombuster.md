# Phantombuster Ruby API Reference
...
```ruby
agents = app.integrations.phantombuster.list_agents(with_argument: true)
launch = app.integrations.phantombuster.launch_agent(id: agents.agents[0].id, bonus_argument: {profileUrl: "https://example.test/profile"})
puts((launch.containerId || launch.id))
```
## Containers
