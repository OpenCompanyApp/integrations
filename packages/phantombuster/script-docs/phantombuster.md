# Phantombuster JavaScript API Reference
...
```js
var agents = app.integrations.phantombuster.list_agents({
  with_argument: true,
})

var launch = app.integrations.phantombuster.launch_agent({
  id: agents.agents[0].id,
  bonus_argument: { profileUrl: "https://example.test/profile" },
})

console.log(launch.containerId || launch.id)
```
## Containers
