...
```ruby
app.integrations.pinboard.add_bookmark(url: "https://example.test/article", description: "Example Article", payload: {extended: "Short note", tags: "research examples", replace: "yes", shared: "no", toread: "yes"})
```
Read bookmarks:

```ruby
app.integrations.pinboard.recent_posts(payload: {tag: "research", count: 25})
app.integrations.pinboard.all_posts(payload: {tag: "research", start: 0, results: 100, meta: "yes"})
```
...
app.integrations.pinboard.rename_tag(old: "old-tag", new: "new-tag")
app.integrations.pinboard.delete_tag(tag: "obsolete")
```
## User And Notes
...
app.integrations.pinboard.get_note(note_id: "note:2a8a2ec9f3a076a1")
```
## Raw GET
...
```ruby
app.integrations.pinboard.api_get(path: "/posts/recent", payload: {tag: "research", count: 10})
```