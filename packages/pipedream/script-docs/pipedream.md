# Pipedream — JavaScript API Reference
...
```js
var result = app.integrations.pipedream.list_workflows({
  page: 1,
  limit: 10,
})
...
```js
var result = app.integrations.pipedream.get_workflow({
  id: "abc_123",
})
...
var result = app.integrations.pipedream.list_components({
  type: "action",
  limit: 20,
})

for (const comp of (result.data)) {
  console.log(comp.key + " (" + comp.app + ")")
}
```
---
...
```js
var result = app.integrations.pipedream.get_component({
  app: "slack",
  id: "send-message",
})
...
```js
var result = app.integrations.pipedream.list_connected_accounts({
  page: 1,
  limit: 10,
})

for (const acct of (result.data)) {
  console.log(acct.id + ": " + acct.name + " (" + acct.app + ")")
}
```
---
...
```js
var result = app.integrations.pipedream.list_triggers({
  workflow_id: "abc_123",
})

for (const trigger of (result.data)) {
  console.log(trigger.type + ": " + (trigger.name || "unnamed"))
}
```
---
...
```js
var result = app.integrations.pipedream.get_current_user({})
