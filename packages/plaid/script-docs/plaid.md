...
```ruby
result = app.integrations.plaid.plaid_transactions_get(body: {access_token: "access-sandbox-example", start_date: "2026-01-01", end_date: "2026-01-31"})
```
For path-based FDX endpoints, pass path parameters as top-level arguments:

```ruby
result = app.integrations.plaid.get_recipient(recipient_id: "recipient-example")
```
