# Plausible Analytics — Ruby API Reference
...
```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors", "pageviews"], date_range: "30d", dimensions: ["event:page"], order_by: "[[\"visitors\",\"desc\"]]", limit: 20)
result.rows.each do |row|
  puts((row["event:page"]).to_s + ": " + (row.visitors).to_s + " visitors")
end
```
### Traffic by country (custom date range)

```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors", "visits", "bounce_rate"], date_range: "custom", date_from: "2026-01-01", date_to: "2026-01-31", dimensions: ["visit:country"], order_by: "[[\"visitors\",\"desc\"]]", limit: 10)
result.rows.each do |row|
  puts((row["visit:country"]).to_s + ": " + (row.visitors).to_s + " visitors, " + (row.bounce_rate).to_s + "% bounce")
end
```
### Filter to specific country

```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors", "pageviews"], date_range: "7d", dimensions: ["event:page"], filters: "[[\"is\",\"visit:country\",[\"US\"]]]")
```
### Filter pages containing /blog

```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors", "pageviews"], date_range: "30d", dimensions: ["event:page"], filters: "[[\"contains\",\"event:page\",[\"/blog\"]]]", order_by: "[[\"pageviews\",\"desc\"]]")
```
### Daily timeseries

```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors"], date_range: "30d", dimensions: ["time:day"])
```
### Aggregate totals (no dimensions)

```ruby
result = app.integrations.plausible.query_stats(site_id: "example.com", metrics: ["visitors", "pageviews", "bounce_rate", "visit_duration"], date_range: "30d")
```
---
