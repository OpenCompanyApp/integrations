...
```ruby
found = app.integrations.pubmed.search(term: "(\"large language model\"[Title/Abstract]) AND medicine", retmax: 10, sort: "pub_date", usehistory: "y")
```
...
```ruby
summaries = app.integrations.pubmed.summary(id: ["40654110", "40654099"])
```
Or use History server keys from ESearch or EPost:

```ruby
abstracts = app.integrations.pubmed.fetch(query_key: found.esearchresult.querykey, web_env: found.esearchresult.webenv, retstart: 0, retmax: 20, rettype: "abstract", retmode: "xml")
```
...
```ruby
links = app.integrations.pubmed.link(id: "40654110", db: "pmc", cmd: "neighbor")
```
## Database Info
...
```ruby
info = app.integrations.pubmed.info(db: "pubmed", version: "2.0")
```
## History Server
...
```ruby
posted = app.integrations.pubmed.post_ids(id: ["40654110", "40654099"])
```
## Spelling And Global Counts

```ruby
spelling = app.integrations.pubmed.spell(term: "asthmaa")
counts = app.integrations.pubmed.global_query(term: "CRISPR")
```
...
```ruby
matches = app.integrations.pubmed.citation_match(citations: ["proc natl acad sci u s a|1991|88|3248|mann bj|example-1|"])
```
