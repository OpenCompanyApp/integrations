 ...
 *
 * Maps to the official Pulumi Cloud endpoint get /api/preview/environments/{orgName}/{envName}/versions/{version}/referrers.
 */
class PulumiEnvironmentsListEnvironmentReferrersPreviewEnvironmentsVersions extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_environments_list_environment_referrers_preview_environments_versions';
    protected const DESCRIPTION = 'ListEnvironmentReferrers

Official Pulumi Cloud endpoint: GET /api/preview/environments/{orgName}/{envName}/versions/{version}/referrers
...
Returns a paginated list of entities that reference a Pulumi ESC environment, including other environments that import it and Pulumi stacks that use it in their configuration. The count parameter limits results (range 1-500). Set allRevisions to true to include references across all revisions, and latestStackVersionOnly to true to return only the latest stack version for each referring stack. Use continuationToken for pagination.';
  ...
  array (
    ...
    protected const PATH = '/api/preview/environments/{orgName}/{envName}/versions/{version}/referrers';
