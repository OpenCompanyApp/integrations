 ...
 * CancelUpdate.
 *
 * Maps to the official Pulumi Cloud endpoint post /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/cancel.
 */
class PulumiStacksCancelUpdateDestroy extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_cancel_update_destroy';
    protected const DESCRIPTION = 'CancelUpdate

Official Pulumi Cloud endpoint: POST /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/cancel
...
Requests cancellation of a service-managed update that is currently in progress. The update must have been started (returns 409 if it has not). Returns 404 if the specified update does not exist. The cancellation is processed asynchronously and the update will transition to a cancelled state.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/cancel';
