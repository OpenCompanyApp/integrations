 ...
 * CompleteUpdate.
 *
 * Maps to the official Pulumi Cloud endpoint post /api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/complete.
 */
class PulumiStacksCompleteUpdateUpdate extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_complete_update_update';
    protected const DESCRIPTION = 'CompleteUpdate

Official Pulumi Cloud endpoint: POST /api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/complete
...
Marks a service-managed update as complete. The request body must include the final status of the update. Returns 400 if the status is unrecognized. Returns 409 if the update has not started, has been cancelled, has already timed out, or has already completed. Requires update token authentication.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/complete';
