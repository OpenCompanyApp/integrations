 ...
 *
 * Maps to the official Pulumi Cloud endpoint patch /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/journalentries.
 */
class PulumiStacksCreateJournalEntriesDestroy extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_create_journal_entries_destroy';
    protected const DESCRIPTION = 'CreateJournalEntries

Official Pulumi Cloud endpoint: PATCH /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/journalentries
...
Creates new journal entries for the specified update. Journal entries record the progression of resource operations during an update, tracking state transitions for each resource. The include_non_activated query parameter controls whether non-activated events are included. Requires update token authentication.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/journalentries';
