 ...
 *
 * Maps to the official Pulumi Cloud endpoint post /api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/events/batch.
 */
class PulumiStacksRecordEngineEventBatchUpdate extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_record_engine_event_batch_update';
    protected const DESCRIPTION = 'RecordEngineEventBatch

Official Pulumi Cloud endpoint: POST /api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/events/batch
...
Records a batch of engine events sent from the Pulumi CLI during a stack update. Engine events represent individual resource operations (create, update, delete, etc.) and diagnostic messages. Batching events reduces the number of API calls during an update. Returns 400 if no events are provided in the batch, or 404 if the update does not exist. Requires update token authentication.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/update/{updateID}/events/batch';
