 ...
 * RecordEngineEvent.
 *
 * Maps to the official Pulumi Cloud endpoint post /api/stacks/{orgName}/{projectName}/{stackName}/refresh/{updateID}/events.
 */
class PulumiStacksRecordEngineEventRefresh extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_record_engine_event_refresh';
    protected const DESCRIPTION = 'RecordEngineEvent

Official Pulumi Cloud endpoint: POST /api/stacks/{orgName}/{projectName}/{stackName}/refresh/{updateID}/events
...
Records a single engine event sent from the Pulumi CLI during a stack update. Engine events represent individual resource operations or diagnostic messages. For better performance, consider using RecordEngineEventBatch to send multiple events in a single request. Returns 400 if no event data is provided, or 404 if the update does not exist. Requires update token authentication.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/refresh/{updateID}/events';
