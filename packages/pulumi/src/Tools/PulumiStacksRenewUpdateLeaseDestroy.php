 ...
 * RenewUpdateLease.
 *
 * Maps to the official Pulumi Cloud endpoint post /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/renew_lease.
 */
class PulumiStacksRenewUpdateLeaseDestroy extends AbstractPulumiTool
{
    protected const NAME = 'pulumi_stacks_renew_update_lease_destroy';
    protected const DESCRIPTION = 'RenewUpdateLease

Official Pulumi Cloud endpoint: POST /api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/renew_lease
...
Renews the lease for a service-managed update that is currently in progress. Leases prevent concurrent operations on the same stack and must be periodically renewed to indicate the update is still active. The renewal duration must be between 0 and 300 seconds. Returns 409 if the update is not currently in progress.';
  ...
  array (
    ...
    protected const PATH = '/api/stacks/{orgName}/{projectName}/{stackName}/destroy/{updateID}/renew_lease';
