# Quickbase JavaScript Reference
...
```js
var apps = app.integrations.quickbase.list_apps({
  params: { name: "Operations" },
})

var app_meta = app.integrations.quickbase.get_app({ appId: "bqxxx" })
```
...
```js
var tables = app.integrations.quickbase.list_tables({ appId: "bqxxx" })
var table = app.integrations.quickbase.get_table({ tableId: "brxxx" })

var fields = app.integrations.quickbase.list_fields({
  tableId: "brxxx",
  params: { includeFieldPerms: true },
})

var field = app.integrations.quickbase.get_field({
  tableId: "brxxx",
  fieldId: 6,
})
```
...
```js
var records = app.integrations.quickbase.list_records({
  tableId: "brxxx",
  where: "{6.EX.'Open'}",
  select: [3, 6, 7],
...
var created = app.integrations.quickbase.create_record({
  tableId: "brxxx",
  fields: [
...
var upserted = app.integrations.quickbase.upsert_records({
  tableId: "brxxx",
  data: [
    { [5]: { value: "Open" } }
  ],
...
```js
var reports = app.integrations.quickbase.list_reports({ tableId: "brxxx" })
var report = app.integrations.quickbase.get_report({
  tableId: "brxxx",
  reportId: "7",
})
var rows = app.integrations.quickbase.run_report({
  tableId: "brxxx",
  reportId: "7",
  body: { skip: 0, top: 100 },
})
...
```js
var user = app.integrations.quickbase.get_current_user({})

var raw = app.integrations.quickbase.api_get({
  path: "/fields",
