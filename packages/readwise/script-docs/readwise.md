...
```ruby
app.integrations.readwise.save_reader_document(url: "https://example.test/article", payload: {tags: ["research", "later"], saved_using: "agent"})
```
```ruby
app.integrations.readwise.list_reader_documents(payload: {location: "archive", updatedAfter: "2026-05-01T00:00:00Z", limit: 100})
```
```ruby
app.integrations.readwise.export_highlights(payload: {updatedAfter: "2026-05-01T00:00:00Z", pageCursor: "cursor-from-previous-response"})
```
