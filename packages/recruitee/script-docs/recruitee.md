# Recruitee JavaScript API Reference
...
```js
var offers = app.integrations.recruitee.list_offers({
  status: "published",
  limit: 25,
})

var offer = app.integrations.recruitee.get_offer({ id: 12345 })

var created = app.integrations.recruitee.create_offer({
  offer: {
...
var updated = app.integrations.recruitee.update_offer({
  id: 12345,
  offer: {
    status: "published",
  }
...
```js
var candidates = app.integrations.recruitee.list_candidates({
  limit: 25,
})

var search = app.integrations.recruitee.search_candidates({
  limit: 50,
  page: 1,
  sort_by: "created_at_desc",
})

var candidate = app.integrations.recruitee.get_candidate({ id: 67890 })

var created = app.integrations.recruitee.create_candidate({
  candidate: {
...
```js
var departments = app.integrations.recruitee.list_departments()

var locations = app.integrations.recruitee.list_locations({
  scope: "active",
  view_mode: "brief",
  limit: 10,
})
...
```js
var uploaded = app.integrations.recruitee.upload_attachment({
  attachment: {
...
```js
var result = app.integrations.recruitee.api_get({
  path: "/locations",
  params: { limit: 10 },
})

var patched = app.integrations.recruitee.api_patch({
  path: "/offers/12345",
  body: {
    offer: {
