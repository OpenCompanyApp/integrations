...
```ruby
services = app.integrations.render.list_services(limit: 20)
```
Create a deploy:

```ruby
deploy = app.integrations.render.trigger_deploy(service_id: "srv-example", body: {clearCache: "do_not_clear"})
```
Cancel a deploy:

```ruby
app.integrations.render.cancel_deploy(service_id: "srv-example", deploy_id: "dep-example")
```
Read service environment variables:

```ruby
env = app.integrations.render.list_environment_variables(service_id: "srv-example", limit: 50)
```
Create a Postgres export:

```ruby
exportResult = app.integrations.render.create_postgres_export(postgres_id: "dpg-example", body: {format: "custom"})
```
Query CPU metrics:

```ruby
cpu = app.integrations.render.get_cpu_usage(resource: "srv-example", start_time: "2026-01-01T00:00:00Z", end_time: "2026-01-01T01:00:00Z")
```
## Tool Families
