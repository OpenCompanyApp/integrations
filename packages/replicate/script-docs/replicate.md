...
```ruby
account = app.integrations.replicate.get_authenticated_account()
```
Create a prediction by version:

```ruby
prediction = app.integrations.replicate.create_prediction(version: "replicate-version-id", input: {prompt: "a quiet forest trail"})
```
Create a prediction using an official model:

```ruby
prediction = app.integrations.replicate.create_prediction_using_official_model(model_owner: "black-forest-labs", model_name: "flux-schnell", input: {prompt: "a quiet forest trail"})
```
Create a prediction using a deployment:

```ruby
prediction = app.integrations.replicate.create_prediction_using_deployment(deployment_owner: "example-org", deployment_name: "image-worker", input: {prompt: "a quiet forest trail"})
```
Cancel a prediction:

```ruby
app.integrations.replicate.cancel_prediction(prediction_id: "prediction-id")
```
Search public models:

```ruby
result = app.integrations.replicate.search_public_models(body: "image generation")
```
Upload a file:

```ruby
file = app.integrations.replicate.create_file(body: {content: "/tmp/example.png", filename: "example.png", type: "image/png", metadata: {source: "agent"}})
```
## Tool Families
