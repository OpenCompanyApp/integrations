...
/**
 * Shared executor for Rootly endpoint-specific tools.
 *
 ...
 */
abstract class AbstractRootlyTool implements Tool
{
    ...
    /** @param  RootlyService  $service  Rootly API client. */ public function __construct(protected RootlyService $service) {}
    ...
    /** @param  array<string, mixed>  $args  Tool arguments for the mapped endpoint. */ public function execute(array $args): ToolResult { try { if(!$this->service->isConfigured()) return ToolResult::error('Rootly integration is not configured.'); return ToolResult::success($this->service->request(static::METHOD, static::PATH, $this->mapped($args, static::PATH_PARAMS, true), $this->mapped($args, static::QUERY_PARAMS), $this->mapped($args, static::HEADER_PARAMS), $this->body($args))); } catch(\Throwable $e) { return ToolResult::error($e->getMessage()); } }
