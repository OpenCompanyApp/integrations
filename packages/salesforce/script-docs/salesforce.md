...
```ruby
result = app.integrations.salesforce.create_lead(first_name: "Jane", last_name: "Smith", company: "Acme Corp", email: "jane@acme.com", phone: "+1-555-0123", title: "VP of Engineering")
puts("Created lead: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.get_lead(id: "00Q5f00001ABCdEFGH")
puts("Lead: " + (result.FirstName).to_s + " " + (result.LastName).to_s)
...
```ruby
result = app.integrations.salesforce.update_lead(id: "00Q5f00001ABCdEFGH", phone: "+1-555-9999", email: "jane.smith@acme.com")
puts("Updated: " + ((result.updated).to_s).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.create_contact(first_name: "John", last_name: "Doe", email: "john@example.com", phone: "+1-555-0456", account_id: "0015f00001ABCdEFGH")
puts("Created contact: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.get_contact(id: "0035f00001ABCdEFGH")
puts("Contact: " + (result.FirstName).to_s + " " + (result.LastName).to_s)
...
```ruby
result = app.integrations.salesforce.create_account(name: "Acme Corp", website: "https://acme.com", phone: "+1-555-0789", industry: "Technology", billing_city: "San Francisco", billing_country: "US")
puts("Created account: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.get_account(id: "0015f00001ABCdEFGH")
puts("Account: " + (result.Name).to_s)
puts("Industry: " + ((result.Industry || "N/A")).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.update_account(id: "0015f00001ABCdEFGH", phone: "+1-555-1111", website: "https://acme-new.com")
puts("Updated: " + ((result.updated).to_s).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.create_opportunity(name: "Acme Corp - New Deal", amount: 50000, stage_name: "Prospecting", close_date: "2026-06-30", account_id: "0015f00001ABCdEFGH", probability: 25)
puts("Created opportunity: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.get_opportunity(id: "0065f00001ABCdEFGH")
puts("Opportunity: " + (result.Name).to_s)
puts("Amount: " + ((result.Amount || "N/A")).to_s)
puts("Stage: " + (result.StageName).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.soql_query(soql: "SELECT Id, Name, Email FROM Contact WHERE LastName = 'Smith' LIMIT 10")
puts("Total records: " + (result.totalSize).to_s)
result.records.each do |record|
  puts((record.Name).to_s + " - " + ((record.Email || "N/A")).to_s)
end
```
---
...
```ruby
result = app.integrations.salesforce.sosl_search(sosl: "FIND {Acme} IN ALL FIELDS RETURNING Account(Id, Name), Contact(Id, Name)")
(result.searchRecords || []).each do |record|
  puts((record.attributes.type).to_s + ": " + (record.Name).to_s)
end
```
---
...
```ruby
result = app.integrations.salesforce.describe_object(object_type: "Account")
...
```ruby
result = app.integrations.salesforce.list_objects()
(result.sobjects || []).each do |obj|
  puts((obj.name).to_s + " — " + (obj.label).to_s)
end
```
---
...
```ruby
result = app.integrations.salesforce.create_task(subject: "Follow up with client", description: "Discuss Q2 renewal terms", status: "Not Started", priority: "High", who_id: "0035f00001ABCdEFGH", what_id: "0015f00001ABCdEFGH", activity_date: "2026-04-15")
puts("Created task: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.create_case(subject: "Login issue reported", description: "Customer cannot log in to the portal since Monday", status: "New", priority: "High", origin: "Web", contact_id: "0035f00001ABCdEFGH", account_id: "0015f00001ABCdEFGH")
puts("Created case: " + (result.id).to_s)
```
---
...
```ruby
result = app.integrations.salesforce.list_recent(limit: 10)
result.each do |item|
  puts((item.attributes.type).to_s + ": " + ((item.Name || item.Id)).to_s)
end
```
---
...
```ruby
result = app.integrations.salesforce.get_user(id: "0055f00001ABCdEFGH")
