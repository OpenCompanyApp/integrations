...
```js
var vehicles = app.integrations.samsara.samsara_list_vehicles({
  limit: 100,
})

var stats = app.integrations.samsara.samsara_get_vehicle_stats({
  types: [ "gps", "obdOdometerMeters" ],
  vehicleIds: [ "vehicle-id" ],
})

var route = app.integrations.samsara.samsara_create_route({
  payload: {
...
var documents = app.integrations.samsara.samsara_list_documents({
...
```js
var users = app.integrations.samsara.samsara_api_get({
  path: "/users",
  limit: 50,
})
```
