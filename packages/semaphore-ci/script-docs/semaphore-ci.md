...
```ruby
run = app.call("integrations.semaphore-ci.run_workflow", payload: {project_id: "project-uuid", reference: "refs/heads/main", pipeline_file: ".semaphore/semaphore.yml", parameters: {DEPLOY_ENV: "staging"}})
```
List pipelines:

```ruby
pipelines = app.call("integrations.semaphore-ci.list_pipelines", project_id: "project-uuid", branch_name: "main")
```
Stop a pipeline:

```ruby
app.call("integrations.semaphore-ci.stop_pipeline", pipeline_id: "pipeline-uuid")
```
## Jobs And Logs

```ruby
job = app.call("integrations.semaphore-ci.get_job", job_id: "job-uuid")
logs = app.call("integrations.semaphore-ci.get_job_logs", job_id: "job-uuid", artifact_job_logs: true)
```
...
```ruby
target = app.call("integrations.semaphore-ci.create_deployment_target", project_id: "project-uuid", payload: {name: "staging", project_id: "project-uuid", unique_token: "idempotency-uuid", description: "Staging deployment target"})
```
Delete uses a required `unique_token` query parameter:

```ruby
app.call("integrations.semaphore-ci.delete_deployment_target", target_id: "target-uuid", unique_token: "idempotency-uuid")
```
## Artifacts
...
```ruby
artifacts = app.call("integrations.semaphore-ci.list_artifacts", scope: "jobs", scope_id: "job-uuid", path: "agent")
```
...
```ruby
result = app.call("integrations.semaphore-ci.api_get", path: "/pipelines", query: {project_id: "project-uuid"})
```
