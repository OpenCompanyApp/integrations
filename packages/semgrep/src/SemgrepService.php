...
/**
 * HTTP client for the Semgrep Web API.
 *
 * Handles bearer token authentication, path and query construction, JSON request
 * dispatch, response parsing, and Semgrep API error normalization.
 */
class SemgrepService
{
    /**
     * @param  string  $apiToken  Semgrep API token with Web API access enabled.
     * @param  string  $baseUrl  Semgrep API base URL.
     */
    public function __construct(private string $apiToken = '', private string $baseUrl = 'https://semgrep.dev') { $this->baseUrl = rtrim($this->baseUrl, '/'); }
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Semgrep API token is not configured.');
        try { $method = strtoupper($method); $url = $this->urlWithQuery($this->baseUrl.$path, $query); $http = Http::withHeaders(array_merge(['Authorization'=>'Bearer '.$this->apiToken,'Content-Type'=>'application/json','Accept'=>'application/json'], $headers))->timeout(120); $response = match ($method) { 'GET'=>$http->get($url), 'POST'=>$http->post($url,$body), 'PUT'=>$http->put($url,$body), 'PATCH'=>$http->patch($url,$body), 'DELETE'=>$http->delete($url,$body), default=>throw new RuntimeException("Unsupported HTTP method: {$method}"), }; if (!$response->successful()) { $error = $response->json('message') ?? $response->json('error') ?? $response->json('errors') ?? $response->body(); Log::error("Semgrep API error: {$method} {$path}", ['status'=>$response->status(),'error'=>$error]); throw new RuntimeException('Semgrep API error ('.$response->status().'): '.(is_string($error)?$error:json_encode($error))); } return $response; } catch (\Illuminate\Http\Client\ConnectionException $e) { Log::error("Semgrep API connection error: {$method} {$path}", ['error'=>$e->getMessage()]); throw new RuntimeException("Failed to connect to Semgrep API: {$e->getMessage()}"); }
    }
