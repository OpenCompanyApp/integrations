...
/**
 * Shared executor for Semgrep endpoint-specific tools.
 *
 * Child classes map to official Web API OpenAPI operations while this base
 * handles configured-state checks, path/query/header/body shaping, and errors.
 */
abstract class AbstractSemgrepTool implements Tool
{
    ...
    /** @param  SemgrepService  $service  Semgrep Web API client. */ public function __construct(protected SemgrepService $service) {}
    ...
    /** @param  array<string, mixed>  $args  Tool arguments for the mapped endpoint. */ public function execute(array $args): ToolResult { try { if(!$this->service->isConfigured()) return ToolResult::error('Semgrep integration is not configured.'); return ToolResult::success($this->service->request(static::METHOD, static::PATH, $this->mapped($args, static::PATH_PARAMS, true), $this->mapped($args, static::QUERY_PARAMS), $this->mapped($args, static::HEADER_PARAMS), $this->body($args))); } catch(\Throwable $e) { return ToolResult::error($e->getMessage()); } }
