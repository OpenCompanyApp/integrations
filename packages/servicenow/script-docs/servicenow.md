...
result = app.integrations.servicenow.list_incidents(query: "active=true")
# High-priority incidents
result = app.integrations.servicenow.list_incidents(query: "priority=1^ORpriority=2", limit: 50)
```
---
...
```ruby
result = app.integrations.servicenow.get_incident(id: "a9e4e1e14f3c41200bae4b8d0210c7d3")
```
---
...
```ruby
result = app.integrations.servicenow.create_incident(short_description: "Email server !responding", description: "The mail server at mail.example.com has been unreachable since 09:00 UTC.", priority: "1")
puts("Created incident: " + (result.result.number).to_s)
```
---
...
```ruby
result = app.integrations.servicenow.update_incident(id: "a9e4e1e14f3c41200bae4b8d0210c7d3", fields: {state: "6", work_notes: "Root cause identified: DNS misconfiguration. Fix applied."})
```
---
...
```ruby
result = app.integrations.servicenow.list_tasks(query: "active=true^assigned_to=javascript:gs.getUserID()", limit: 10)
```
---
...
```ruby
result = app.integrations.servicenow.get_task(id: "b5e2f1e14f3c41200bae4b8d0210c8e4")
```
---
...
```ruby
result = app.integrations.servicenow.create_task(short_description: "Provision new laptop for onboarding employee", description: "Order && configure a MacBook Pro for Jane Smith, starting March 1.", priority: "3")
```
---
...
result = app.integrations.servicenow.list_users(query: "active=true^department=IT", limit: 50)
result.result.each do |user|
...
```ruby
result = app.integrations.servicenow.get_user(id: "6816f79cc0a8016401c5a33be04be441")
...
```ruby
result = app.integrations.servicenow.get_current_user()
puts("Logged in as: " + (result.result.name).to_s)
```
---
