...
/**
 * Shared executor for ShipEngine endpoint-specific tools.
 *
 * Handles configured-state checks, path/query/header mapping, body validation,
 * service dispatch, and error conversion.
 */
abstract class AbstractShipEngineTool implements Tool
{
    ...
    /**
     * @param  ShipEngineService  $service  ShipEngine API client.
     */
    public function __construct(protected ShipEngineService $service) {}
    ...
    /**
     * Execute the mapped ShipEngine API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('ShipEngine API key is required.');
            }
    ...
    private function body(array $args): array
    {
        ...
        if (static::BODY_REQUIRED && (!is_array($body) || $body === [])) {
            throw new InvalidArgumentException('body must be a non-empty object matching the ShipEngine request schema.');
        }
