...
```ruby
products = app.integrations.shopify.list_products(limit: 25, status: "active", query: {fields: "id,title,status"})
product = app.integrations.shopify.create_product(payload: {product: {title: "Example Widget", status: "draft"}})
orders = app.integrations.shopify.list_orders(status: "open", financial_status: "paid", limit: 10)
refund_preview = app.integrations.shopify.calculate_order_refund(order_id: "1234567890", payload: {refund: {shipping: {full_refund: true}}})
```
## Coverage Notes
