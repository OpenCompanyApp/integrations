...
/**
 * Base class for focused Shopify Admin REST endpoint tools.
 *
 * Handles configuration checks, required arguments, path interpolation, and
 * query/body shaping for one Shopify REST operation.
 */
abstract class AbstractShopifyRestTool implements Tool
{
    ...
    /**
     * @param  ShopifyService  $service  The Shopify Admin REST API client.
     */
    public function __construct(
        protected ShopifyService $service,
    ) {}
    ...
    /**
     * Execute one Shopify Admin REST operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            if (!$this->service->isConfigured()) {
                return ToolResult::error('Shopify integration is not configured. Access token and shop domain or base URL are required.');
            }
            ...
            $result = match ($this->method) {
                ...
                default => throw new RuntimeException("Unsupported Shopify tool method: {$this->method}"),
            };
