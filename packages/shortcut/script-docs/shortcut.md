...
```ruby
stories = app.integrations.shortcut.search_stories(body: {query: "state:unstarted owner:me"})
story = app.integrations.shortcut.get_story(story_public_id: 123)
created = app.integrations.shortcut.create_story(body: {name: "Draft integration docs", workflow_state_id: 500000001})
```
## Common Tools
...
All examples use fake IDs and safe placeholder values. Real API tokens should be configured through the host credential resolver, not hard-coded in Ruby.
