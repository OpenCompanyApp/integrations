...
```ruby
jobs = app.call("integrations.smartrecruiters.smartrecruiters_jobs_jobs_all", q: "java developer", limit: 10)
candidate = app.call("integrations.smartrecruiters.smartrecruiters_candidates_candidates_get", id: "candidate-123")
```

`smartrecruiters_webhooks_subscriptions_create` is the exact subscription tool. It requires one `body` object; the generated provider intentionally exposes that object as the upstream webhook schema rather than inventing individual field names. Its decoded JSON response is returned unchanged when the service responds with JSON.
