<?php

namespace OpenCompany\Integrations\SmartRecruiters\Tools;

/**
 * Create a new user..
 *
 * Maps to users-api.json endpoint POST /users.
 */
class SmartRecruitersUsersUsersCreate extends AbstractSmartRecruitersTool
{
    protected const NAME = "smartrecruiters_users_users_create";
    protected const DESCRIPTION = "Create a new user.\n\nOfficial SmartRecruiters endpoint: POST /users from users-api.json.";
    protected const PARAMETERS = [
        "body" => [
            "type" => "object",
            "required" => false,
            "description" => "User object to be created",
        ],
    ];
    protected const METHOD = "POST";
    protected const BASE_URL = "https://api.smartrecruiters.com/user-api/v201804";
    protected const PATH = "/users";
    protected const PATH_PARAMS = [];
    protected const QUERY_PARAMS = [];
    protected const HEADER_PARAMS = [];
    protected const QUERY_STYLES = [];
    protected const BODY_REQUIRED = false;
    protected const BODY_MODE = "json";
    protected const AUTH_MODE = "either";
}
