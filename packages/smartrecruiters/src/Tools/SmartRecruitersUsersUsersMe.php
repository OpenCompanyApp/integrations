<?php

namespace OpenCompany\Integrations\SmartRecruiters\Tools;

/**
 * Get details of my user.
 *
 * Maps to users-api.json endpoint GET /users/me.
 */
class SmartRecruitersUsersUsersMe extends AbstractSmartRecruitersTool
{
    protected const NAME = "smartrecruiters_users_users_me";
    protected const DESCRIPTION = "Get details of my user\n\nOfficial SmartRecruiters endpoint: GET /users/me from users-api.json.";
    protected const PARAMETERS = [];
    protected const METHOD = "GET";
    protected const BASE_URL = "https://api.smartrecruiters.com/user-api/v201804";
    protected const PATH = "/users/me";
    protected const PATH_PARAMS = [];
    protected const QUERY_PARAMS = [];
    protected const HEADER_PARAMS = [];
    protected const QUERY_STYLES = [];
    protected const BODY_REQUIRED = false;
    protected const BODY_MODE = "json";
    protected const AUTH_MODE = "either";
}
