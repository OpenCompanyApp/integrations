 ...
 *
 * Maps to the official Snyk endpoint get /tenants/{tenant_id}/inventory/assets.
 */
class SnykListAssetsTenant extends AbstractSnykTool
{
    protected const NAME = 'snyk_list_assets_tenant';
    protected const DESCRIPTION = 'List or search all assets (synchronous) (Early Access)

Official Snyk endpoint: GET /tenants/{tenant_id}/inventory/assets

Retrieves a polymorphic list of all asset types for a given tenant. The response includes different asset types (containers, container images, etc.) in a single unified response following the JSON:API specification. This endpoint supports both simple listing and synchronous search with filtering, sorting, and cursor-based pagination through query parameters. #### Required permissions - `View Tenant Details (tenant.read)`';
