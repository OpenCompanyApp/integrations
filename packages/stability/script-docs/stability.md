# Stability AI Ruby API Reference
...
```ruby
example = {content_type: "image/png", body_base64: "...", finish_reason: "SUCCESS", seed: "123"}
```
## Account
...
```ruby
image = app.integrations.stability.generate_core_image(prompt: "product photo of a matte black desk lamp", aspect_ratio: "1:1", output_format: "png")
```
...
```ruby
result = app.integrations.stability.remove_background(image: "/tmp/product.png", output_format: "png")
```
## Upscaling

```ruby
fast = app.integrations.stability.fast_upscale(image: "/tmp/small.png", output_format: "png")
```
...
```ruby
job = app.integrations.stability.image_video(image: "/tmp/hero.png", motion_bucket_id: 127)
video = app.integrations.stability.get_video_result(id: job.id)
```
