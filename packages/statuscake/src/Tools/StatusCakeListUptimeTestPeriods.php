 ...
 *
 * Maps to the official StatusCake endpoint GET /uptime/{test_id}/periods.
 */
class StatusCakeListUptimeTestPeriods extends AbstractStatusCakeTool
{
    protected const NAME = 'statuscake_list_uptime_test_periods';
    protected const DESCRIPTION = 'Returns a list of uptime check periods for a given id, detailing the creation time of the period, when it ended and the duration. The returned results are a paginated series. Alongside the response data is a `links` object referencing the current response document, `self`, and the next page in the series, `next`.

Official StatusCake endpoint: GET /uptime/{test_id}/periods.';
    ...
    protected const PATH = '/uptime/{test_id}/periods';
