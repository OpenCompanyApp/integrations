# Storyblok — JavaScript API Reference
...
```js
var result = app.integrations.storyblok.list_stories({
  per_page: 25,
})
...
```js
var result = app.integrations.storyblok.get_story({
  id: 12345678,
})

console.log("Name: " + result.story.name)
console.log("Slug: " + result.story.slug)
console.log("Content: " + json.stringify(result.story.content))
```
---
...
```js
var result = app.integrations.storyblok.create_story({
  name: "My New Page",
  slug: "my-new-page",
  content: {
    component: "page",
...
console.log("Created story: " + result.story.id)
```
...
```js
var result = app.integrations.storyblok.update_story({
  id: 12345678,
  content: {
    component: "page",
    title: "Updated Title",
...
console.log("Updated story: " + result.story.name)
```
...
```js
var result = app.integrations.storyblok.delete_story({
  id: 12345678,
})
...
```js
var result = app.integrations.storyblok.list_components({})
...
```js
var result = app.integrations.storyblok.get_current_user({})
...
app.integrations.storyblok.list_stories({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.storyblok.default.list_stories({ /* parameters */ })

// Named accounts
app.integrations.storyblok.production.list_stories({ /* parameters */ })
app.integrations.storyblok.staging.list_stories({ /* parameters */ })
```
