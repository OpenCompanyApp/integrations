# Stripe Connect — JavaScript API Reference
...
```js
var result = app.integrations["stripe-connect"].list_accounts({
    limit: 25,
})
...
```js
var account = app.integrations["stripe-connect"].get_account({
    id: "acct_1234567890",
})
...
```js
// List all payouts
var result = app.integrations["stripe-connect"].list_payouts({
    limit: 25,
})
...
```js
var payout = app.integrations["stripe-connect"].get_payout({
    id: "po_1234567890",
})
...
```js
var result = app.integrations["stripe-connect"].list_balances({
    limit: 25,
})
...
```js
var result = app.integrations["stripe-connect"].list_capabilities({
...
```js
var user = app.integrations["stripe-connect"].get_current_user({})
...
app.integrations["stripe-connect"].function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations["stripe-connect"].default.function_name({ /* parameters */ })

// Named accounts
app.integrations["stripe-connect"].work.function_name({ /* parameters */ })
app.integrations["stripe-connect"].personal.function_name({ /* parameters */ })
```
