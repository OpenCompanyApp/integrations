...
```ruby
projects = app.integrations.supabase.list_projects()
```
Get a project:

```ruby
project = app.integrations.supabase.get_project(project_ref: "abcdefghijklmnopqrst")
```
Create a project:

```ruby
project = app.integrations.supabase.create_project(name: "Example App", db_pass: "use-a-secret-from-your-host", organization_slug: "example-org")
```
List organizations:

```ruby
organizations = app.integrations.supabase.list_organizations()
```
List organization projects:

```ruby
projects = app.integrations.supabase.list_organization_projects(slug: "example-org", limit: 20)
```
Get project API keys:

```ruby
keys = app.integrations.supabase.get_project_api_keys(project_ref: "abcdefghijklmnopqrst")
```
## Tool Families
