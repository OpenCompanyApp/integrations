...
```js
var apps = app.integrations.svix.list_applications({
  limit: 10,
})

var app = app.integrations.svix.create_application({
  get_if_exists: true,
  payload: {
    name: "Example tenant",
    uid: "tenant_123",
...
var endpoint = app.integrations.svix.create_endpoint({
  app_id: app.id,
...
var message = app.integrations.svix.create_message({
  app_id: app.id,
...
```js
var event_types = app.integrations.svix.list_event_types({
  limit: 50,
  include_archived: false,
})

var event_type = app.integrations.svix.create_event_type({
  payload: {
    name: "invoice.paid",
...
```js
var messages = app.integrations.svix.list_messages({
  app_id: "app_123",
  limit: 25,
  with_content: true,
})

var attempts = app.integrations.svix.list_attempts_by_msg({
  app_id: "app_123",
  msg_id: "msg_123",
})

var replay = app.integrations.svix.resend_webhook({
  app_id: "app_123",
  msg_id: "msg_123",
  endpoint_id: "ep_123",
...
```js
var stream = app.integrations.svix.create_stream({
  payload: {
    name: "Audit stream",
    uid: "audit",
  }
...
var sink = app.integrations.svix.create_sink({
  stream_id: stream.id,
  payload: {
    name: "Warehouse",
    type: "webhook",
...
var source = app.integrations.svix.create_ingest_source({
  payload: {
