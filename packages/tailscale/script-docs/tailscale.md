...
```ruby
devices = app.integrations.tailscale.list_tailnet_devices(tailnet: "-", fields: "all")
key = app.integrations.tailscale.create_auth_key_or_trust_credential(tailnet: "-", body: {capabilities: {devices: {create: {reusable: false}}}})
```
## Request Bodies
