...
/**
 * HTTP client for the Tailscale API.
 *
 * Handles API-token Basic authentication, path and query construction,
 * JSON request dispatch, response parsing, and API error normalization.
 */
class TailscaleService
{
    /**
     * @param  string  $apiToken  Tailscale API access token.
     * @param  string  $baseUrl  Tailscale API base URL.
     */
    public function __construct(private string $apiToken = '', private string $baseUrl = 'https://api.tailscale.com/api/v2') { $this->baseUrl = rtrim($this->baseUrl, '/'); }
    ...
    {
        if (!$this->isConfigured()) throw new RuntimeException('Tailscale API token is not configured.');
        try { $method = strtoupper($method); $url = $this->urlWithQuery($this->baseUrl.$path, $query); $http = Http::withHeaders(array_merge(['Authorization'=>'Basic '.base64_encode($this->apiToken.':'),'Content-Type'=>'application/json','Accept'=>'application/json'], $headers))->timeout(120); $response = match ($method) { 'GET'=>$http->get($url), 'POST'=>$http->post($url,$body), 'PUT'=>$http->put($url,$body), 'PATCH'=>$http->patch($url,$body), 'DELETE'=>$http->delete($url,$body), default=>throw new RuntimeException("Unsupported HTTP method: {$method}"), }; if (!$response->successful()) { $error = $response->json('message') ?? $response->json('error') ?? $response->body(); Log::error("Tailscale API error: {$method} {$path}", ['status'=>$response->status(),'error'=>$error]); throw new RuntimeException('Tailscale API error ('.$response->status().'): '.(is_string($error)?$error:json_encode($error))); } return $response; } catch (\Illuminate\Http\Client\ConnectionException $e) { Log::error("Tailscale API connection error: {$method} {$path}", ['error'=>$e->getMessage()]); throw new RuntimeException("Failed to connect to Tailscale API: {$e->getMessage()}"); }
    }
