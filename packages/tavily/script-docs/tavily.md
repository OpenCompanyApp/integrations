# Tavily — JavaScript API Reference
...
```js
var result = app.integrations.tavily.search({
...
```js
var extracted = app.integrations.tavily.extract({
  urls: [
...
```js
var crawl = app.integrations.tavily.crawl({
...
```js
var mapped = app.integrations.tavily.map({
  url: "https://example.test/docs",
  select_paths: ["/docs/.*"],
  limit: 100,
})
...
```js
var task = app.integrations.tavily.create_research_task({
...
```js
var report = app.integrations.tavily.get_research_task({
...
```js
var usage = app.integrations.tavily.get_usage()
console.log(usage.key && usage.key.usage)
```
## Multi-Account Usage
