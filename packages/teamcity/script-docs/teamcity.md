...
```ruby
server = app.integrations.teamcity.get_server_info()
```
List recent failed builds:

```ruby
builds = app.integrations.teamcity.list_builds(locator: "status:FAILURE,count:10", fields: "build(id,number,status,state,webUrl,buildTypeId)")
```
Trigger a build:

```ruby
queued = app.integrations.teamcity.queue_build(payload: {buildType: {id: "Project_Build"}, branchName: "main"})
```
Cancel a running build:

```ruby
app.integrations.teamcity.cancel_build(locator: "id:12345", payload: {comment: "Canceled by automation", readdIntoQueue: false})
```
Pause the build queue:

```ruby
app.integrations.teamcity.set_queue_paused(paused: true, reason: "Maintenance window")
```
## Locator Notes
...
```ruby
result = app.integrations.teamcity.api_get(path: "/projects", query: {fields: "project(id,name,href)"})
```
