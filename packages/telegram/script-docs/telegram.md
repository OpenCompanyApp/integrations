...
```ruby
msg = app.integrations.telegram.send_message(chat_id: "123456789", text: "Hello from the AI agent! 🤖", parse_mode: "Markdown")
puts("Sent message ID: " + (msg.message_id).to_s)
```
---
...
```ruby
updates = app.integrations.telegram.list_updates(limit: 10)
updates.each do |update|
  if update.message
    puts("From: " + (update.message.from.first_name).to_s + " — " + (update.message.text).to_s)
  end
end
```
---
...
```ruby
bot = app.integrations.telegram.get_bot_info()
puts("Bot: @" + (bot.username).to_s + " (" + (bot.first_name).to_s + ")")
```
---
...
```ruby
chats = app.integrations.telegram.list_chats(limit: 50)
chats.each do |chat|
  name = (chat.title || (chat.first_name || "Unknown"))
  puts((name).to_s + " (" + (chat.type).to_s + ") — " + (chat.id).to_s)
end
```
---
...
```ruby
chat = app.integrations.telegram.get_chat(chat_id: "-1001234567890")
puts("Chat: " + ((chat.title || "DM")).to_s + " (" + (chat.type).to_s + ")")
if chat.member_count
  puts("Members: " + (chat.member_count).to_s)
end
```
---
...
```ruby
msg = app.integrations.telegram.send_photo(chat_id: "123456789", photo: "https://example.com/photo.jpg", caption: "Check this out! 📸", parse_mode: "Markdown")
puts("Sent photo, message ID: " + (msg.message_id).to_s)
```
---
...
```ruby
bot = app.integrations.telegram.get_current_user()
puts("Bot: @" + (bot.username).to_s + " (" + (bot.first_name).to_s + ")")
```
---
