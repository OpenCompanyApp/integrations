...
/**
 * Shared executor for Temporal endpoint-specific tools.
 *
 ...
 */
abstract class AbstractTemporalTool implements Tool
{
    ...
    /** @param  TemporalService  $service  Temporal API client. */ public function __construct(protected TemporalService $service) {}
    ...
    /** @param  array<string, mixed>  $args  Tool arguments. @return array<string, mixed> */ private function body(array $args): array { $body=$args['body']??[]; if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the Temporal API request schema.'); if($body!==[]&&!is_array($body)) throw new InvalidArgumentException('body must be an object.'); return $body; }
}
