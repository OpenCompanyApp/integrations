 ...
 * Describe task queue.
 *
 * Maps to the official Temporal endpoint get /api/v1/namespaces/{namespace}/task-queues/{task_queue.name}.
 */
class TemporalDescribeTaskQueue extends AbstractTemporalTool
{
    protected const NAME = 'temporal_describe_task_queue';
    protected const DESCRIPTION = 'Describe task queue

Official Temporal endpoint: GET /api/v1/namespaces/{namespace}/task-queues/{task_queue.name}
...
DescribeTaskQueue returns the following information about the target task queue, broken down by Build ID:
 ...
 per task type).',
    ...
    protected const PATH = '/api/v1/namespaces/{namespace}/task-queues/{task_queue.name}';
