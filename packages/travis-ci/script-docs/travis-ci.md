...
```ruby
builds = app.call("integrations.travis-ci.list_repository_builds", repository: "acme/web", query: {limit: 10, sort_by: "number:desc", include: "build.commit,build.jobs"})
```
Restart a failed build:

```ruby
app.call("integrations.travis-ci.restart_build", build_id: 86601346)
```
Fetch a plain text job log:

```ruby
log = app.call("integrations.travis-ci.get_job_log", job_id: 86601347, plain_text: true)
```
Trigger a build request:

```ruby
request = app.call("integrations.travis-ci.create_request", repository: "acme/web", payload: {request: {branch: "main", message: "Triggered by agent"}})
```
## Raw API Helpers
...
```ruby
repo = app.call("integrations.travis-ci.api_get", path: "/repo/891", query: {include: "repository.current_build"})
```
