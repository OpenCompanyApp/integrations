# TrustMRR - JavaScript API Reference
...
```js
var result = app.integrations.trustmrr.trustmrr_list_startups({
  category: "saas",
  on_sale: true,
  min_revenue: 50000,
  sort: "revenue-desc",
...
for (const startup of (result.startups)) {
  console.log(startup.name + " - MRR: $" + (startup.revenue.mrr / 100))
}
```
...
```js
var result = app.integrations.trustmrr.trustmrr_get_startup({
  slug: "shipfast",
})

console.log(result.name)
console.log("Revenue: $" + (result.revenue.last30Days / 100))
console.log("MRR: $" + (result.revenue.mrr / 100))
```
---
...
app.integrations.trustmrr.function_name({ /* parameters */ })

// Explicit default (portable across setups)
app.integrations.trustmrr.default.function_name({ /* parameters */ })

// Named accounts
app.integrations.trustmrr.work.function_name({ /* parameters */ })
app.integrations.trustmrr.personal.function_name({ /* parameters */ })
```
