...
```ruby
result = app.integrations.trustmrr.list_startups(category: "saas", on_sale: true, min_revenue: 50000, sort: "revenue-desc", limit: 10)
result.startups.each do |startup|
  puts((startup.name).to_s + " - MRR: $" + ((startup.revenue.mrr / 100)).to_s)
end
```
...
```ruby
result = app.integrations.trustmrr.get_startup(slug: "shipfast")
puts(result.name)
puts("Revenue: $" + ((result.revenue.last30Days / 100)).to_s)
puts("MRR: $" + ((result.revenue.mrr / 100)).to_s)
```
---
