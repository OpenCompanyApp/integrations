...
```ruby
monitors = app.integrations.uptimerobot.monitors_list(status: "UP,DOWN", limit: 50, custom_field: ["environment:production", "team:core"])
created = app.integrations.uptimerobot.monitors_create(body: {name: "Production API", url: "https://example.test/health", type: "HTTP"})
user = app.integrations.uptimerobot.user_get()
```
