...
```js
var submitted = app.integrations.urlscan.submit_scan({
  body: {
...
var results = app.integrations.urlscan.search_datasource({
  q: "page.domain:example.test",
  datasource: "scans",
  size: 10,
})

var quotas = app.integrations.urlscan.get_quotas({})
```
