...
```ruby
submitted = app.integrations.urlscan.submit(body: {url: "https://example.test", visibility: "unlisted", tags: ["triage"]})
results = app.integrations.urlscan.search_datasource(q: "page.domain:example.test", datasource: "scans", size: 10)
quotas = app.integrations.urlscan.get_quotas()
```
