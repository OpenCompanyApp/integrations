# HashiCorp Vault — Ruby API Reference
...
policies = app.integrations.vault.list_policies()
```
## Secrets (KV v2)
...
```ruby
result = app.integrations.vault.list_secrets(engine_path: "secret", path: "myapp")
...
```ruby
result = app.integrations.vault.get_secret(path: "myapp/database")
secret = result.data.data
puts("Username: " + (secret.username).to_s)
puts("Password: " + (secret.password).to_s)
puts("Version: " + (result.data.metadata.version).to_s)
```
Retrieve a specific version:

```ruby
result = app.integrations.vault.get_secret(path: "myapp/database", version: 3)
puts("Version 3 password: " + (result.data.data.password).to_s)
```
...
```ruby
result = app.integrations.vault.create_secret(path: "myapp/database", data: {username: "admin", password: "s3cret_p@ssw0rd", host: "db.example.com", port: 5432})
puts("Created version: " + (result.data.version).to_s)
```
Store structured configuration:

```ruby
result = app.integrations.vault.create_secret(path: "myapp/config", data: {debug: false, log_level: "info", max_connections: 100, allowed_origins: ["https://example.com", "https://app.example.com"]})
```
...
```ruby
result = app.integrations.vault.delete_secret(path: "myapp/database")
if result.success
...
```ruby
result = app.integrations.vault.list_policies()
result.data.policies.each do |name|
  puts(name)
end
```
...
```ruby
result = app.integrations.vault.get_policy(name: "my-app-policy")
puts("Policy: " + (result.data.name).to_s)
puts("Rules:\n" + (result.data.rules).to_s)
```
## Token Information
...
```ruby
result = app.integrations.vault.get_current_user()
info = result.data
puts("Display name: " + (info.display_name).to_s)
puts("Policies: " + (info.policies.join(", ")).to_s)
puts("TTL: " + (info.ttl).to_s + "s")
puts("Renewable: " + ((info.renewable).to_s).to_s)
```
## Common Workflows
...
```ruby
# 1. Store credentials
app.integrations.vault.create_secret(path: "production/database", data: {username: "app_user", password: "generated_secure_password", host: "prod-db.internal", port: 5432, database: "myapp"})
# 2. Retrieve credentials when needed
result = app.integrations.vault.get_secret(path: "production/database")
creds = result.data.data
dsn = "postgresql://" + ("postgresql://%s:%s@%s:%d/%s").to_s + ":" + (creds.username).to_s + "@" + (creds.password).to_s + ":" + (creds.host).to_s + "/" + (creds.port).to_s + ""
puts("Credentials loaded; connection string intentionally not logged")
```
### Rotate a secret
...
current = app.integrations.vault.get_secret(path: path)
puts("Current version: " + (current.data.metadata.version).to_s)
# 2. Write new secret (creates a new version)
result = app.integrations.vault.create_secret(path: path, data: {api_key: "new_rotated_key_xyz", rotated_at: "2026-01-01T00:00:00Z"})
puts("New version: " + (result.data.version).to_s)
```
### Audit policies
...
result.data.policies.each do |name|
  if (name != "root")
    policy = app.integrations.vault.get_policy(name: name)
    puts("=== " + (name).to_s + " ===")
    puts(policy.data.rules)
    puts()
  end
end
```
...
result = app.integrations.vault.get_current_user()
info = result.data
puts("Token display name: " + (info.display_name).to_s)
puts("Token policies: " + (info.policies.join(", ")).to_s)
if (info.ttl < 300)
  puts("Warning: token expires in less than 5 minutes!")
end
```
## Response Structure
...
result = app.integrations.vault.get_secret(path: "myapp/config")
...
result = app.integrations.vault.list_secrets(path: "myapp")
