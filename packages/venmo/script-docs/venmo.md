...
```ruby
result = app.integrations.venmo.list_payments(limit: 20, offset: 0)
```
...
```ruby
payment = app.integrations.venmo.get_payment(id: "1234567890")
```
...
```ruby
payment = app.integrations.venmo.create_payment(amount: 25, user_id: "1234567890", note: "Dinner", audience: "private")
```
...
```ruby
result = app.integrations.venmo.list_users(query: "janesmith", limit: 10)
```
...
```ruby
user = app.integrations.venmo.get_user(id: "1234567890")
```
## Transactions
...
```ruby
result = app.integrations.venmo.list_transactions(action: "pay", limit: 20)
```
...
```ruby
me = app.integrations.venmo.get_current_user()
```
## Notes
