 ...
 */
class VeroApiDelete implements Tool
{
    /**
    ...
    {
        return 'vero_api_delete';
    }

    public function description(): string
    {
        return 'Call a relative Vero API DELETE path. Use for documented endpoints not yet exposed as first-class tools.';
    }

    public function parameters(): array
    {
        return [
            'path' => ['type' => 'string', 'required' => true, 'description' => 'Relative Vero API path. Absolute URLs are rejected.'],
            'payload' => ['type' => 'object', 'description' => 'Optional JSON request body. auth_token is added as a query parameter automatically.'],
        ];
    }
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            return ToolResult::success($this->service->apiDelete($path, $args['payload'] ?? []));
        } catch (\Throwable $e) {
