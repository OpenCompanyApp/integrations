...
```ruby
result = app.call("integrations.voyage-ai.create_embedding", input: ["Install guide", "API reference"], model: "voyage-4", input_type: "document", output_dimension: 1024)
```
...
```ruby
result = app.call("integrations.voyage-ai.create_contextualized_embeddings", inputs: [["Intro chunk", "Setup chunk", "API chunk"]], model: "voyage-context-3", input_type: "document")
```
...
```ruby
result = app.call("integrations.voyage-ai.create_multimodal_embeddings", model: "voyage-multimodal-3.5", inputs: [{content: [{type: "text", text: "This is a product screenshot."}, {type: "image_url", image_url: "https://example.test/screenshot.png"}]}])
```
...
```ruby
ranked = app.call("integrations.voyage-ai.rerank", query: "How do I upload files?", documents: ["Use the Files API.", "Use the Billing API."], model: "rerank-2.5", top_k: 1, return_documents: true)
```
## upload_file
...
```ruby
file = app.call("integrations.voyage-ai.upload_file", filename: "batch-input.jsonl", content: "{\"custom_id\":\"row-1\",\"body\":{\"input\":\"hello\"}}" + "\n", purpose: "batch")
```
## list_files
...
```ruby
files = app.call("integrations.voyage-ai.list_files", purpose: "batch", limit: 20, order: "desc")
```
## retrieve_file
...
```ruby
file = app.call("integrations.voyage-ai.retrieve_file", file_id: "file_abc123")
```
...
```ruby
content = app.call("integrations.voyage-ai.retrieve_file_content", file_id: "file_output123", accept: "text/plain")
```
## delete_file
...
```ruby
app.call("integrations.voyage-ai.delete_file", file_id: "file_abc123")
```
## bulk_delete_files
...
```ruby
app.call("integrations.voyage-ai.bulk_delete_files", file_ids: ["file_abc123", "file_def456"])
```
## create_batch
...
```ruby
batch = app.call("integrations.voyage-ai.create_batch", endpoint: "v1/embeddings", input_file_id: "file_abc123", completion_window: "12h", request_params: {model: "voyage-4", input_type: "document"}, metadata: {corpus: "docs"})
```
## list_batches
...
```ruby
batches = app.call("integrations.voyage-ai.list_batches", limit: 20)
```
## retrieve_batch
...
```ruby
batch = app.call("integrations.voyage-ai.retrieve_batch", batch_id: "batch_abc123")
```
## cancel_batch
...
```ruby
app.call("integrations.voyage-ai.cancel_batch", batch_id: "batch_abc123")
```
## Multi-Account Usage
...
```ruby
app.call("integrations.voyage-ai.create_embedding")
app.call("integrations.voyage-ai.default.create_embedding")
app.call("integrations.voyage-ai.research.create_embedding")
```