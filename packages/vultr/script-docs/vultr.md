...
```ruby
account = app.integrations.vultr.get_account_info()
puts(account.account.email)
instances = app.integrations.vultr.list_instances(per_page: 50)
(instances.instances || []).each do |instance|
  puts((instance.id).to_s + " " + (instance.label).to_s + " " + (instance.status).to_s)
end
instance = app.integrations.vultr.get_instance(id: "11111111-2222-3333-4444-555555555555")
puts(instance.instance.main_ip)
plans = app.integrations.vultr.list_plans(type: "vc2")
regions = app.integrations.vultr.list_regions()
```
## Generated Tool Shape

Tool names follow the upstream operation name, normalized to snake_case with a `vultr_` prefix in metadata and exposed without the prefix in Ruby. For example, the upstream `create-instance` operation is available as `app.integrations.vultr.create_instance(...)`.
...
```ruby
created = app.integrations.vultr.create_instance(region: "ewr", plan: "vc2-1c-1gb", os_id: 1743, label: "example-agent-node")
app.integrations.vultr.update_instance(id: created.instance.id, label: "renamed-agent-node")
```
## Coverage Notes
