# Cisco Webex Ruby API Reference
...
```ruby
rooms = app.integrations.webex.list_rooms(max: 20)
(rooms.items || []).each do |room|
  puts((room.id).to_s + " " + (room.title).to_s)
end
```
Create a message with either `text`, `markdown`, or both:

```ruby
app.integrations.webex.create_message(room_id: "room_123", text: "Summary report attached.", markdown: "**Summary report**\n\n* Revenue is up\n* Churn is flat")
```
Pass official Webex fields that are not first-class parameters through
`payload` on create/update tools:

```ruby
app.integrations.webex.create_webhook(name: "Room messages", target_url: "https://example.test/webex", resource: "messages", event: "created", filter: "roomId=room_123")
```
## Rooms
