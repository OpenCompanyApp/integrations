...
```ruby
result = app.integrations.wildix.get_personal_info()
```
...
```ruby
calls = app.integrations.wildix.list_user_active_calls()
```
### Make a call

```ruby
result = app.integrations.wildix.call_control_make_call(destination: "+15550101000", device: "Web")
```
### Get PBX colleagues with filters

```ruby
users = app.integrations.wildix.get_pbx_colleagues(count: 25, start: 0, query: {"filter[email][]" => ["ada@example.test"]})
```
### Create an OAuth2 client

```ruby
client = app.integrations.wildix.create_pbx_o_auth2_client(name: "Agent App", redirect_uri: ["https://example.test/oauth/callback"])
```
## Notes
