...
```ruby
countries = app.integrations.worldbank.list_countries(region: "LCN", income_level: "UMC")
```
...
```ruby
brazil = app.integrations.worldbank.country_info(code: "BR")
```
...
```ruby
source_indicators = app.integrations.worldbank.source_indicators(source_id: "2", per_page: 25)
```
...
```ruby
gdp = app.integrations.worldbank.indicator_info(indicator: "NY.GDP.MKTP.CD")
```
...
```ruby
data = app.integrations.worldbank.get_indicator_data(indicator: "SP.POP.TOTL", countries: "US;CN;BR", date_range: "2020:2023")
```
...
```ruby
data = app.integrations.worldbank.multi_indicator_data(countries: "CHN;AGO", indicators: "SI.POV.DDAY;SP.POP.TOTL", source: "2", date_range: "2000:2010")
```
### Compare Countries
...
```ruby
comparison = app.integrations.worldbank.compare_countries(indicator: "NY.GDP.MKTP.CD", countries: "US;CN;DE;JP")
```
