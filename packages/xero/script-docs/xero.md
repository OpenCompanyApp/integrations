# Client for the Xero Accounting REST API — Ruby API Reference
...
```ruby
result = app.integrations.xero.list_invoices(page: 1, page_size: 50, statuses: "AUTHORISED")
```
## xero_get_invoice
...
```ruby
result = app.integrations.xero.get_invoice(invoice_id: "abc123-def456-...")
```
## xero_create_invoice
...
```ruby
result = app.integrations.xero.create_invoice(contact_id: "abc123-def456-...", type: "ACCREC", date: "2026-04-07", due_date: "2026-05-07", reference: "Order #12345", line_items: [{description: "Consulting services", quantity: 10, unit_amount: 150, account_code: "200"}, {description: "Software license", quantity: 1, unit_amount: 500, account_code: "400"}], status: "AUTHORISED")
```
## xero_list_contacts
...
```ruby
result = app.integrations.xero.list_contacts(page: 1, order: "Name ASC")
```
## xero_get_contact
...
```ruby
result = app.integrations.xero.get_contact(contact_id: "abc123-def456-...")
```
## xero_list_accounts
...
```ruby
result = app.integrations.xero.list_accounts(order: "Code ASC")
```
...
```ruby
result = app.integrations.xero.get_current_user()
```
---
