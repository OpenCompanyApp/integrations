...
/**
 * Shared executor for YouTube endpoint-specific tools.
 *
 ...
 */
abstract class AbstractYouTubeTool implements Tool
{
    ...
    /**
     * @param  YouTubeService  $service  YouTube Data API client.
     */
    public function __construct(protected YouTubeService $service) {}
    ...
    /**
     * Execute the mapped YouTube REST method.
     *
    ...
    {
        try {
            if (!$this->service->isConfigured()) return ToolResult::error('YouTube integration is not configured.');
    ...
    {
        $body=$args['body']??[];
        if(static::BODY_REQUIRED&&(!is_array($body)||$body===[])) throw new InvalidArgumentException('body must be a non-empty object matching the YouTube Data API request schema.');
