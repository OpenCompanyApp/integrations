...
/**
 * Base class for focused Zendesk Sell REST endpoint tools.
 *
 * Handles configuration checks, required parameters, path interpolation,
 * query/body shaping, and the API's documented data envelope for writes.
 */
abstract class AbstractZendeskSellEndpointTool implements Tool
{
    ...
    protected bool $wrapData = true;

    /**
     * @param  ZendeskSellService  $service  The Zendesk Sell API client.
     */
    public function __construct(
        protected ZendeskSellService $service,
    ) {}
    ...
    /**
     * Execute one Zendesk Sell REST API operation.
     *
    ...
    public function execute(array $args): ToolResult
    {
        try {
            ...
            if ($this->wrapData && in_array($this->method, ['POST', 'PUT'], true)) {
                $body = ['data' => $body];
            }
            ...
            $result = match ($this->method) {
                ...
                default => throw new RuntimeException("Unsupported Zendesk Sell tool method: {$this->method}"),
            };
